#!/usr/bin/env python3
"""RUST-087 TEST-ONLY monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-087 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_087_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_monitor_set_rotation_fixture.py SOURCE_SHA"
        )
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust086-monitor-bundle.json",
        "RUST-087 predecessor monitor bundle",
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust086-observed-fork-monitor-bundle.json",
        "RUST-087 predecessor observed fork monitor bundle",
    )
    ...
    set_sha = rotation_verify.sha256(
        material_verify.canonical(rotation_verify.new_monitor_set())
    )
    reports = [
    ...
    fork_statement = next(
        ...
        if any(
            report["statement"][key] != target[key]
    ...
    (OUT / "axven-rust087-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust087-monitor-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust087-successor-monitor-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust087-successor-fork-bundle.json").write_bytes(
        material_verify.canonical(
            successor_bundle([reports[0], reports[1], fork_report], set_sha)
        )
    )
    print("RUST-087 TEST-only monitor-set rotation fixture: GREEN")
