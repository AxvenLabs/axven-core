#!/usr/bin/env python3
"""RUST-071 TEST-ONLY monitor-rotation-journal observer-set rotation producer."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-071 TEST-only monitor-rotation-journal observer public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_071_monitor_rotation_journal_observer_set_rotation_fixture.py SOURCE_SHA")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        ...
        "RUST-070 predecessor observation bundle",
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust070-observed-fork-bundle.json",
        "RUST-070 predecessor observed fork bundle",
    )
    ...
    (OUT / "axven-rust071-monitor-rotation-journal-observer-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust071-monitor-rotation-journal-observer-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust071-successor-observer-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust071-successor-fork-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-071 TEST-only monitor-rotation-journal observer-set rotation fixture: GREEN")
