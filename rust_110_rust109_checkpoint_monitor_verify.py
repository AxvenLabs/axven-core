#!/usr/bin/env python3
"""RUST-110: TEST-ONLY RUST-109 monitor-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-rust110-monitor-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-rust110-monitor-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-rust110-monitor-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST110_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
MONITOR_1_PUBLIC_KEY = bytes.fromhex("fcbe38632417bb5b875d49a3c02270633917c6093fd01ebb59ff6416e37afe0c")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("23b631709df22832a04b29a1677ab761058ecb42329e14fd484269e63fe55c60")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("f9b439834644d493ba526939f77f7cdadf5220486962e808867bac0a513b92b8")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust109_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust109_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-110 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-110 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-110 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-110 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-110 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-110 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-110 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-109 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-110 monitor report does not match canonical RUST-109 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-110 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-110 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-110 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 213:
        raise AssertionError("unexpected RUST-110 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 211:
        raise AssertionError("unexpected RUST-110 path count")
    base_paths = path_args[:210]
    monitor_bundle_path = path_args[210]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[208], "RUST-109 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 215 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_110_rust109_checkpoint_monitor_verify.py verify "
            "... RUST109_FINAL_CHECKPOINT MONITOR_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
