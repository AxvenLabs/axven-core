#!/usr/bin/env python3
"""RUST-130 TEST-ONLY RUST-129 checkpoint monitor producer."""
...
def private_for(monitor_id: str) -> Ed25519PrivateKey:
    ...
    if public != monitor_verify.PINNED_MONITORS[monitor_id]:
        raise AssertionError("RUST-130 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_130_rust129_checkpoint_monitor_fixture.py SOURCE_SHA")
    source_sha = sys.argv[1]

    final_journal_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust129-final-monitor-rotation-journal.json",
        "RUST-129 final monitor rotation journal",
    )
    ...
    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust129-observed-fork-monitor-rotation-checkpoint.json",
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-130 canonical target source mismatch")
    ...
    (OUT / "axven-rust130-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle(reports))
    )
    (OUT / "axven-rust130-observed-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle([reports[0], reports[1], fork_report]))
    )
    print("RUST-130 TEST-only RUST-129 journal checkpoint monitor fixture: GREEN")
