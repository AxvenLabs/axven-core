#!/usr/bin/env python3
"""RUST-132 static policy for TEST-ONLY second RUST-130 checkpoint monitor rotation."""
...
DOC = ROOT / "docs/history/rust/RUST_132.md"
VERIFY = ROOT / "rust_132_multistep_rust130_checkpoint_monitor_rotation_verify.py"
FIXTURE = ROOT / "rust_132_multistep_rust130_checkpoint_monitor_rotation_fixture.py"
SELFTEST = ROOT / "rust_132_multistep_rust130_checkpoint_monitor_rotation_selftest.py"
WORKFLOW = ROOT / ".github/workflows/native-rust132-multistep-checkpoint-monitor-rotation.yml"
BASE = ROOT / "rust_131_rust130_checkpoint_monitor_rotation_verify.py"
PREDECESSOR_WORKFLOW = ROOT / ".github/workflows/native-rust131-rust130-checkpoint-monitor-rotation.yml"
EXPECTED_RUST131_GIT_BLOB = "234484e7bf869f8007cc7585d1afaecac8c9de03"
EXPECTED_RUST131_WORKFLOW_GIT_BLOB = "5f3b8b9e04605e347c152df5e1997d5bda56cee1"
...
def main() -> None:
    ...
    assert blob(BASE.read_bytes()) == EXPECTED_RUST131_GIT_BLOB
    assert blob(PREDECESSOR_WORKFLOW.read_bytes()) == EXPECTED_RUST131_WORKFLOW_GIT_BLOB
    require(
        verify,
        ("import rust_131_rust130_checkpoint_monitor_rotation_verify as rotation1_verify",),
        "RUST-132 verifier composition",
    )
    checks += 1
    print("[GREEN] exact reviewed RUST-131 verifier and workflow are pinned")
    ...
    checks += 1
    print("[GREEN] detached RUST-132 verifier/selftest have no signing or network capability")
    ...
    require(
        ...
        (
            ...
            "Ed25519PrivateKey", "RUST-132 TEST-only monitor public-key pin mismatch",
        ),
        "RUST-132 producer fixture",
    )
    require(
        selftest,
        (
    ...
    require(
        ...
        (
            ...
            'python-version: "3.13.15"', "chmod 0444", "axven-rust132",
    ...
    require(
        ...
        (
            ...
            "fixed 272-path manifest", "105-file verifier-only detached consumer",
            "Production consensus remains Python-authoritative.",
        ),
        "RUST-132 documentation",
    )
    checks += 1
    print("[GREEN] documentation preserves TEST-only multi-step rotation boundary")

    assert checks == 6
    print("RUST-132 static policy: 6/6 checks passed")
