#!/usr/bin/env python3
"""RUST-093 detached availability and fail-closed journal selftest."""
...
def main() -> None:
    if len(sys.argv) != 170:
        raise SystemExit(
            "usage: rust_093_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_monitor_rotation_journal_monitor_rotation_journal_selftest.py "
    ...
    if len(base) != 166:
        raise AssertionError("unexpected RUST-093 selftest base path count")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(base[155], "RUST-093 old monitor bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base[156], "RUST-093 first rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base[157], "RUST-093 first auth")
    first_successor_raw, _ = floor_verify.load_canonical(base[158], "RUST-093 first successor")
    second_rotation_raw, _ = floor_verify.load_canonical(base[159], "RUST-093 second rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base[160], "RUST-093 second auth")
    final_bundle_raw, _ = floor_verify.load_canonical(base[161], "RUST-093 final bundle")
    ...
    prefix_journal_raw, prefix_journal = floor_verify.load_canonical(base[162], "RUST-093 prefix journal")
    prefix_checkpoint_raw, prefix_checkpoint = floor_verify.load_canonical(base[163], "RUST-093 prefix checkpoint")
    final_journal_raw, final_journal = floor_verify.load_canonical(base[164], "RUST-093 final journal")
    final_checkpoint_raw, final_checkpoint = floor_verify.load_canonical(base[165], "RUST-093 final checkpoint")
    fork_raw, fork_checkpoint = floor_verify.load_canonical(fork_path, "RUST-093 observed fork checkpoint")
    ...
    for subset in itertools.combinations(prefix_checkpoint["monitors"], journal_verify.THRESHOLD):
        ...
        prefix_ok += 1
    if prefix_ok != 3:
        raise AssertionError("unexpected RUST-093 prefix subset count")
    print("[GREEN] RUST-093 prefix checkpoint availability: 3/3 valid two-monitor subsets accepted")
    ...
    for subset in itertools.combinations(final_checkpoint["monitors"], journal_verify.THRESHOLD):
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-093 final subset count")
    print("[GREEN] RUST-093 final checkpoint availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("prefix-genesis-set-rewrite", lambda: run_with(162, write("a.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_sha256"] = "0" * 64
        fail("prefix-first-rotation-digest", lambda: run_with(162, write("b.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_auth_sha256"] = "0" * 64
        fail("prefix-first-auth-digest", lambda: run_with(162, write("c.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitored_checkpoint_sha256"] = "0" * 64
        fail("prefix-monitored-checkpoint", lambda: run_with(162, write("d.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitored_checkpoint_statement_sha256"] = "0" * 64
        fail("prefix-monitored-statement", lambda: run_with(162, write("e.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["observed_target_sha256"] = "0" * 64
        fail("prefix-observed-target", lambda: run_with(162, write("f.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["activation_source_commit"] = "0" * 40
        fail("prefix-source", lambda: run_with(162, write("g.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["production"] = True
        fail("prefix-production", lambda: run_with(162, write("h.json", value))); cases += 1

        value = copy.deepcopy(final_journal); value["entries"] = value["entries"][:2]
        fail("final-entry-truncation", lambda: run_with(164, write("i.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][0]["monitor_bundle_sha256"] = "0" * 64
        fail("final-prefix-rewrite", lambda: run_with(164, write("j.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["sequence"] = 1
        fail("final-sequence-rollback", lambda: run_with(164, write("k.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["predecessor_entry_sha256"] = "0" * 64
        fail("final-predecessor-entry", lambda: run_with(164, write("l.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_sha256"] = "0" * 64
        fail("final-rotation-digest", lambda: run_with(164, write("m.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_auth_sha256"] = "0" * 64
        fail("final-rotation-auth-digest", lambda: run_with(164, write("n.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["monitor_bundle_sha256"] = "0" * 64
        fail("final-monitor-bundle", lambda: run_with(164, write("o.json", value))); cases += 1
        ...
        fail("final-revocation-omission", lambda: run_with(164, write("p.json", value))); cases += 1

        value = copy.deepcopy(prefix_checkpoint); value["threshold"] = 1
        fail("prefix-threshold", lambda: run_with(163, write("q.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["monitors"] = value["monitors"][:1]
        fail("prefix-below-threshold", lambda: run_with(163, write("r.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["monitors"] = [value["monitors"][0], copy.deepcopy(value["monitors"][0])]
        fail("prefix-duplicate", lambda: run_with(163, write("s.json", value))); cases += 1
        ...
        fail("prefix-signature", lambda: run_with(163, write("t.json", value))); cases += 1

        value = copy.deepcopy(final_checkpoint); value["statement"]["previous_checkpoint_sha256"] = "0" * 64
        fail("final-previous-checkpoint", lambda: run_with(165, write("u.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["head_entry_sha256"] = "0" * 64
        fail("final-head-entry", lambda: run_with(165, write("v.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitored_checkpoint_sha256"] = "0" * 64
        fail("final-monitored-checkpoint", lambda: run_with(165, write("w.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitored_checkpoint_statement_sha256"] = "0" * 64
        fail("final-monitored-statement", lambda: run_with(165, write("x.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observed_target_sha256"] = "0" * 64
        fail("final-observed-target", lambda: run_with(165, write("y.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["activation_source_commit"] = "0" * 40
        fail("final-source", lambda: run_with(165, write("z.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["production"] = True
        fail("final-production", lambda: run_with(165, write("aa.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitor_set_sequence"] = 1
        fail("final-monitor-set-sequence", lambda: run_with(165, write("ab.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitor_set_sha256"] = "0" * 64
        fail("final-monitor-set-digest", lambda: run_with(165, write("ac.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["threshold"] = 1
        fail("final-threshold", lambda: run_with(165, write("ad.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["monitors"] = value["monitors"][:1]
        fail("final-below-threshold", lambda: run_with(165, write("ae.json", value))); cases += 1
        ...
        fail("final-signature", lambda: run_with(165, write("af.json", value))); cases += 1

        noncanonical_journal = root / "noncanonical-journal.json"
        noncanonical_journal.write_text(json.dumps(final_journal, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-journal", lambda: run_with(164, noncanonical_journal)); cases += 1
        ...
        fail("noncanonical-final-checkpoint", lambda: run_with(165, noncanonical_checkpoint)); cases += 1
    ...
    if cases != 35:
        raise AssertionError(f"unexpected RUST-093 selftest case count: {cases}")
    print("RUST-093 monitor rotation journal fail-closed contract: 35/35 expected cases passed")
