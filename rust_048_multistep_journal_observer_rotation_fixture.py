#!/usr/bin/env python3
"""RUST-048 TEST-ONLY second journal-observer rotation producer."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-048 TEST-only journal-observer public-key pin mismatch")
    return private
    ...
    return [
        {
            ...
            "signature": base64.b64encode(private_for(observer_id, rotation2_verify.PREDECESSOR_PINNED_OBSERVERS).sign(message)).decode("ascii"),
        }
...
def final_report(observer_id: str, target: dict, set_sha: str) -> dict:
    ...
    return {
        ...
        "signature": base64.b64encode(
            private_for(observer_id, rotation2_verify.FINAL_PINNED_OBSERVERS).sign(rotation2_verify.final_message(statement))
        ).decode("ascii"),
    }
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(c not in "0123456789abcdef" for c in sys.argv[1]):
        raise SystemExit("usage: rust_048_multistep_journal_observer_rotation_fixture.py SOURCE_SHA")
    ...
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust047-successor-journal-observer-bundle.json", "first successor bundle"
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust046-observed-fork-bundle.json", "observed fork bundle"
    )
    ...
    fork_report = final_report(rotation2_verify.J5_ID, fork_target, set_sha)

    (OUT / "axven-rust048-second-journal-observer-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust048-second-journal-observer-set-rotation-auth.json").write_bytes(material_verify.canonical(auth))
    (OUT / "axven-rust048-final-journal-observer-bundle.json").write_bytes(material_verify.canonical(bundle(reports, set_sha)))
    (OUT / "axven-rust048-final-fork-bundle.json").write_bytes(material_verify.canonical(bundle([reports[0], reports[1], fork_report], set_sha)))
    print("RUST-048 TEST-only second journal-observer rotation fixture: GREEN")
