#!/usr/bin/env python3
"""RUST-112 static policy for TEST-ONLY second RUST-110 checkpoint monitor rotation."""
...
DOC = ROOT / "docs/history/rust/RUST_112.md"
VERIFY = ROOT / "rust_112_multistep_rust110_checkpoint_monitor_rotation_verify.py"
FIXTURE = ROOT / "rust_112_multistep_rust110_checkpoint_monitor_rotation_fixture.py"
SELFTEST = ROOT / "rust_112_multistep_rust110_checkpoint_monitor_rotation_selftest.py"
WORKFLOW = ROOT / ".github/workflows/native-rust112-multistep-checkpoint-monitor-rotation.yml"
BASE = ROOT / "rust_111_rust110_checkpoint_monitor_rotation_verify.py"
PREDECESSOR_WORKFLOW = ROOT / ".github/workflows/native-rust111-rust110-checkpoint-monitor-rotation.yml"
EXPECTED_RUST111_GIT_BLOB = "4d2cf9311815536f3e689cfb5a1debb348acd39c"
EXPECTED_RUST111_WORKFLOW_GIT_BLOB = "e64b3506d006cf230149df3f322860ed8808003d"
...
def main() -> None:
    ...
    assert blob(BASE.read_bytes()) == EXPECTED_RUST111_GIT_BLOB
    assert blob(PREDECESSOR_WORKFLOW.read_bytes()) == EXPECTED_RUST111_WORKFLOW_GIT_BLOB
    require(
        verify,
        ("import rust_111_rust110_checkpoint_monitor_rotation_verify as rotation1_verify",),
        "RUST-112 verifier composition",
    )
    checks += 1
    print("[GREEN] exact reviewed RUST-111 verifier and workflow are pinned")
    ...
    checks += 1
    print("[GREEN] detached RUST-112 verifier/selftest have no signing or network capability")
    ...
    require(
        ...
        (
            ...
            "final_monitor_set_sha256", "observed RUST-112 final same-parent checkpoint fork",
            "*monitor_verify.TARGET_KEYS",
            "base_paths[209]", "base_paths[211]", "base_paths[212]", "base_paths[213]",
            "path_args[214:217]",
        ),
        "RUST-112 verifier",
    )
    ...
    require(
        ...
        (
            ...
            "Ed25519PrivateKey", "RUST-112 TEST-only monitor public-key pin mismatch",
        ),
        "RUST-112 producer fixture",
    )
    require(
        selftest,
        (
    ...
    require(
        ...
        (
            ...
            'python-version: "3.13.15"', "chmod 0444", "axven-rust112",
            "env -i HOME=/tmp PATH=/usr/bin:/bin", "/usr/bin/python3 -S",
            '"rust_*.py"', '"RUST_*.md"', 'printf -v n3 "%03d" "$n"',
            'test ! -e "$c/rust_112_multistep_rust110_checkpoint_monitor_rotation_fixture.py"',
            'test "$(find "$c" -maxdepth 1 -type f | wc -l)" -eq 85',
            "expected 214 RUST-111 paths", "expected 217 RUST-112 paths",
            "axven-rust112-final-monitor-bundle.json",
        ),
        "RUST-112 workflow",
    )
    for forbidden in (
    ...
    require(
        ...
        (
            ...
            "fixed 217-path manifest", "85-file verifier-only detached consumer",
            "Production consensus remains Python-authoritative.",
        ),
        "RUST-112 documentation",
    )
    checks += 1
    print("[GREEN] documentation preserves TEST-only multi-step rotation boundary")

    assert checks == 6
    print("RUST-112 static policy: 6/6 checks passed")
