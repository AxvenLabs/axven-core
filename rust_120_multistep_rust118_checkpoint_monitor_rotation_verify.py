#!/usr/bin/env python3
"""RUST-120: TEST-ONLY second monitor-set rotation for RUST-118 checkpoint monitors."""
...
ROTATION_SCHEMA = "axven-native-rust120-monitor-set-rotation-v2"
ROTATION_AUTH_SCHEMA = "axven-native-rust120-monitor-set-rotation-quorum-v2"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.rust120-monitor-set-rotation.v2+json"
FINAL_BUNDLE_SCHEMA = "axven-native-rust120-checkpoint-monitor-bundle-v3"
FINAL_REPORT_SCHEMA = "axven-native-rust120-checkpoint-monitor-report-v3"
FINAL_STATEMENT_SCHEMA = "axven-native-rust120-checkpoint-monitor-statement-v3"
ROTATION_DOMAIN = b"AXVEN_NATIVE_RUST120_MONITOR_SET_ROTATION_V2\x00"
FINAL_DOMAIN = b"AXVEN_NATIVE_RUST120_CHECKPOINT_MONITOR_V3\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
PREDECESSOR_SET_SEQUENCE = 1
FINAL_SET_SEQUENCE = 2

M5_ID = "rust-120-test-only-monitor-rotation-journal-monitor-5-v1"
M5_PUBLIC_KEY = bytes.fromhex("8b237d788e8eaaef550c6d125823fa45f1fd5fc29b2c88bdf871119471fc1312")
...
def validate_rotation(
    ...
    if (
        ...
        raise AssertionError("invalid RUST-120 second monitor rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != FINAL_SET_SEQUENCE:
        raise AssertionError("invalid RUST-120 second monitor rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(rotation1_verify.new_monitor_set())):
        raise AssertionError("RUST-120 predecessor monitor set mismatch")
    if rotation.get("to_set") != final_monitor_set():
        raise AssertionError("unexpected RUST-120 final monitor set")
    if rotation.get("cumulative_revoked_monitor_ids") != CUMULATIVE_REVOKED_MONITOR_IDS:
        raise AssertionError("RUST-120 cumulative monitor revocation mismatch")
    if rotation.get("predecessor_rotation_sha256") != sha256(first_rotation_raw):
        raise AssertionError("RUST-120 predecessor rotation digest mismatch")
    if rotation.get("predecessor_rotation_auth_sha256") != sha256(first_auth_raw):
        raise AssertionError("RUST-120 predecessor rotation authorization digest mismatch")
    if rotation.get("predecessor_successor_bundle_sha256") != sha256(first_successor_raw):
        raise AssertionError("RUST-120 predecessor successor bundle digest mismatch")
    for key in TARGET_KEYS:
        if rotation.get(key) != target[key]:
            raise AssertionError(f"RUST-120 inherited target binding mismatch: {key}")
    if rotation.get("production") is not False:
        raise AssertionError("production second monitor rotation forbidden in RUST-120")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-120 rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid RUST-120 rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-120 rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("RUST-120 rotation authorization payload mismatch")
    rows = auth.get("monitors")
    if not isinstance(rows, list) or not (THRESHOLD <= len(rows) <= len(PREDECESSOR_PINNED_MONITORS)):
        raise AssertionError("invalid RUST-120 rotation authorization size")
    if not all(isinstance(row, dict) and frozenset(row) == AUTH_MONITOR_KEYS for row in rows):
        raise AssertionError("invalid RUST-120 rotation authorization entry")
    ids = [row["monitor_id"] for row in rows]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-120 rotation authorizer ids must be unique and sorted")
    if any(monitor_id not in PREDECESSOR_PINNED_MONITORS for monitor_id in ids):
        raise AssertionError("unknown RUST-120 rotation authorizer")
...
def validate_final_bundle(bundle: dict, target: dict) -> None:
    ...
    if (
        ...
        raise AssertionError("invalid RUST-120 final monitor bundle fields")
    if (
        bundle.get("final_monitor_set_sequence") != FINAL_SET_SEQUENCE
        or bundle.get("final_monitor_set_sha256") != set_sha
    ):
        raise AssertionError("RUST-120 final monitor bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-120 final monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production final monitoring forbidden in RUST-120")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(FINAL_PINNED_MONITORS)):
        raise AssertionError("invalid RUST-120 final monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-120 final monitor ids must be unique and sorted")
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-120 final monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed RUST-120 final same-parent checkpoint fork")
        if not exact:
            raise AssertionError("RUST-120 final monitor report does not match canonical checkpoint")


def verify(*args) -> None:
    if len(args) != 241:
        raise AssertionError("unexpected RUST-120 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 239:
        raise AssertionError("unexpected RUST-120 path count")

    base_paths = path_args[:236]
    second_rotation_path, second_auth_path, final_bundle_path = path_args[236:239]
    ...
    checkpoint_raw, checkpoint = floor_verify.load_canonical(
        base_paths[231], "RUST-117 final monitor rotation checkpoint"
    )
    ...
    if target["activation_source_commit"] != expected_source_sha:
        raise AssertionError("RUST-120 checkpoint source mismatch")

    first_rotation_raw, _ = floor_verify.load_canonical(base_paths[233], "RUST-120 first monitor rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base_paths[234], "RUST-120 first monitor rotation authorization")
    first_successor_raw, _ = floor_verify.load_canonical(base_paths[235], "RUST-120 first successor monitor bundle")
    second_rotation_raw, second_rotation = floor_verify.load_canonical(second_rotation_path, "RUST-120 second monitor rotation")
    _, second_auth = floor_verify.load_canonical(second_auth_path, "RUST-120 second rotation authorization")
    _, final_bundle = floor_verify.load_canonical(final_bundle_path, "RUST-120 final monitor bundle")
    ...
    print(
        "RUST-120 multi-step RUST-117 journal checkpoint monitor rotation: GREEN "
...
def main() -> None:
    if len(sys.argv) != 243 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_120_multistep_rust118_checkpoint_monitor_rotation_verify.py verify "
            "... RUST119_ROTATION AUTH SUCCESSOR RUST120_ROTATION AUTH FINAL_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
