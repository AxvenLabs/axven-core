#!/usr/bin/env python3
"""RUST-109 detached availability and fail-closed journal selftest."""
...
def main() -> None:
    if len(sys.argv) != 214:
        raise SystemExit(
            "usage: rust_109_rust106_checkpoint_monitor_rotation_journal_selftest.py "
    ...
    if len(base) != 210:
        raise AssertionError("unexpected RUST-109 selftest base path count")
    ...
    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        base[198], "RUST-105 final monitor rotation checkpoint"
    )
    target = monitor_verify.checkpoint_target(monitored_checkpoint_raw, monitored_checkpoint["statement"])
    old_bundle_raw, _ = floor_verify.load_canonical(base[199], "RUST-109 old monitor bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base[200], "RUST-109 first rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base[201], "RUST-109 first auth")
    first_successor_raw, _ = floor_verify.load_canonical(base[202], "RUST-109 first successor")
    second_rotation_raw, _ = floor_verify.load_canonical(base[203], "RUST-109 second rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base[204], "RUST-109 second auth")
    final_bundle_raw, _ = floor_verify.load_canonical(base[205], "RUST-109 final bundle")
    ...
    prefix_journal_raw, prefix_journal = floor_verify.load_canonical(base[206], "RUST-109 prefix journal")
    prefix_checkpoint_raw, prefix_checkpoint = floor_verify.load_canonical(base[207], "RUST-109 prefix checkpoint")
    final_journal_raw, final_journal = floor_verify.load_canonical(base[208], "RUST-109 final journal")
    final_checkpoint_raw, final_checkpoint = floor_verify.load_canonical(base[209], "RUST-109 final checkpoint")
    fork_raw, fork_checkpoint = floor_verify.load_canonical(fork_path, "RUST-109 observed fork checkpoint")
    ...
    for subset in itertools.combinations(prefix_checkpoint["monitors"], journal_verify.THRESHOLD):
        ...
        prefix_ok += 1
    if prefix_ok != 3:
        raise AssertionError("unexpected RUST-109 prefix subset count")
    print("[GREEN] RUST-109 prefix checkpoint availability: 3/3 valid two-monitor subsets accepted")
    ...
    for subset in itertools.combinations(final_checkpoint["monitors"], journal_verify.THRESHOLD):
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-109 final subset count")
    print("[GREEN] RUST-109 final checkpoint availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("prefix-genesis-set-rewrite", lambda: run_with(206, write("a.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_sha256"] = "0" * 64
        fail("prefix-first-rotation-digest", lambda: run_with(206, write("b.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_auth_sha256"] = "0" * 64
        fail("prefix-first-auth-digest", lambda: run_with(206, write("c.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitored_checkpoint_sha256"] = "0" * 64
        fail("prefix-monitored-checkpoint", lambda: run_with(206, write("d.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitored_checkpoint_statement_sha256"] = "0" * 64
        fail("prefix-monitored-statement", lambda: run_with(206, write("e.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["observed_target_sha256"] = "0" * 64
        fail("prefix-observed-target", lambda: run_with(206, write("f.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["activation_source_commit"] = "0" * 40
        fail("prefix-source", lambda: run_with(206, write("g.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["production"] = True
        fail("prefix-production", lambda: run_with(206, write("h.json", value))); cases += 1

        value = copy.deepcopy(final_journal); value["entries"] = value["entries"][:2]
        fail("final-entry-truncation", lambda: run_with(208, write("i.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][0]["monitor_bundle_sha256"] = "0" * 64
        fail("final-prefix-rewrite", lambda: run_with(208, write("j.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["sequence"] = 1
        fail("final-sequence-rollback", lambda: run_with(208, write("k.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["predecessor_entry_sha256"] = "0" * 64
        fail("final-predecessor-entry", lambda: run_with(208, write("l.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_sha256"] = "0" * 64
        fail("final-rotation-digest", lambda: run_with(208, write("m.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_auth_sha256"] = "0" * 64
        fail("final-rotation-auth-digest", lambda: run_with(208, write("n.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["monitor_bundle_sha256"] = "0" * 64
        fail("final-monitor-bundle", lambda: run_with(208, write("o.json", value))); cases += 1
        ...
        fail("final-revocation-omission", lambda: run_with(208, write("p.json", value))); cases += 1

        value = copy.deepcopy(prefix_checkpoint); value["threshold"] = 1
        fail("prefix-threshold", lambda: run_with(207, write("q.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["monitors"] = value["monitors"][:1]
        fail("prefix-below-threshold", lambda: run_with(207, write("r.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["monitors"] = [value["monitors"][0], copy.deepcopy(value["monitors"][0])]
        fail("prefix-duplicate", lambda: run_with(207, write("s.json", value))); cases += 1
        ...
        fail("prefix-signature", lambda: run_with(207, write("t.json", value))); cases += 1

        value = copy.deepcopy(final_checkpoint); value["statement"]["previous_checkpoint_sha256"] = "0" * 64
        fail("final-previous-checkpoint", lambda: run_with(209, write("u.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["head_entry_sha256"] = "0" * 64
        fail("final-head-entry", lambda: run_with(209, write("v.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitored_checkpoint_sha256"] = "0" * 64
        fail("final-monitored-checkpoint", lambda: run_with(209, write("w.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitored_checkpoint_statement_sha256"] = "0" * 64
        fail("final-monitored-statement", lambda: run_with(209, write("x.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observed_target_sha256"] = "0" * 64
        fail("final-observed-target", lambda: run_with(209, write("y.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["activation_source_commit"] = "0" * 40
        fail("final-source", lambda: run_with(209, write("z.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["production"] = True
        fail("final-production", lambda: run_with(209, write("aa.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitor_set_sequence"] = 1
        fail("final-monitor-set-sequence", lambda: run_with(209, write("ab.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitor_set_sha256"] = "0" * 64
        fail("final-monitor-set-digest", lambda: run_with(209, write("ac.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["threshold"] = 1
        fail("final-threshold", lambda: run_with(209, write("ad.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["monitors"] = value["monitors"][:1]
        fail("final-below-threshold", lambda: run_with(209, write("ae.json", value))); cases += 1
        ...
        fail("final-signature", lambda: run_with(209, write("af.json", value))); cases += 1

        noncanonical_journal = root / "noncanonical-journal.json"
        noncanonical_journal.write_text(json.dumps(final_journal, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-journal", lambda: run_with(208, noncanonical_journal)); cases += 1
        ...
        fail("noncanonical-final-checkpoint", lambda: run_with(209, noncanonical_checkpoint)); cases += 1
    ...
    if cases != 35:
        raise AssertionError(f"unexpected RUST-109 selftest case count: {cases}")
    print("RUST-109 monitor rotation journal fail-closed contract: 35/35 expected cases passed")
