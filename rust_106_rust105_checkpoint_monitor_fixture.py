#!/usr/bin/env python3
"""RUST-106 TEST-ONLY RUST-105 checkpoint monitor producer."""
...
def private_for(monitor_id: str) -> Ed25519PrivateKey:
    ...
    if public != monitor_verify.PINNED_MONITORS[monitor_id]:
        raise AssertionError("RUST-106 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_106_rust105_checkpoint_monitor_fixture.py SOURCE_SHA")
    source_sha = sys.argv[1]

    final_journal_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust105-final-monitor-rotation-journal.json",
        "RUST-105 final monitor rotation journal",
    )
    ...
    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust105-observed-fork-monitor-rotation-checkpoint.json",
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-106 canonical target source mismatch")
    ...
    (OUT / "axven-rust106-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle(reports))
    )
    (OUT / "axven-rust106-observed-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle([reports[0], reports[1], fork_report]))
    )
    print("RUST-106 TEST-only RUST-105 journal checkpoint monitor fixture: GREEN")
