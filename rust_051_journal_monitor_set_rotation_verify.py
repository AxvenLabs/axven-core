#!/usr/bin/env python3
"""RUST-051: TEST-ONLY journal-monitor set rotation verifier."""
...
MONITOR_SET_SCHEMA = "axven-native-journal-observer-journal-monitor-set-v1"
ROTATION_SCHEMA = "axven-native-journal-observer-journal-monitor-set-rotation-v1"
ROTATION_AUTH_SCHEMA = "axven-native-journal-observer-journal-monitor-set-rotation-quorum-v1"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-journal-observer-journal-monitor-set-rotation.v1+json"
SUCCESSOR_BUNDLE_SCHEMA = "axven-native-journal-observer-journal-checkpoint-monitor-bundle-v2"
SUCCESSOR_REPORT_SCHEMA = "axven-native-journal-observer-journal-checkpoint-monitor-report-v2"
SUCCESSOR_STATEMENT_SCHEMA = "axven-native-journal-observer-journal-checkpoint-monitor-statement-v2"
ROTATION_DOMAIN = b"AXVEN_NATIVE_JOURNAL_OBSERVER_JOURNAL_MONITOR_SET_ROTATION_V1\x00"
SUCCESSOR_DOMAIN = b"AXVEN_NATIVE_JOURNAL_OBSERVER_JOURNAL_CHECKPOINT_MONITOR_V2\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
OLD_SET_SEQUENCE = 0
NEW_SET_SEQUENCE = 1
...
ROTATION_KEYS = frozenset({
    ...
    "predecessor_monitor_bundle_sha256", "journal_observer_checkpoint_sha256",
...
def validate_rotation(rotation: dict, old_bundle_raw: bytes, target: dict, source_sha: str) -> None:
    if not isinstance(rotation, dict) or frozenset(rotation) != ROTATION_KEYS or rotation.get("schema") != ROTATION_SCHEMA:
        raise AssertionError("invalid journal-monitor rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != NEW_SET_SEQUENCE:
        raise AssertionError("invalid journal-monitor rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(old_monitor_set())):
        raise AssertionError("journal-monitor predecessor set mismatch")
    if rotation.get("to_set") != new_monitor_set():
        raise AssertionError("unexpected successor journal-monitor set")
    if rotation.get("revoked_monitor_ids") != [REVOKED_MONITOR_ID]:
        raise AssertionError("journal-monitor revocation continuity mismatch")
    ...
    if rotation.get("journal_observer_checkpoint_sha256") != target["journal_observer_checkpoint_sha256"]:
        raise AssertionError("rotation journal-observer checkpoint binding mismatch")
    if rotation.get("monitor_journal_checkpoint_sha256") != target["monitor_journal_checkpoint_sha256"]:
        raise AssertionError("rotation monitor-journal checkpoint binding mismatch")
    if rotation.get("monitor_journal_checkpoint_statement_sha256") != target["monitor_journal_checkpoint_statement_sha256"]:
        raise AssertionError("rotation monitor-journal checkpoint-statement binding mismatch")
    ...
    if rotation.get("production") is not False:
        raise AssertionError("production journal-monitor rotation forbidden in RUST-051")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if not isinstance(auth, dict) or frozenset(auth) != AUTH_KEYS or auth.get("schema") != ROTATION_AUTH_SCHEMA:
        raise AssertionError("invalid journal-monitor rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid journal-monitor rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid journal-monitor rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("journal-monitor rotation authorization payload mismatch")
    monitors = auth.get("monitors")
    if not isinstance(monitors, list) or not (THRESHOLD <= len(monitors) <= len(OLD_PINNED_MONITORS)):
        raise AssertionError("invalid journal-monitor rotation authorization size")
    if not all(isinstance(item, dict) and frozenset(item) == AUTH_MONITOR_KEYS for item in monitors):
        raise AssertionError("invalid journal-monitor rotation authorization entry")
    ...
    if any(monitor_id not in OLD_PINNED_MONITORS for monitor_id in ids):
        raise AssertionError("unknown journal-monitor rotation authorizer")
...
def validate_successor_bundle(bundle: dict, target: dict) -> None:
    ...
    if not isinstance(bundle, dict) or frozenset(bundle) != SUCCESSOR_BUNDLE_KEYS or bundle.get("schema") != SUCCESSOR_BUNDLE_SCHEMA:
        raise AssertionError("invalid successor journal-monitor bundle fields")
    if bundle.get("monitor_set_sequence") != NEW_SET_SEQUENCE or bundle.get("monitor_set_sha256") != set_sha:
        raise AssertionError("successor journal-monitor bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid successor journal-monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production successor journal monitoring forbidden in RUST-051")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(NEW_PINNED_MONITORS)):
        raise AssertionError("invalid successor journal-monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("successor journal-monitor ids must be unique and sorted")
    matching = 0
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("successor journal-monitor source mismatch")
        same_parent = (
        ...
        if not exact:
            raise AssertionError("successor journal-monitor report does not match canonical checkpoint")
        matching += 1
    if matching < THRESHOLD:
        raise AssertionError("successor journal-monitor quorum below threshold")


def verify(*args) -> None:
    if len(args) != 51:
        raise AssertionError("unexpected RUST-051 verifier argument count")
    *base_paths, old_bundle_path, rotation_path, auth_path, successor_path, expected_source_sha, required_floor_text = args
    if len(base_paths) != 45:
        raise AssertionError("unexpected RUST-051 base path count")
    ...
    if target["activation_source_commit"] != expected_source_sha:
        raise AssertionError("RUST-051 checkpoint source mismatch")
    old_bundle_raw, _ = floor_verify.load_canonical(old_bundle_path, "predecessor journal-monitor bundle")
    rotation_raw, rotation = floor_verify.load_canonical(rotation_path, "journal-monitor rotation")
    _, auth = floor_verify.load_canonical(auth_path, "journal-monitor rotation authorization")
    _, successor = floor_verify.load_canonical(successor_path, "successor journal-monitor bundle")
    ...
    print(
        "RUST-051 journal-monitor set rotation: GREEN "
        f"source={expected_source_sha} revoked={REVOKED_MONITOR_ID} successor={ids}"
    )


def main() -> None:
    if len(sys.argv) != 53 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_051_journal_monitor_set_rotation_verify.py verify "
