#!/usr/bin/env python3
"""RUST-132 detached second monitor-set rotation availability/fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 276:
        raise SystemExit(
            "usage: rust_132_multistep_rust130_checkpoint_monitor_rotation_selftest.py "
    ...
    if len(base) != 272:
        raise AssertionError("unexpected RUST-132 selftest base path count")
    ...
    for subset in itertools.combinations(second_auth["monitors"], rotation2_verify.THRESHOLD):
        ...
        auth_ok += 1
    if auth_ok != 3:
        raise AssertionError("unexpected RUST-132 authorization subset count")
    print("[GREEN] RUST-132 predecessor authorization availability: 3/3 valid two-monitor subsets accepted")
    ...
    for subset in itertools.combinations(final_bundle["reports"], rotation2_verify.THRESHOLD):
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-132 final monitoring subset count")
    print("[GREEN] RUST-132 final monitoring availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("rotation-sequence-rollback", lambda: run_with(269, write("a.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["from_set_sha256"] = "0" * 64
        fail("rotation-predecessor-set", lambda: run_with(269, write("b.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["to_set"]["threshold"] = 1
        fail("rotation-final-set", lambda: run_with(269, write("c.json", value))); cases += 1
        ...
        fail("rotation-cumulative-revocation", lambda: run_with(269, write("d.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_sha256"] = "0" * 64
        fail("rotation-predecessor-rotation", lambda: run_with(269, write("e.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_auth_sha256"] = "0" * 64
        fail("rotation-predecessor-auth", lambda: run_with(269, write("f.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_successor_bundle_sha256"] = "0" * 64
        fail("rotation-predecessor-successor", lambda: run_with(269, write("g.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"rt{idx}.json": run_with(269, write(n, v)),
            )
            cases += 1

        value = copy.deepcopy(second_rotation); value["production"] = True
        fail("rotation-production", lambda: run_with(269, write("h.json", value))); cases += 1

        value = copy.deepcopy(second_auth); value["threshold"] = 1
        fail("auth-threshold", lambda: run_with(270, write("i.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = value["monitors"][:1]
        fail("auth-below-threshold", lambda: run_with(270, write("j.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = [value["monitors"][0], copy.deepcopy(value["monitors"][0])]
        fail("auth-duplicate", lambda: run_with(270, write("k.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = list(reversed(value["monitors"]))
        fail("auth-unsorted", lambda: run_with(270, write("l.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["payload_sha256"] = "0" * 64
        fail("auth-payload", lambda: run_with(270, write("m.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"ft{idx}.json": run_with(271, write(n, v)),
            )
            cases += 1
        ...
        fail("final-signature", lambda: run_with(271, write("y.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["production"] = True
        fail("final-production", lambda: run_with(271, write("z.json", value))); cases += 1

        noncanonical = root / "noncanonical.json"
        noncanonical.write_text(json.dumps(final_bundle, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-bundle", lambda: run_with(271, noncanonical)); cases += 1
        fail("first-successor-replay", lambda: run_with(271, base[268])); cases += 1
        fail("observed-valid-final-same-parent-fork", lambda: run_with(271, fork_path)); cases += 1

    if cases != 53:
        raise AssertionError(f"unexpected RUST-132 selftest case count: {cases}")
    print("RUST-132 multi-step monitor rotation fail-closed contract: 53/53 expected cases passed")
