#!/usr/bin/env python3
"""RUST-118: TEST-ONLY RUST-117 monitor-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-rust118-monitor-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-rust118-monitor-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-rust118-monitor-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST118_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
MONITOR_1_PUBLIC_KEY = bytes.fromhex("a87b8a99bd88a69686c994a80b629d8154871aa295540834c01d79f4f916502f")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("70df9e2279adbec6d12bf2921184c9222eb24ed852005bf640139f52e59cd9ae")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("62a611b472d89b0e5fc93c069b9f700b4c552d55bc0e87b56008ef17b6b2bebe")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust117_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust117_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-118 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-118 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-118 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-118 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-118 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-118 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-118 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-117 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-118 monitor report does not match canonical RUST-117 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-118 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-118 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-118 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 235:
        raise AssertionError("unexpected RUST-118 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 233:
        raise AssertionError("unexpected RUST-118 path count")
    base_paths = path_args[:232]
    monitor_bundle_path = path_args[232]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[230], "RUST-117 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 237 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_118_rust117_checkpoint_monitor_verify.py verify "
            "... RUST117_FINAL_CHECKPOINT MONITOR_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
