#!/usr/bin/env python3
"""RUST-079 TEST-ONLY RUST-077 checkpoint observer-set rotation producer."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-079 TEST-only journal checkpoint observer public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_079_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_set_rotation_fixture.py SOURCE_SHA"
        )
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        ...
        "RUST-078 predecessor observer bundle",
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust078-observed-fork-observer-bundle.json",
        "RUST-078 predecessor observed fork bundle",
    )
    ...
    (OUT / "axven-rust079-observer-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust079-observer-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust079-successor-observer-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust079-successor-fork-observer-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-079 TEST-only RUST-077 checkpoint observer-set rotation fixture: GREEN")
