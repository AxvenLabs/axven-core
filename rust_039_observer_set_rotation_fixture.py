#!/usr/bin/env python3
"""RUST-039 TEST-ONLY observer-set rotation producer. Private seeds remain producer-side."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-039 TEST-only observer public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(c not in "0123456789abcdef" for c in sys.argv[1]):
        raise SystemExit("usage: rust_039_observer_set_rotation_fixture.py SOURCE_SHA")
    ...
    reports = [successor_report(observer_id, target, set_sha) for observer_id in sorted(rotation_verify.NEW_PINNED_OBSERVERS)]
    successor = {
    ...
    (OUT / "axven-rust039-observer-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust039-observer-set-rotation-auth.json").write_bytes(material_verify.canonical(auth))
    (OUT / "axven-rust039-successor-observer-bundle.json").write_bytes(material_verify.canonical(successor))
    (OUT / "axven-rust039-successor-fork-bundle.json").write_bytes(material_verify.canonical(fork_successor))
    print("RUST-039 TEST-only observer-set rotation fixture: GREEN")
