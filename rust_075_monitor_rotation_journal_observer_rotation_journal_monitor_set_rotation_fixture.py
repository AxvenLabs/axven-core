#!/usr/bin/env python3
"""RUST-075 TEST-ONLY monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-075 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit(
            "usage: rust_075_monitor_rotation_journal_observer_rotation_journal_monitor_set_rotation_fixture.py SOURCE_SHA"
        )
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust074-monitor-bundle.json", "RUST-075 predecessor monitor bundle"
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust074-observed-fork-monitor-bundle.json",
        "RUST-075 predecessor observed fork monitor bundle",
    )
    ...
    (OUT / "axven-rust075-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust075-monitor-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust075-successor-monitor-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust075-successor-fork-bundle.json").write_bytes(
        material_verify.canonical(
            successor_bundle([reports[0], reports[1], fork_report], set_sha)
        )
    )
    print("RUST-075 TEST-only monitor-set rotation fixture: GREEN")
