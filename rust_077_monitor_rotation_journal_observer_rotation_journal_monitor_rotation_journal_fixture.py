#!/usr/bin/env python3
"""RUST-077 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
OUT = Path("/tmp")
SEEDS = {
    monitor_verify.MONITOR_2_ID: "b9" * 32,
    monitor_verify.MONITOR_3_ID: "c9" * 32,
    rotation1_verify.M4_ID: "d9" * 32,
    rotation2_verify.M5_ID: "e9" * 32,
}
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-077 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit(
            "usage: rust_077_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_fixture.py SOURCE_SHA"
        )
    source_sha = sys.argv[1]

    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust073-final-observer-rotation-checkpoint.json",
        "RUST-073 final observer rotation checkpoint",
    )
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-077 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust074-monitor-bundle.json", "RUST-077 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust075-monitor-set-rotation.json", "RUST-077 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust075-monitor-set-rotation-auth.json", "RUST-077 first rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust075-successor-monitor-bundle.json", "RUST-077 first successor monitor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust076-second-monitor-set-rotation.json", "RUST-077 second monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust076-second-monitor-set-rotation-auth.json", "RUST-077 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust076-final-monitor-bundle.json", "RUST-077 final monitor bundle"
    )
    ...
    (OUT / "axven-rust077-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust077-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust077-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust077-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust077-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-077 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
