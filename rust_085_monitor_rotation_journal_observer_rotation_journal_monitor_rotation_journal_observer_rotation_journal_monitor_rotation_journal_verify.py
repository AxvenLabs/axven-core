#!/usr/bin/env python3
"""RUST-085: TEST-ONLY append-only RUST-081 checkpoint-monitor rotation journal verifier."""
...
JOURNAL_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-set-rotation-journal-v1"
ENTRY_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-set-rotation-journal-entry-v1"
CHECKPOINT_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-set-rotation-journal-checkpoint-v1"
STATEMENT_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-set-rotation-journal-checkpoint-statement-v1"
CHECKPOINT_DOMAIN = b"AXVEN_NATIVE_MONITOR_ROTATION_JOURNAL_OBSERVER_ROTATION_JOURNAL_MONITOR_ROTATION_JOURNAL_OBSERVER_ROTATION_JOURNAL_MONITOR_SET_ROTATION_JOURNAL_CHECKPOINT_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
def target_digest(target: dict) -> str:
    if frozenset(target) != monitor_verify.TARGET_KEYS:
        raise AssertionError("unexpected RUST-085 inherited target fields")
...
def validate_journal(journal: dict, expected: dict, label: str) -> None:
    if (
        ...
        raise AssertionError(f"invalid {label} RUST-085 monitor rotation journal fields")
    if journal.get("production") is not False:
        raise AssertionError(f"production {label} monitor rotation journal forbidden in RUST-085")
    if journal != expected:
        raise AssertionError(f"{label} RUST-085 monitor rotation journal continuity mismatch")
    entries = journal.get("entries")
    if not isinstance(entries, list) or not entries:
        raise AssertionError(f"empty {label} RUST-085 monitor rotation journal")
    for index, entry in enumerate(entries):
        if (
            ...
            raise AssertionError(f"invalid {label} RUST-085 monitor rotation journal entry")
        if type(entry.get("sequence")) is not int or entry["sequence"] != index:
            raise AssertionError(f"non-monotonic {label} RUST-085 journal sequence")
        if index == 0:
            if entry.get("predecessor_entry_sha256") is not None:
                raise AssertionError(f"unexpected {label} RUST-085 journal genesis predecessor")
        elif entry.get("predecessor_entry_sha256") != sha256(canonical(entries[index - 1])):
            raise AssertionError(f"broken {label} RUST-085 journal hash chain")
...
def validate_checkpoint_envelope(
    ...
    if (
        ...
        raise AssertionError(f"invalid {label} RUST-085 checkpoint fields")
    if checkpoint.get("algorithm") != ALGORITHM or checkpoint.get("production") is not False:
        raise AssertionError(f"invalid {label} RUST-085 checkpoint boundary")
    if type(checkpoint.get("threshold")) is not int or checkpoint["threshold"] != THRESHOLD:
        raise AssertionError(f"invalid {label} RUST-085 checkpoint threshold")
    statement = checkpoint.get("statement")
    if not isinstance(statement, dict) or frozenset(statement) != STATEMENT_KEYS:
        raise AssertionError(f"invalid {label} RUST-085 checkpoint statement fields")
    if statement.get("schema") != STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError(f"invalid {label} RUST-085 checkpoint statement boundary")
    monitors = checkpoint.get("monitors")
    if not isinstance(monitors, list) or not (THRESHOLD <= len(monitors) <= len(pins)):
        raise AssertionError(f"invalid {label} RUST-085 checkpoint signature count")
    if not all(
        isinstance(item, dict) and frozenset(item) == CHECKPOINT_MONITOR_KEYS
        for item in monitors
    ):
        raise AssertionError(f"invalid {label} RUST-085 checkpoint signature row")
    ids = [item["monitor_id"] for item in monitors]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError(f"{label} RUST-085 checkpoint monitor ids must be unique and sorted")
    if any(monitor_id not in pins for monitor_id in ids):
        raise AssertionError(f"unknown {label} RUST-085 checkpoint signer")
...
def validate_checkpoint(
    ...
    if statement != expected_statement:
        raise AssertionError(f"{label} RUST-085 checkpoint statement mismatch")
...
def verify(*args) -> None:
    if len(args) != 146:
        raise AssertionError("unexpected RUST-085 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 144:
        raise AssertionError("unexpected RUST-085 path count")

    base_paths = path_args[:140]
    prefix_journal_path, prefix_checkpoint_path, final_journal_path, final_checkpoint_path = path_args[140:144]
    ...
    if target["activation_source_commit"] != expected_source_sha:
        raise AssertionError("RUST-085 checkpoint source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(base_paths[133], "RUST-085 old monitor bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base_paths[134], "RUST-085 first monitor rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base_paths[135], "RUST-085 first rotation authorization")
    first_successor_raw, _ = floor_verify.load_canonical(base_paths[136], "RUST-085 first successor bundle")
    second_rotation_raw, _ = floor_verify.load_canonical(base_paths[137], "RUST-085 second monitor rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base_paths[138], "RUST-085 second rotation authorization")
    final_bundle_raw, _ = floor_verify.load_canonical(base_paths[139], "RUST-085 final monitor bundle")
    ...
    prefix_journal_raw, prefix_journal = floor_verify.load_canonical(
        prefix_journal_path, "RUST-085 prefix monitor rotation journal"
    )
    prefix_checkpoint_raw, prefix_checkpoint = floor_verify.load_canonical(
        prefix_checkpoint_path, "RUST-085 prefix monitor rotation checkpoint"
    )
    final_journal_raw, final_journal = floor_verify.load_canonical(
        final_journal_path, "RUST-085 final monitor rotation journal"
    )
    _, final_checkpoint = floor_verify.load_canonical(
        final_checkpoint_path, "RUST-085 final monitor rotation checkpoint"
    )
    ...
    if final_journal["entries"][:2] != prefix_journal["entries"]:
        raise AssertionError("final RUST-085 monitor rotation journal rewrites checkpointed prefix")
    ...
    print(
        "RUST-085 append-only RUST-081 checkpoint monitor rotation journal: GREEN "
        f"source={expected_source_sha} entries=3 checkpoint={sha256(canonical(final_checkpoint))}"
    )


def main() -> None:
    if len(sys.argv) != 148 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_085_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_verify.py verify "
