#!/usr/bin/env python3
"""RUST-046 detached availability and fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 39:
        raise SystemExit(
            "usage: rust_046_monitor_journal_gossip_selftest.py "
            "... FINAL_MON_CHECKPOINT BUNDLE FORK_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
    ...
    *base_paths, bundle_path, fork_bundle_path = paths
    ...
    if len(base_paths) != 34:
        raise AssertionError("unexpected RUST-046 selftest base path count")

    final_checkpoint_raw, checkpoint = floor_verify.load_canonical(base_paths[-1], "final monitor checkpoint")
    _, bundle = floor_verify.load_canonical(bundle_path, "journal observer bundle")
    target = gossip_verify.canonical_target(final_checkpoint_raw, checkpoint, source_sha)
    cases = 0
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print("[GREEN] RUST-046 journal-observer availability: 3/3 valid two-observer subsets accepted")
        ...
        value = copy.deepcopy(bundle); value["schema"] = "axven-native-monitor-rotation-journal-checkpoint-observation-bundle-v0"; mutations.append(("bundle-schema", value))
        value = copy.deepcopy(bundle); value["reports"][0]["schema"] = "axven-native-monitor-rotation-journal-checkpoint-observation-v0"; mutations.append(("report-schema", value))
        ...
        value = copy.deepcopy(bundle); value["reports"][0]["statement"]["observer_checkpoint_sha256"] = "0" * 64; mutations.append(("observer-checkpoint-digest", value))
        ...
        fail("observed-cross-observer-monitor-journal-fork", lambda: run(fork_bundle_path)); cases += 1

    if cases != 22:
        raise AssertionError(f"unexpected RUST-046 selftest case count: {cases}")
    print("RUST-046 monitor-journal observation fail-closed contract: 22/22 expected cases passed")
