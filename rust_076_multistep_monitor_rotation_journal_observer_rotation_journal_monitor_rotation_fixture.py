#!/usr/bin/env python3
"""RUST-076 TEST-ONLY second monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-076 TEST-only monitor public-key pin mismatch")
    return private
...
def final_report(monitor_id: str, target: dict, set_sha: str) -> dict:
    ...
    private = private_for(monitor_id, rotation2_verify.FINAL_PINNED_MONITORS)
    return {
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_076_multistep_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_fixture.py SOURCE_SHA"
        )
    ...
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust075-monitor-set-rotation.json", "RUST-076 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust075-monitor-set-rotation-auth.json",
        "RUST-076 first monitor rotation authorization",
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust075-successor-monitor-bundle.json",
        "RUST-076 first successor monitor bundle",
    )
    _, predecessor_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust075-successor-fork-bundle.json",
        "RUST-076 predecessor fork monitor bundle",
    )
    ...
    (OUT / "axven-rust076-second-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust076-second-monitor-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust076-final-monitor-bundle.json").write_bytes(
        material_verify.canonical(final_bundle(reports, set_sha))
    )
    (OUT / "axven-rust076-final-fork-bundle.json").write_bytes(
        material_verify.canonical(final_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-076 TEST-only second monitor-set rotation fixture: GREEN")
