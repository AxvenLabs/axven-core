#!/usr/bin/env python3
"""RUST-090 TEST-ONLY RUST-089 checkpoint monitor producer."""
...
OUT = Path("/tmp")
SEEDS = {
    monitor_verify.MONITOR_1_ID: "fa" * 32,
...
def private_for(monitor_id: str) -> Ed25519PrivateKey:
    ...
    if public != monitor_verify.PINNED_MONITORS[monitor_id]:
        raise AssertionError("RUST-090 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_090_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_monitor_rotation_journal_monitor_fixture.py SOURCE_SHA"
        )
    source_sha = sys.argv[1]

    final_journal_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust089-final-monitor-rotation-journal.json",
        "RUST-089 final monitor rotation journal",
    )
    ...
    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust089-observed-fork-monitor-rotation-checkpoint.json",
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-090 canonical target source mismatch")
    ...
    (OUT / "axven-rust090-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle(reports))
    )
    (OUT / "axven-rust090-observed-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle([reports[0], reports[1], fork_report]))
    )
    print("RUST-090 TEST-only monitor-rotation-journal monitor fixture: GREEN")
