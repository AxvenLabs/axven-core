#!/usr/bin/env python3
"""RUST-071: TEST-ONLY monitor-rotation-journal observer-set rotation verifier."""
...
OBSERVER_SET_SCHEMA = "axven-native-monitor-rotation-journal-observer-set-v1"
ROTATION_SCHEMA = "axven-native-monitor-rotation-journal-observer-set-rotation-v1"
ROTATION_AUTH_SCHEMA = "axven-native-monitor-rotation-journal-observer-set-rotation-quorum-v1"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-monitor-rotation-journal-observer-set-rotation.v1+json"
...
ROTATION_DOMAIN = b"AXVEN_NATIVE_MONITOR_ROTATION_JOURNAL_OBSERVER_SET_ROTATION_V1\x00"
...
def validate_rotation(rotation: dict, old_bundle_raw: bytes, target: dict) -> None:
    if not isinstance(rotation, dict) or frozenset(rotation) != ROTATION_KEYS or rotation.get("schema") != ROTATION_SCHEMA:
        raise AssertionError("invalid RUST-071 monitor-rotation-journal observer rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != NEW_SET_SEQUENCE:
        raise AssertionError("invalid RUST-071 observer rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(old_observer_set())):
        raise AssertionError("RUST-071 observer predecessor set mismatch")
    if rotation.get("to_set") != new_observer_set():
        raise AssertionError("unexpected RUST-071 successor observer set")
    if rotation.get("revoked_observer_ids") != [REVOKED_OBSERVER_ID]:
        raise AssertionError("RUST-071 observer revocation continuity mismatch")
    if rotation.get("predecessor_observation_bundle_sha256") != sha256(old_bundle_raw):
        raise AssertionError("RUST-071 predecessor observation bundle mismatch")
    for key in observer_verify.TARGET_KEYS:
        if rotation.get(key) != target[key]:
            raise AssertionError(f"RUST-071 rotation inherited target mismatch: {key}")
    if rotation.get("production") is not False:
        raise AssertionError("production observer rotation forbidden in RUST-071")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if not isinstance(auth, dict) or frozenset(auth) != AUTH_KEYS or auth.get("schema") != ROTATION_AUTH_SCHEMA:
        raise AssertionError("invalid RUST-071 observer rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid RUST-071 observer rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-071 observer rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("RUST-071 observer rotation authorization payload mismatch")
    observers = auth.get("observers")
    if not isinstance(observers, list) or not (THRESHOLD <= len(observers) <= len(OLD_PINNED_OBSERVERS)):
        raise AssertionError("invalid RUST-071 observer rotation authorization size")
    if not all(isinstance(item, dict) and frozenset(item) == AUTH_OBSERVER_KEYS for item in observers):
        raise AssertionError("invalid RUST-071 observer rotation authorization entry")
    ids = [item["observer_id"] for item in observers]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-071 rotation authorizer ids must be unique and sorted")
    if any(observer_id not in OLD_PINNED_OBSERVERS for observer_id in ids):
        raise AssertionError("unknown RUST-071 observer rotation authorizer")
...
def validate_successor_bundle(bundle: dict, target: dict) -> None:
    ...
    if not isinstance(bundle, dict) or frozenset(bundle) != SUCCESSOR_BUNDLE_KEYS or bundle.get("schema") != SUCCESSOR_BUNDLE_SCHEMA:
        raise AssertionError("invalid RUST-071 successor observation bundle fields")
    if bundle.get("observer_set_sequence") != NEW_SET_SEQUENCE or bundle.get("observer_set_sha256") != set_sha:
        raise AssertionError("RUST-071 successor observation bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-071 successor observation threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production successor observation forbidden in RUST-071")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(NEW_PINNED_OBSERVERS)):
        raise AssertionError("invalid RUST-071 successor observation report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-071 successor observer ids must be unique and sorted")
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-071 successor observer source mismatch")
        same_epoch_parent = (
        ...
        if same_epoch_parent and not exact:
            raise AssertionError("observed successor same-parent RUST-069 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-071 successor observer report does not match canonical checkpoint")


def verify(*args) -> None:
    if len(args) != 106:
        raise AssertionError("unexpected RUST-071 verifier argument count")
    *base_paths, old_bundle_path, rotation_path, auth_path, successor_path, expected_source_sha, required_floor_text = args
    if len(base_paths) != 100:
        raise AssertionError("unexpected RUST-071 base path count")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        old_bundle_path, "RUST-071 predecessor observer bundle"
    )
    rotation_raw, rotation = floor_verify.load_canonical(
        rotation_path, "RUST-071 observer-set rotation"
    )
    _, auth = floor_verify.load_canonical(auth_path, "RUST-071 rotation authorization")
    ...
    print(
        "RUST-071 monitor-rotation-journal observer-set rotation: GREEN "
        f"source={expected_source_sha} revoked={REVOKED_OBSERVER_ID} successor={ids}"
    )


def main() -> None:
    if len(sys.argv) != 108 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_071_monitor_rotation_journal_observer_set_rotation_verify.py verify "
