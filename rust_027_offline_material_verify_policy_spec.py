#!/usr/bin/env python3
"""RUST-027: static policy for verification-only build-material consumer separation."""
from __future__ import annotations

from pathlib import Path
import axven

ROOT = Path(__file__).resolve().parent
WORKFLOW = ROOT / ".github/workflows/native-build-material-consumer.yml"
VERIFIER = ROOT / "rust_027_offline_material_verify.py"
PRODUCTION = ("axven.py", "core.py", "p2p.py", "rpc.py", "wallet.py", "axven_core.py")
PUBLIC_KEY = "4dd000548d1ed66588e6c23531163bd12c9c5dbca5eb932d4c6a75cde6525064"


def text(path: Path | str) -> str:
    target = path if isinstance(path, Path) else ROOT / path
    return target.read_text(encoding="utf-8")


def main() -> None:
    checks = 0
    workflow = text(WORKFLOW)
    verifier = text(VERIFIER)
    doc = text("docs/history/rust/RUST_027.md")

    for marker in (
        "bash rust_025_upstream_authenticated_detached_build.sh",
        "python rust_026_build_material_attestation.py generate",
        "python rust_026_build_material_attestation.py seal",
        "Bridge RUST-023 manifest canonical encoding",
        'python "$consumer/verifier.py" verify',
        'python "$consumer/verifier.py" selftest',
        PUBLIC_KEY,
    ):
        assert marker in workflow + "\n" + verifier, marker
    checks += 1
    print("[GREEN] RUST-027 consumes real RUST-025/RUST-026 materials with an independent verifier")

    for forbidden in (
        "TEST_SEED",
        "Ed25519PrivateKey",
        "PrivateFormat",
        "private_bytes",
        "def seal",
        "import rust_0",
        "from rust_0",
        "import axven",
        "from axven",
        "subprocess",
        "git ",
    ):
        assert forbidden not in verifier, forbidden
    assert "Ed25519PublicKey" in verifier
    assert 'KEY_ID = "rust-026-test-only-ed25519-v1"' in verifier
    assert 'DOMAIN = b"AXVEN_NATIVE_BUILD_MATERIAL_ATTESTATION_V1\\x00"' in verifier
    checks += 1
    print("[GREEN] detached consumer has public-key verification only and no signing capability")

    detached_at = workflow.index("Detached RUST-027 verification-only consumer")
    reverify_at = workflow.index("Reverify pristine RUST-027 consumer result")
    detached = workflow[detached_at:reverify_at]
    for marker in (
        "env -i",
        'python "$consumer/verifier.py" verify',
        'python "$consumer/verifier.py" selftest',
        'test "$(find "$consumer" -maxdepth 1 -type f | wc -l)" -eq 3',
        "/tmp/axven-rust025-dependencies",
        "/tmp/axven-rust025-vendor",
        "/tmp/axven-rust025-toolchain.json",
    ):
        assert marker in detached, marker
    assert "10/10 expected cases passed" in verifier
    checks += 1
    print("[GREEN] verifier runs detached under env -i with a 10/10 fail-closed external-evidence contract")

    assert "permissions:\n  contents: read" in workflow
    assert "persist-credentials: false" in workflow
    lower = workflow.lower()
    for forbidden in (
        "id-token: write",
        "attestations: write",
        "packages: write",
        "contents: write",
        "actions/upload-artifact",
        "actions/attest",
        "maturin publish",
        "twine upload",
        "gh release",
        "docker push",
    ):
        assert forbidden not in lower, forbidden
    assert "verification-only" in doc.lower()
    assert "does not upload or publish" in doc.lower()
    checks += 1
    print("[GREEN] RUST-027 introduces no publication, signing, OIDC or deployment privilege")

    for name in PRODUCTION:
        assert "axven_native" not in text(name), name
    assert axven.CHAIN_ID == "axven-devnet-2"
    assert axven.CONFIG_FINGERPRINT == "ac56ced3ca38dd449dabc3fc0091a3cc4dce6e05c692dcf836f1e493e7efabae"
    assert axven._genesis().hash() == "a49413203b4a00f3c5b3a5901e8cd198b09f41f58295f22c927883f7fe4e1ab3"
    assert axven.CHAIN_CONFIG["smt_activation_height"] == 10_000
    assert axven.CHAIN_CONFIG["pq_hybrid_activation_height"] == 2_000
    assert axven.CHAIN_CONFIG["pq_pure_activation_height"] == 5_000
    assert "production consensus remains python-authoritative" in doc.lower()
    checks += 1
    print("[GREEN] production Python authority and canonical chain identity remain unchanged")

    assert checks == 5
    print("RUST-027 verification-only consumer policy contract: 5/5 GREEN")


if __name__ == "__main__":
    main()
