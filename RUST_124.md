# RUST-124 — TEST-ONLY second RUST-122 checkpoint monitor-set rotation

RUST-124 composes the exact reviewed RUST-123 first monitor-set rotation and performs the second TEST-only rotation for the RUST-122 checkpoint-monitor family.

## Contract

- Monitor set rotates from M2/M3/M4 to M3/M4/M5 with a 2-of-3 threshold.
- M1 remains revoked from RUST-123 and M2 is newly revoked; cumulative revocation continuity is explicit.
- The second rotation binds the exact RUST-123 rotation, authorization and successor-bundle SHA-256 values plus all 12 inherited RUST-121 checkpoint target fields.
...
- CI keeps generated evidence read-only with chmod 0444, uses a fixed 250-path manifest and an 97-file verifier-only detached consumer under `env -i` and `/usr/bin/python3 -S`.
