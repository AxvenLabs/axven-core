#!/usr/bin/env python3
"""RUST-067 TEST-ONLY monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-067 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit(
            "usage: rust_067_monitor_rotation_journal_observer_rotation_journal_monitor_set_rotation_fixture.py SOURCE_SHA"
        )
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust066-monitor-bundle.json", "RUST-067 predecessor monitor bundle"
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust066-observed-fork-monitor-bundle.json",
        "RUST-067 predecessor observed fork monitor bundle",
    )
    ...
    (
        OUT / "axven-rust067-monitor-set-rotation.json"
    ...
    ).write_bytes(material_verify.canonical(successor_bundle(reports, set_sha)))
    (
    ...
    print("RUST-067 TEST-only monitor-set rotation fixture: GREEN")
