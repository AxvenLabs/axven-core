#!/usr/bin/env python3
"""RUST-144 TEST-ONLY second monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-144 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_144_multistep_rust142_checkpoint_monitor_rotation_fixture.py SOURCE_SHA")
    ...
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust143-monitor-set-rotation.json",
        "RUST-144 first monitor-set rotation",
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust143-monitor-set-rotation-auth.json",
        "RUST-144 first monitor-set rotation authorization",
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust143-successor-monitor-bundle.json",
        "RUST-144 first successor monitor bundle",
    )
    _, first_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust143-successor-fork-bundle.json",
        "RUST-144 first successor fork bundle",
    )
    ...
    (OUT / "axven-rust144-second-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust144-second-monitor-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust144-final-monitor-bundle.json").write_bytes(
        material_verify.canonical(final_bundle(reports, set_sha))
    )
    (OUT / "axven-rust144-final-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(final_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-144 TEST-only second monitor-set rotation fixture: GREEN")
