#!/usr/bin/env python3
"""RUST-113 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-113 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_113_rust110_checkpoint_monitor_rotation_journal_fixture.py SOURCE_SHA")
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-113 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust110-monitor-bundle.json", "RUST-113 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust111-monitor-set-rotation.json", "RUST-113 first monitor rotation"
    )
    ...
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust112-second-monitor-set-rotation-auth.json", "RUST-113 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust112-final-monitor-bundle.json", "RUST-113 final monitor bundle"
    )
    ...
    (OUT / "axven-rust113-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust113-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust113-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust113-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust113-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-113 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
