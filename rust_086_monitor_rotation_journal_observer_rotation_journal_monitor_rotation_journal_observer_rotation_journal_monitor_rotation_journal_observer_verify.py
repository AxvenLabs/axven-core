#!/usr/bin/env python3
"""RUST-086: TEST-ONLY observer quorum for the RUST-085 final monitor-rotation journal checkpoint."""
...
BUNDLE_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-checkpoint-observation-bundle-v1"
REPORT_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-checkpoint-observation-v1"
STATEMENT_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-checkpoint-observation-statement-v1"
OBSERVATION_DOMAIN = b"AXVEN_NATIVE_MONITOR_ROTATION_JOURNAL_OBSERVER_ROTATION_JOURNAL_MONITOR_ROTATION_JOURNAL_OBSERVER_ROTATION_JOURNAL_MONITOR_ROTATION_JOURNAL_CHECKPOINT_OBSERVATION_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

OBSERVER_1_ID = "rust-086-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-1-v1"
OBSERVER_2_ID = "rust-086-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-2-v1"
OBSERVER_3_ID = "rust-086-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-3-v1"
OBSERVER_1_PUBLIC_KEY = bytes.fromhex("c2f38d34dafe402561da5a0a278e8a3255e0fc9c2e58c0209966a589fd07b631")
OBSERVER_2_PUBLIC_KEY = bytes.fromhex("259145ad2655219b76837676ba31b9779035ffaa88c457a595d22e4a374527c2")
OBSERVER_3_PUBLIC_KEY = bytes.fromhex("ba3b611e2882c1b6aa4b2ae3ec78ea0736e3ad99238353450171507a4b9f15b5")
PINNED_OBSERVERS = {
...
def canonical_target(checkpoint_raw: bytes, checkpoint: dict, source_sha: str) -> dict:
    ...
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("missing final RUST-085 monitor rotation journal checkpoint statement")
    if statement.get("schema") != journal_verify.STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError("invalid final RUST-085 monitor rotation journal checkpoint statement boundary")
    if statement.get("activation_source_commit") != source_sha:
        raise AssertionError("final RUST-085 monitor rotation journal checkpoint source mismatch")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if not isinstance(bundle, dict) or frozenset(bundle) != BUNDLE_KEYS or bundle.get("schema") != BUNDLE_SCHEMA:
        raise AssertionError("invalid RUST-086 journal checkpoint observer bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-086 journal checkpoint observer threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-086 journal checkpoint observer bundle forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_OBSERVERS)):
        raise AssertionError("invalid RUST-086 journal checkpoint observer report count")

    statements = [validate_report(report) for report in reports]
    ids = [statement["observer_id"] for statement in statements]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-086 journal checkpoint observer ids must be unique and sorted")

    matching = 0
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-086 journal checkpoint observer source mismatch")
        same_epoch_parent = (
        ...
        if same_epoch_parent and not exact:
            raise AssertionError(
                "observed cross-observer same-parent RUST-085 monitor rotation journal checkpoint fork"
            )
        if not exact:
            raise AssertionError("RUST-086 observer report does not match canonical RUST-085 checkpoint")
        matching += 1
    if matching < THRESHOLD:
        raise AssertionError("RUST-086 journal checkpoint observer quorum below threshold")
...
def verify(*args) -> None:
    ...
    *base_paths, observation_bundle_path, expected_source_sha, required_floor_text = args
    ...
    _, bundle = floor_verify.load_canonical(
        observation_bundle_path, "RUST-086 journal checkpoint observer bundle"
    )
    ...
    print(
        "RUST-086 monitor-rotation-journal checkpoint observation: GREEN "
...
def main() -> None:
    if len(sys.argv) != 149 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_086_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_verify.py verify "
