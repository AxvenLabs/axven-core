#!/usr/bin/env python3
"""RUST-086 TEST-ONLY RUST-085 checkpoint monitor producer."""
...
def private_for(monitor_id: str) -> Ed25519PrivateKey:
    ...
    if public != monitor_verify.PINNED_MONITORS[monitor_id]:
        raise AssertionError("RUST-086 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_086_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_monitor_fixture.py SOURCE_SHA"
        )
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-086 canonical target source mismatch")
    ...
    (OUT / "axven-rust086-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle(reports))
    )
    (OUT / "axven-rust086-observed-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle([reports[0], reports[1], fork_report]))
    )
    print("RUST-086 TEST-only monitor-rotation-journal monitor fixture: GREEN")
