#!/usr/bin/env python3
"""RUST-146: TEST-ONLY RUST-145 monitor-rotation-journal checkpoint monitor verifier."""
...
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST146_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

MONITOR_1_ID = "rust-142-test-only-monitor-rotation-journal-monitor-1-v1"
MONITOR_2_ID = "rust-142-test-only-monitor-rotation-journal-monitor-2-v1"
MONITOR_3_ID = "rust-142-test-only-monitor-rotation-journal-monitor-3-v1"
MONITOR_1_PUBLIC_KEY = bytes.fromhex("ef747fff9b4726c7a4e53504644181d4a700efd6f229444038f47e143fb8c59a")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("c86d01ad8b2fb694b22ff60e3c9d9e7ed90db5604772b74450d40913b9a9cbaf")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("b2a942ff4c98718bed76e255987f6d59b1a72d3b2cd2510003e6170ac63a9ffb")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust145_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust145_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-146 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-146 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-146 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-146 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-146 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-146 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-146 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-145 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-146 monitor report does not match canonical RUST-145 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-146 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-146 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-146 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 312:
        raise AssertionError("unexpected RUST-146 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 310:
        raise AssertionError("unexpected RUST-146 path count")
    base_paths = path_args[:309]
    monitor_bundle_path = path_args[309]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[307], "RUST-145 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 314 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_146_rust145_checkpoint_monitor_verify.py verify "
