#!/usr/bin/env python3
"""RUST-072: TEST-ONLY multi-step observer rotation for RUST-070 checkpoint observers."""
...
ROTATION_SCHEMA = "axven-native-monitor-rotation-journal-observer-set-rotation-v2"
ROTATION_AUTH_SCHEMA = "axven-native-monitor-rotation-journal-observer-set-rotation-quorum-v2"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-monitor-rotation-journal-observer-set-rotation.v2+json"
...
ROTATION_DOMAIN = b"AXVEN_NATIVE_MONITOR_ROTATION_JOURNAL_OBSERVER_SET_ROTATION_V2\x00"
...
def validate_rotation(
    ...
    if (
        ...
        raise AssertionError("invalid RUST-072 second observer rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != FINAL_SET_SEQUENCE:
        raise AssertionError("invalid RUST-072 second observer rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(rotation1_verify.new_observer_set())):
        raise AssertionError("RUST-072 predecessor observer set mismatch")
    if rotation.get("to_set") != final_observer_set():
        raise AssertionError("unexpected RUST-072 final observer set")
    if rotation.get("cumulative_revoked_observer_ids") != CUMULATIVE_REVOKED_OBSERVER_IDS:
        raise AssertionError("RUST-072 cumulative observer revocation mismatch")
    if rotation.get("predecessor_rotation_sha256") != sha256(first_rotation_raw):
        raise AssertionError("RUST-072 predecessor rotation digest mismatch")
    if rotation.get("predecessor_rotation_auth_sha256") != sha256(first_auth_raw):
        raise AssertionError("RUST-072 predecessor rotation authorization digest mismatch")
    if rotation.get("predecessor_successor_bundle_sha256") != sha256(first_successor_raw):
        raise AssertionError("RUST-072 predecessor successor bundle digest mismatch")
    for key in TARGET_KEYS:
        if rotation.get(key) != target[key]:
            raise AssertionError(f"RUST-072 inherited target binding mismatch: {key}")
    if rotation.get("production") is not False:
        raise AssertionError("production second observer rotation forbidden in RUST-072")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-072 rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid RUST-072 rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-072 rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("RUST-072 rotation authorization payload mismatch")
    rows = auth.get("observers")
    if not isinstance(rows, list) or not (
        THRESHOLD <= len(rows) <= len(PREDECESSOR_PINNED_OBSERVERS)
    ):
        raise AssertionError("invalid RUST-072 rotation authorization size")
    if not all(
        isinstance(row, dict) and frozenset(row) == AUTH_OBSERVER_KEYS for row in rows
    ):
        raise AssertionError("invalid RUST-072 rotation authorization entry")
    ids = [row["observer_id"] for row in rows]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-072 rotation authorizer ids must be unique and sorted")
    if any(observer_id not in PREDECESSOR_PINNED_OBSERVERS for observer_id in ids):
        raise AssertionError("unknown RUST-072 rotation authorizer")
...
def validate_final_bundle(bundle: dict, target: dict) -> None:
    ...
    if (
        ...
        raise AssertionError("invalid RUST-072 final observer bundle fields")
    if (
        bundle.get("observer_set_sequence") != FINAL_SET_SEQUENCE
        or bundle.get("observer_set_sha256") != set_sha
    ):
        raise AssertionError("RUST-072 final observer bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-072 final observer threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production final observation forbidden in RUST-072")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (
        THRESHOLD <= len(reports) <= len(FINAL_PINNED_OBSERVERS)
    ):
        raise AssertionError("invalid RUST-072 final observer report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-072 final observer ids must be unique and sorted")
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-072 final observer source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed RUST-072 final same-parent checkpoint fork")
        if not exact:
            raise AssertionError("RUST-072 final observer report does not match canonical checkpoint")


def verify(*args) -> None:
    if len(args) != 109:
        raise AssertionError("unexpected RUST-072 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 107:
        raise AssertionError("unexpected RUST-072 path count")

    base_paths = path_args[:100]
    old_bundle_path, first_rotation_path, first_auth_path, first_successor_path = path_args[100:104]
    second_rotation_path, second_auth_path, final_bundle_path = path_args[104:107]
    ...
    first_rotation_raw, _ = floor_verify.load_canonical(
        first_rotation_path, "RUST-072 first observer rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        first_auth_path, "RUST-072 first observer rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        first_successor_path, "RUST-072 first successor observer bundle"
    )
    second_rotation_raw, second_rotation = floor_verify.load_canonical(
        second_rotation_path, "RUST-072 second observer rotation"
    )
    _, second_auth = floor_verify.load_canonical(
        second_auth_path, "RUST-072 second rotation authorization"
    )
    _, final_bundle = floor_verify.load_canonical(
        final_bundle_path, "RUST-072 final observer bundle"
    )
    ...
    print(
        "RUST-072 multi-step observer rotation: GREEN "
...
def main() -> None:
    if len(sys.argv) != 111 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_072_multistep_monitor_rotation_journal_observer_set_rotation_verify.py verify "
