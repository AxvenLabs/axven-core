#!/usr/bin/env python3
"""RUST-061 TEST-ONLY observer-rotation-journal monitor rotation journal/checkpoint producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-061 TEST-only monitor rotation journal public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit(
            "usage: rust_061_observer_rotation_journal_monitor_rotation_journal_fixture.py SOURCE_SHA"
        )
    ...
    observed_checkpoint_raw, observed_checkpoint = floor_verify.load_canonical(
        ...
        "final observer-rotation-journal checkpoint",
    )
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust058-monitor-bundle.json", "old observer-rotation-journal monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust059-observer-rotation-journal-monitor-set-rotation.json",
        "first monitor rotation",
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust059-observer-rotation-journal-monitor-set-rotation-auth.json",
        "first monitor rotation authorization",
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust059-successor-monitor-bundle.json",
        "first successor monitor bundle",
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust060-second-observer-rotation-journal-monitor-set-rotation.json",
        "second monitor rotation",
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust060-second-observer-rotation-journal-monitor-set-rotation-auth.json",
        "second monitor rotation authorization",
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust060-final-monitor-bundle.json", "final monitor bundle"
    )
    ...
    (OUT / "axven-rust061-prefix-monitor-rotation-journal.json").write_bytes(
        prefix_journal_raw
    )
    (OUT / "axven-rust061-prefix-monitor-rotation-checkpoint.json").write_bytes(
        prefix_checkpoint_raw
    )
    (OUT / "axven-rust061-final-monitor-rotation-journal.json").write_bytes(
        final_journal_raw
    )
    (OUT / "axven-rust061-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust061-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-061 TEST-only observer-rotation-journal monitor rotation journal/checkpoint fixture: GREEN")
