#!/usr/bin/env python3
"""RUST-078: TEST-ONLY observer quorum for the RUST-077 final journal checkpoint."""
...
BUNDLE_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-checkpoint-observation-bundle-v1"
REPORT_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-checkpoint-observation-v1"
STATEMENT_SCHEMA = "axven-native-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-checkpoint-observation-statement-v1"
OBSERVATION_DOMAIN = b"AXVEN_NATIVE_MONITOR_ROTATION_JOURNAL_OBSERVER_ROTATION_JOURNAL_MONITOR_ROTATION_JOURNAL_CHECKPOINT_OBSERVATION_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

OBSERVER_1_ID = "rust-078-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-1-v1"
OBSERVER_2_ID = "rust-078-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-2-v1"
OBSERVER_3_ID = "rust-078-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-rotation-journal-observer-3-v1"
OBSERVER_1_PUBLIC_KEY = bytes.fromhex("efe73e55679c12ea72d2584b9bf7e248f2266f8609381ca8b60cc369a5334a8d")
OBSERVER_2_PUBLIC_KEY = bytes.fromhex("31f3322d4923d36c41c109bdb0099193187bed99942096e4926a24c77efd0d2f")
OBSERVER_3_PUBLIC_KEY = bytes.fromhex("fa4834147f6e690c3693eff61336046403cd8ae2a14f31b3c407358569239565")
PINNED_OBSERVERS = {
...
def canonical_target(checkpoint_raw: bytes, checkpoint: dict, source_sha: str) -> dict:
    ...
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("missing final RUST-077 monitor rotation journal checkpoint statement")
    if statement.get("schema") != journal_verify.STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError("invalid final RUST-077 monitor rotation journal checkpoint statement boundary")
    if statement.get("activation_source_commit") != source_sha:
        raise AssertionError("final RUST-077 monitor rotation journal checkpoint source mismatch")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if not isinstance(bundle, dict) or frozenset(bundle) != BUNDLE_KEYS or bundle.get("schema") != BUNDLE_SCHEMA:
        raise AssertionError("invalid RUST-078 journal checkpoint observer bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-078 journal checkpoint observer threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-078 journal checkpoint observer bundle forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_OBSERVERS)):
        raise AssertionError("invalid RUST-078 journal checkpoint observer report count")

    statements = [validate_report(report) for report in reports]
    ids = [statement["observer_id"] for statement in statements]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-078 journal checkpoint observer ids must be unique and sorted")

    matching = 0
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-078 journal checkpoint observer source mismatch")
        same_epoch_parent = (
        ...
        if not exact:
            raise AssertionError("RUST-078 observer report does not match canonical RUST-077 checkpoint")
        matching += 1
    if matching < THRESHOLD:
        raise AssertionError("RUST-078 journal checkpoint observer quorum below threshold")


def verify(*args) -> None:
    if len(args) != 125:
        raise AssertionError("unexpected RUST-078 verifier argument count")
    *base_paths, observation_bundle_path, expected_source_sha, required_floor_text = args
    if len(base_paths) != 122:
        raise AssertionError("unexpected RUST-078 base path count")

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[-2], "RUST-077 final monitor rotation journal"
    )
    ...
    journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-078 canonical final",
    )
    _, bundle = floor_verify.load_canonical(
        observation_bundle_path, "RUST-078 journal checkpoint observer bundle"
    )
    ...
    print(
        "RUST-078 monitor-rotation-journal checkpoint observation: GREEN "
...
def main() -> None:
    if len(sys.argv) != 127 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_078_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_verify.py verify "
