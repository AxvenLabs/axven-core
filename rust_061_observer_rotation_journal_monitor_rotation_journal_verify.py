#!/usr/bin/env python3
"""RUST-061: TEST-ONLY append-only observer-rotation-journal monitor rotation journal verifier."""
...
ENTRY_SCHEMA = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-entry-v1"
CHECKPOINT_SCHEMA = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-checkpoint-v1"
STATEMENT_SCHEMA = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-checkpoint-statement-v1"
CHECKPOINT_DOMAIN = b"AXVEN_NATIVE_OBSERVER_ROTATION_JOURNAL_MONITOR_SET_ROTATION_JOURNAL_CHECKPOINT_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
STATEMENT_KEYS = frozenset({
    ...
    "head_entry_sha256", "previous_checkpoint_sha256",
    "observer_rotation_journal_checkpoint_sha256",
    "observer_rotation_journal_checkpoint_statement_sha256",
...
def validate_journal(journal: dict, expected: dict, label: str) -> None:
    if not isinstance(journal, dict) or frozenset(journal) != JOURNAL_KEYS or journal.get("schema") != JOURNAL_SCHEMA:
        raise AssertionError(f"invalid {label} observer-rotation-journal monitor rotation journal fields")
    if journal.get("production") is not False:
        raise AssertionError(f"production {label} observer-rotation-journal monitor rotation journal forbidden in RUST-061")
    if journal != expected:
        raise AssertionError(f"{label} observer-rotation-journal monitor rotation journal continuity mismatch")
    entries = journal.get("entries")
    if not isinstance(entries, list) or not entries:
        raise AssertionError(f"empty {label} observer-rotation-journal monitor rotation journal")
    for index, entry in enumerate(entries):
        if not isinstance(entry, dict) or frozenset(entry) != ENTRY_KEYS or entry.get("schema") != ENTRY_SCHEMA:
            raise AssertionError(f"invalid {label} observer-rotation-journal monitor rotation journal entry")
        if type(entry.get("sequence")) is not int or entry["sequence"] != index:
            raise AssertionError(f"non-monotonic {label} observer-rotation-journal monitor rotation journal sequence")
        if index == 0:
            ...
            raise AssertionError(f"broken {label} observer-rotation-journal monitor rotation journal hash chain")
...
def validate_checkpoint_envelope(
    ...
    if not isinstance(checkpoint, dict) or frozenset(checkpoint) != CHECKPOINT_KEYS or checkpoint.get("schema") != CHECKPOINT_SCHEMA:
        raise AssertionError(f"invalid {label} observer-rotation-journal monitor rotation checkpoint fields")
    if checkpoint.get("algorithm") != ALGORITHM or checkpoint.get("production") is not False:
        raise AssertionError(f"invalid {label} observer-rotation-journal monitor rotation checkpoint boundary")
    if type(checkpoint.get("threshold")) is not int or checkpoint["threshold"] != THRESHOLD:
        raise AssertionError(f"invalid {label} observer-rotation-journal monitor rotation checkpoint threshold")
    statement = checkpoint.get("statement")
    if not isinstance(statement, dict) or frozenset(statement) != STATEMENT_KEYS:
        raise AssertionError(f"invalid {label} observer-rotation-journal monitor rotation statement fields")
    if statement.get("schema") != STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError(f"invalid {label} observer-rotation-journal monitor rotation statement boundary")
    monitors = checkpoint.get("monitors")
    if not isinstance(monitors, list) or not (THRESHOLD <= len(monitors) <= len(pins)):
        raise AssertionError(f"invalid {label} checkpoint signature count")
    if not all(isinstance(item, dict) and frozenset(item) == CHECKPOINT_MONITOR_KEYS for item in monitors):
        raise AssertionError(f"invalid {label} checkpoint signature row")
    ids = [item["monitor_id"] for item in monitors]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError(f"{label} checkpoint monitor ids must be unique and sorted")
    if any(monitor_id not in pins for monitor_id in ids):
        raise AssertionError(f"unknown {label} checkpoint signer")
...
def validate_checkpoint(
    ...
    if statement != expected_statement:
        raise AssertionError(f"{label} observer-rotation-journal monitor rotation checkpoint statement mismatch")
...
def reject_observed_fork(
    ...
    if same_parent and canonical_checkpoint_raw != observed_checkpoint_raw:
        raise AssertionError("observed same-parent observer-rotation-journal monitor-rotation-journal checkpoint fork")

def verify(*args) -> None:
    if len(args) != 80:
        raise AssertionError("unexpected RUST-061 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 78:
        raise AssertionError("unexpected RUST-061 path count")

    base_paths = path_args[:74]
    prefix_journal_path, prefix_checkpoint_path, final_journal_path, final_checkpoint_path = path_args[74:78]
    ...
    observed_checkpoint_raw, observed_checkpoint = floor_verify.load_canonical(
        base_paths[66], "final observer-rotation-journal checkpoint"
    )
    ...
    if target["activation_source_commit"] != expected_source_sha:
        raise AssertionError("RUST-061 checkpoint source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(base_paths[67], "old observer-rotation-journal monitor bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base_paths[68], "first monitor rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base_paths[69], "first monitor rotation authorization")
    first_successor_raw, _ = floor_verify.load_canonical(base_paths[70], "first successor monitor bundle")
    second_rotation_raw, _ = floor_verify.load_canonical(base_paths[71], "second monitor rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base_paths[72], "second monitor rotation authorization")
    final_bundle_raw, _ = floor_verify.load_canonical(base_paths[73], "final monitor bundle")
    ...
    prefix_journal_raw, prefix_journal = floor_verify.load_canonical(prefix_journal_path, "prefix monitor rotation journal")
    prefix_checkpoint_raw, prefix_checkpoint = floor_verify.load_canonical(prefix_checkpoint_path, "prefix monitor rotation checkpoint")
    final_journal_raw, final_journal = floor_verify.load_canonical(final_journal_path, "final monitor rotation journal")
    _, final_checkpoint = floor_verify.load_canonical(final_checkpoint_path, "final monitor rotation checkpoint")
    ...
    print(
        "RUST-061 append-only observer-rotation-journal monitor rotation journal: GREEN "
        f"source={expected_source_sha} entries=3 checkpoint={sha256(canonical(final_checkpoint))}"
    )

def main() -> None:
    if len(sys.argv) != 82 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_061_observer_rotation_journal_monitor_rotation_journal_verify.py verify "
