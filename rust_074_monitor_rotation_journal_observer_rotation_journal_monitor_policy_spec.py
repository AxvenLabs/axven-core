#!/usr/bin/env python3
"""RUST-074 static policy for TEST-ONLY RUST-073 checkpoint monitoring."""
...
DOC = ROOT / "docs/history/rust/RUST_074.md"
VERIFY = ROOT / "rust_074_monitor_rotation_journal_observer_rotation_journal_monitor_verify.py"
FIXTURE = ROOT / "rust_074_monitor_rotation_journal_observer_rotation_journal_monitor_fixture.py"
SELFTEST = ROOT / "rust_074_monitor_rotation_journal_observer_rotation_journal_monitor_selftest.py"
WORKFLOW = ROOT / ".github/workflows/native-monitor-rotation-journal-observer-rotation-journal-monitor-v2.yml"
BASE = ROOT / "rust_073_monitor_rotation_journal_observer_rotation_journal_verify.py"
EXPECTED_RUST073_GIT_BLOB = "69537ff6744cb39d1c23539face6d3999db25992"
...
def main() -> None:
    ...
    assert blob(BASE.read_bytes()) == EXPECTED_RUST073_GIT_BLOB
    require(
        verify,
        ("import rust_073_monitor_rotation_journal_observer_rotation_journal_verify as journal_verify",),
        "RUST-074 verifier composition",
    )
    checks += 1
    print("[GREEN] exact reviewed RUST-073 verifier is composed")
    ...
    checks += 1
    print("[GREEN] detached RUST-074 verifier/selftest have no signing or network capability")

    require(
        verify,
        (
            "THRESHOLD = 2",
            "production RUST-074 observer-rotation-journal monitoring forbidden",
            "observed monitor same-parent RUST-073 observer rotation journal checkpoint fork",
    ...
    require(
        ...
        (
            ...
            "RUST-074 TEST-only monitor public-key pin mismatch",
        ),
        "RUST-074 producer fixture",
    )
    require(
        selftest,
        (
    ...
    require(
        ...
        (
            ...
            'python-version: "3.13.15"', "chmod 0444", "axven-rust074",
            "env -i HOME=/tmp PATH=/usr/bin:/bin", "/usr/bin/python3 -S",
            'test ! -e "$c/rust_074_monitor_rotation_journal_observer_rotation_journal_monitor_fixture.py"',
            'test "$(find "$c" -maxdepth 1 -type f | wc -l)" -eq 47',
            'test "$(wc -l < /tmp/axven-rust074-paths)" -eq 112',
        ),
        "RUST-074 workflow",
    )
    for forbidden in (
    ...
    require(
        ...
        (
            ...
            "exact RUST-073 final checkpoint SHA-256",
            "same-parent", "RUST-073 checkpoint replay",
            "Production consensus remains Python-authoritative.",
        ),
        "RUST-074 documentation",
    )
    checks += 1
    print("[GREEN] documentation preserves TEST-only monitoring boundary")

    assert checks == 6
    print("RUST-074 static policy: 6/6 checks passed")
