#!/usr/bin/env python3
"""RUST-098: TEST-ONLY RUST-097 monitor-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-rust098-monitor-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-rust098-monitor-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-rust098-monitor-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST098_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

MONITOR_1_ID = "rust-098-test-only-monitor-rotation-journal-monitor-1-v1"
MONITOR_2_ID = "rust-098-test-only-monitor-rotation-journal-monitor-2-v1"
MONITOR_3_ID = "rust-098-test-only-monitor-rotation-journal-monitor-3-v1"
MONITOR_1_PUBLIC_KEY = bytes.fromhex("e5145a37d984d244ce11e69388cb36dedc828c1a277ca347d50f5076a60959e8")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("248acbdbaf9e050196de704bea2d68770e519150d103b587dae2d9cad53dd930")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("7d59c5623dd40a74aa4d5a32ac645d3b3f95daeae4c22be25476dd6a486f7382")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust097_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust097_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-098 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-098 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-098 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-098 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-098 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-098 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-098 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-097 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-098 monitor report does not match canonical RUST-097 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-098 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-098 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-098 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 180:
        raise AssertionError("unexpected RUST-098 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 178:
        raise AssertionError("unexpected RUST-098 path count")
    base_paths = path_args[:177]
    monitor_bundle_path = path_args[177]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[175], "RUST-097 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 182 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_098_rust097_checkpoint_monitor_verify.py verify "
            "... RUST097_FINAL_CHECKPOINT MONITOR_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
