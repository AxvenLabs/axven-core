#!/usr/bin/env python3
"""RUST-131 detached monitor-set rotation availability/fail-closed selftest."""
...
import rust_131_rust130_checkpoint_monitor_rotation_verify as rotation_verify
...
def main() -> None:
    if len(sys.argv) != 273:
        raise SystemExit(
            "usage: rust_131_rust130_checkpoint_monitor_rotation_selftest.py "
    ...
    if len(base) != 269:
        raise AssertionError("unexpected RUST-131 selftest base path count")
    ...
    rotation_raw, rotation = floor_verify.load_canonical(base[266], "RUST-131 monitor-set rotation")
    _, auth = floor_verify.load_canonical(base[267], "RUST-131 rotation authorization")
    _, successor = floor_verify.load_canonical(base[268], "RUST-131 successor monitor bundle")
    target = monitor_verify.checkpoint_target(checkpoint_raw, checkpoint["statement"])
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-131 selftest target source mismatch")
    ...
    for subset in itertools.combinations(auth["monitors"], rotation_verify.THRESHOLD):
        ...
        auth_ok += 1
    if auth_ok != 3:
        raise AssertionError("unexpected RUST-131 predecessor authorization subset count")
    print("[GREEN] RUST-131 predecessor authorization availability: 3/3 valid two-monitor subsets accepted")
    ...
    for subset in itertools.combinations(successor["reports"], rotation_verify.THRESHOLD):
        ...
        successor_ok += 1
    if successor_ok != 3:
        raise AssertionError("unexpected RUST-131 successor monitoring subset count")
    print("[GREEN] RUST-131 successor monitoring availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("successor-threshold", lambda: run_with(268, write("m.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"] = value["reports"][:1]
        fail("successor-below-threshold", lambda: run_with(268, write("n.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][1] = copy.deepcopy(value["reports"][0])
        fail("successor-duplicate", lambda: run_with(268, write("o.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"] = list(reversed(value["reports"]))
        fail("successor-unsorted", lambda: run_with(268, write("p.json", value))); cases += 1
        value = copy.deepcopy(successor); value["monitor_set_sequence"] = 0
        fail("successor-set-sequence", lambda: run_with(268, write("q.json", value))); cases += 1
        value = copy.deepcopy(successor); value["monitor_set_sha256"] = "0" * 64
        fail("successor-set-digest", lambda: run_with(268, write("r.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][0]["statement"]["monitor_id"] = rotation_verify.REVOKED_MONITOR_ID
        fail("revoked-monitor-resurrection", lambda: run_with(268, write("s.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][0]["statement"]["successor_monitor_set_sequence"] = 0
        fail("successor-statement-set-sequence", lambda: run_with(268, write("ss.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][0]["statement"]["successor_monitor_set_sha256"] = "0" * 64
        fail("successor-statement-set-digest", lambda: run_with(268, write("sd.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"st{idx}.json": run_with(268, write(n, v)),
            )
            cases += 1
        ...
        fail("successor-signature", lambda: run_with(268, write("t.json", value))); cases += 1
        value = copy.deepcopy(successor); value["production"] = True
        fail("successor-production", lambda: run_with(268, write("u.json", value))); cases += 1

        noncanonical = root / "noncanonical.json"
        noncanonical.write_text(json.dumps(successor, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-successor", lambda: run_with(268, noncanonical)); cases += 1
        fail("old-rust130-bundle-replay", lambda: run_with(268, base[265])); cases += 1
        fail("observed-valid-successor-same-parent-fork", lambda: run_with(268, fork_path)); cases += 1

    if cases != 50:
        raise AssertionError(f"unexpected RUST-131 selftest case count: {cases}")
    print("RUST-131 monitor-set rotation fail-closed contract: 50/50 expected cases passed")
