#!/usr/bin/env python3
"""RUST-126: TEST-ONLY RUST-125 monitor-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-rust126-monitor-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-rust126-monitor-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-rust126-monitor-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST126_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
MONITOR_1_PUBLIC_KEY = bytes.fromhex("fc947730f49eb01427a66e050733294d9e520e545c7a27125a780634e0860a27")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("68460ebef3b138164ec7fd8610e95800df7598f70f2f2ea7db5172ac74ebc144")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("350d62629e4a2ed9aaf2b42323d78151b1a905ee9c92ce4dc9c28500be310f61")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust125_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust125_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-126 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-126 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-126 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-126 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-126 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-126 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-126 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-125 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-126 monitor report does not match canonical RUST-125 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-126 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-126 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-126 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 257:
        raise AssertionError("unexpected RUST-126 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 255:
        raise AssertionError("unexpected RUST-126 path count")
    base_paths = path_args[:254]
    monitor_bundle_path = path_args[254]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[252], "RUST-125 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 259 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_126_rust125_checkpoint_monitor_verify.py verify "
            "... RUST125_FINAL_CHECKPOINT MONITOR_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
