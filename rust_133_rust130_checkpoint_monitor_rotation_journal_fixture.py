#!/usr/bin/env python3
"""RUST-133 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
OUT = Path("/tmp")
SEEDS = {
    monitor_verify.MONITOR_2_ID: "2e" * 32,
    monitor_verify.MONITOR_3_ID: "3e" * 32,
    rotation1_verify.M4_ID: "4e" * 32,
    rotation2_verify.M5_ID: "5e" * 32,
}
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-133 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_133_rust130_checkpoint_monitor_rotation_journal_fixture.py SOURCE_SHA")
    source_sha = sys.argv[1]

    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust129-final-monitor-rotation-checkpoint.json",
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-133 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust130-monitor-bundle.json", "RUST-133 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust131-monitor-set-rotation.json", "RUST-133 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust131-monitor-set-rotation-auth.json", "RUST-133 first rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust131-successor-monitor-bundle.json", "RUST-133 first successor monitor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust132-second-monitor-set-rotation.json", "RUST-133 second monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust132-second-monitor-set-rotation-auth.json", "RUST-133 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust132-final-monitor-bundle.json", "RUST-133 final monitor bundle"
    )
    ...
    (OUT / "axven-rust133-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust133-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust133-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust133-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust133-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-133 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
