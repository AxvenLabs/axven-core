#!/usr/bin/env python3
"""RUST-134 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 281:
        raise SystemExit(
            "usage: rust_134_rust133_checkpoint_monitor_selftest.py "
    ...
    if len(base) != 277:
        raise AssertionError("unexpected RUST-134 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[274], "RUST-133 final monitor rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-134 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[276], "RUST-134 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-134 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-134 monitor subset count")
    print("[GREEN] RUST-134 monitor availability: 3/3 valid two-monitor subsets accepted")

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path("/tmp/axven-rust133-observed-fork-monitor-rotation-checkpoint.json"),
        "RUST-133 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-134 signed observed same-parent fork evidence validated")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        paths = list(base); paths[276] = noncanonical
        fail(
            "noncanonical-monitor-bundle",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[276] = base[275]
        fail(
            "rust133-checkpoint-replay",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[276] = fork_bundle_path
        fail(
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-134 fail-closed case count: {cases}")
    print("RUST-134 RUST-133 journal checkpoint monitor fail-closed contract: 27/27 expected cases passed")
