#!/usr/bin/env python3
"""RUST-062 detached availability and fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 83:
        raise SystemExit(
            "usage: rust_062_observer_rotation_journal_monitor_rotation_journal_gossip_selftest.py "
            "... FINAL_MONITOR_ROTATION_CHECKPOINT BUNDLE FORK_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
    ...
    *base_paths, bundle_path, fork_bundle_path = paths
    ...
    if len(base_paths) != 78:
        raise AssertionError("unexpected RUST-062 selftest base path count")
    ...
    _, bundle = floor_verify.load_canonical(
        bundle_path, "monitor-rotation-journal observer bundle"
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print(
            "[GREEN] RUST-062 monitor-rotation-journal observer availability: "
        ...
        value = copy.deepcopy(bundle); value["schema"] = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-checkpoint-observation-bundle-v0"; mutations.append(("bundle-schema", value))
        value = copy.deepcopy(bundle); value["reports"][0]["schema"] = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-checkpoint-observation-v0"; mutations.append(("report-schema", value))
        ...
        value = copy.deepcopy(bundle); value["reports"][0]["statement"]["observer_rotation_journal_checkpoint_sha256"] = "0" * 64; mutations.append(("observer-rotation-journal-checkpoint-digest", value))
        value = copy.deepcopy(bundle); value["reports"][0]["statement"]["observer_rotation_journal_checkpoint_statement_sha256"] = "0" * 64; mutations.append(("observer-rotation-journal-checkpoint-statement-digest", value))
        ...
        fail(
            "observed-cross-observer-monitor-rotation-journal-fork",
            lambda: run(fork_bundle_path),
        ); cases += 1

    if cases != 25:
        raise AssertionError(
            f"unexpected RUST-062 selftest case count: {cases}"
        )
    print(
        "RUST-062 monitor-rotation-journal observation fail-closed contract: "
