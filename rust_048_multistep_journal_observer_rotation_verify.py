#!/usr/bin/env python3
"""RUST-048: TEST-ONLY multi-step monitor-journal observer rotation verifier."""
...
ROTATION_AUTH_SCHEMA = "axven-native-monitor-journal-observer-set-rotation-quorum-v2"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-monitor-journal-observer-set-rotation.v2+json"
FINAL_BUNDLE_SCHEMA = "axven-native-monitor-rotation-journal-checkpoint-observation-bundle-v3"
FINAL_REPORT_SCHEMA = "axven-native-monitor-rotation-journal-checkpoint-observation-v3"
FINAL_STATEMENT_SCHEMA = "axven-native-monitor-rotation-journal-checkpoint-observation-statement-v3"
ROTATION_DOMAIN = b"AXVEN_NATIVE_MONITOR_JOURNAL_OBSERVER_SET_ROTATION_V2\x00"
FINAL_DOMAIN = b"AXVEN_NATIVE_MONITOR_ROTATION_JOURNAL_CHECKPOINT_OBSERVATION_V3\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
FINAL_PINNED_OBSERVERS = {
    ...
    rotation1_verify.J4_ID: rotation1_verify.J4_PUBLIC_KEY,
    J5_ID: J5_PUBLIC_KEY,
}
ROTATION_KEYS = frozenset({
    ...
    "checkpoint_sha256", "checkpoint_statement_sha256", "activation_source_commit", "production",
})
AUTH_KEYS = frozenset({"schema", "algorithm", "threshold", "payload_type", "payload_sha256", "observers", "production"})
AUTH_OBSERVER_KEYS = frozenset({"observer_id", "signature"})
FINAL_BUNDLE_KEYS = frozenset({"schema", "observer_set_sequence", "observer_set_sha256", "threshold", "reports", "production"})
...
FINAL_STATEMENT_KEYS = frozenset({
    ...
    "checkpoint_sha256", "checkpoint_statement_sha256", "monitor_set_sequence", "monitor_set_sha256",
    "journal_sha256", "head_entry_sha256", "previous_checkpoint_sha256", "observer_checkpoint_sha256",
...
def validate_rotation(rotation: dict, target: dict, first_rotation_raw: bytes, first_successor_raw: bytes, source_sha: str) -> None:
    if not isinstance(rotation, dict) or frozenset(rotation) != ROTATION_KEYS or rotation.get("schema") != ROTATION_SCHEMA:
        raise AssertionError("invalid second journal-observer rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != FINAL_SET_SEQUENCE:
        raise AssertionError("invalid second journal-observer rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(rotation1_verify.new_observer_set())):
        raise AssertionError("second journal-observer predecessor mismatch")
    if rotation.get("to_set") != final_observer_set():
        raise AssertionError("unexpected final journal-observer set")
    if rotation.get("cumulative_revoked_observer_ids") != CUMULATIVE_REVOKED_OBSERVER_IDS:
        raise AssertionError("cumulative journal-observer revocation mismatch")
    ...
    if rotation.get("checkpoint_statement_sha256") != target["checkpoint_statement_sha256"]:
        raise AssertionError("second rotation checkpoint-statement binding mismatch")
    if rotation.get("activation_source_commit") != source_sha:
        raise AssertionError("second journal-observer rotation source mismatch")
    if rotation.get("production") is not False:
        raise AssertionError("production second journal-observer rotation forbidden in RUST-048")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if not isinstance(auth, dict) or frozenset(auth) != AUTH_KEYS or auth.get("schema") != ROTATION_AUTH_SCHEMA:
        raise AssertionError("invalid second journal-observer rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid second journal-observer rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid second journal-observer rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("second journal-observer rotation authorization payload mismatch")
    ...
    if not isinstance(observers, list) or not (THRESHOLD <= len(observers) <= len(PREDECESSOR_PINNED_OBSERVERS)):
        raise AssertionError("invalid second journal-observer rotation authorization size")
    if not all(isinstance(item, dict) and frozenset(item) == AUTH_OBSERVER_KEYS for item in observers):
        raise AssertionError("invalid second journal-observer rotation authorization entry")
    ...
    if any(observer_id not in PREDECESSOR_PINNED_OBSERVERS for observer_id in ids):
        raise AssertionError("unknown second journal-observer rotation authorizer")
    message = rotation_message(rotation_raw)
    for item in observers:
        material_verify.ed25519_verify(
            PREDECESSOR_PINNED_OBSERVERS[item["observer_id"]],
            material_verify.decode_signature(item["signature"]), message,
        )
...
def validate_final_bundle(bundle: dict, target: dict) -> None:
    ...
    if not isinstance(bundle, dict) or frozenset(bundle) != FINAL_BUNDLE_KEYS or bundle.get("schema") != FINAL_BUNDLE_SCHEMA:
        raise AssertionError("invalid final journal-observation bundle fields")
    if bundle.get("observer_set_sequence") != FINAL_SET_SEQUENCE or bundle.get("observer_set_sha256") != set_sha:
        raise AssertionError("final journal-observation bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid final journal-observation threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production final journal observation forbidden in RUST-048")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(FINAL_PINNED_OBSERVERS)):
        raise AssertionError("invalid final journal-observation report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("final journal-observer ids must be unique and sorted")
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("final journal-observer source mismatch")
        same_epoch_parent = (
        ...
        if not exact:
            raise AssertionError("final journal-observer report does not match canonical checkpoint")


def verify(*args) -> None:
    if len(args) != 43:
        raise AssertionError("unexpected RUST-048 verifier argument count")
    *base_paths, old_bundle_path, first_rotation_path, first_auth_path, first_successor_path, second_rotation_path, second_auth_path, final_bundle_path, expected_source_sha, required_floor_text = args
    if len(base_paths) != 34:
        raise AssertionError("unexpected RUST-048 base path count")
    rotation1_verify.verify(
        *base_paths, old_bundle_path, first_rotation_path, first_auth_path, first_successor_path,
    ...
    first_successor_raw, _ = floor_verify.load_canonical(first_successor_path, "first successor journal-observer bundle")
    second_rotation_raw, second_rotation = floor_verify.load_canonical(second_rotation_path, "second journal-observer rotation")
    _, second_auth = floor_verify.load_canonical(second_auth_path, "second journal-observer rotation authorization")
    _, final_bundle = floor_verify.load_canonical(final_bundle_path, "final journal-observer bundle")
    ...
    print(
        "RUST-048 multi-step journal-observer rotation: GREEN "
        f"source={expected_source_sha} sequence=2 revoked={','.join(CUMULATIVE_REVOKED_OBSERVER_IDS)} final={ids}"
    )


def main() -> None:
    if len(sys.argv) != 45 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_048_multistep_journal_observer_rotation_verify.py verify "
            "... FIRST_ROTATION FIRST_AUTH FIRST_SUCCESSOR SECOND_ROTATION SECOND_AUTH FINAL_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
