#!/usr/bin/env python3
"""RUST-102 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 193:
        raise SystemExit(
            "usage: rust_102_rust101_checkpoint_monitor_selftest.py "
    ...
    if len(base) != 189:
        raise AssertionError("unexpected RUST-102 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[186], "RUST-101 final monitor rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-102 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[188], "RUST-102 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-102 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-102 monitor subset count")
    print("[GREEN] RUST-102 monitor availability: 3/3 valid two-monitor subsets accepted")

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path("/tmp/axven-rust101-observed-fork-monitor-rotation-checkpoint.json"),
        "RUST-101 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-102 signed observed same-parent fork evidence validated")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        paths = list(base); paths[188] = noncanonical
        fail(
            "noncanonical-monitor-bundle",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[188] = base[187]
        fail(
            "rust101-checkpoint-replay",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[188] = fork_bundle_path
        fail(
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-102 fail-closed case count: {cases}")
    print("RUST-102 RUST-101 journal checkpoint monitor fail-closed contract: 27/27 expected cases passed")
