#!/usr/bin/env python3
"""RUST-053: TEST-ONLY append-only journal-monitor rotation journal/checkpoint verifier."""
...
ENTRY_SCHEMA = "axven-native-journal-observer-journal-monitor-set-rotation-journal-entry-v1"
CHECKPOINT_SCHEMA = "axven-native-journal-observer-journal-monitor-set-rotation-journal-checkpoint-v1"
STATEMENT_SCHEMA = "axven-native-journal-observer-journal-monitor-set-rotation-journal-checkpoint-statement-v1"
CHECKPOINT_DOMAIN = b"AXVEN_NATIVE_JOURNAL_OBSERVER_JOURNAL_MONITOR_SET_ROTATION_JOURNAL_CHECKPOINT_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

JOURNAL_KEYS = frozenset({
    "schema", "activation_source_commit", "journal_observer_checkpoint_sha256",
...
def validate_journal(journal: dict, expected: dict, label: str) -> None:
    if not isinstance(journal, dict) or frozenset(journal) != JOURNAL_KEYS or journal.get("schema") != JOURNAL_SCHEMA:
        raise AssertionError(f"invalid {label} journal-monitor journal fields")
    if journal.get("production") is not False:
        raise AssertionError(f"production {label} journal-monitor journal forbidden in RUST-053")
    if journal != expected:
        raise AssertionError(f"{label} journal-monitor journal continuity mismatch")
    entries = journal.get("entries")
    if not isinstance(entries, list) or not entries:
        raise AssertionError(f"empty {label} journal-monitor journal")
    for index, entry in enumerate(entries):
        if not isinstance(entry, dict) or frozenset(entry) != ENTRY_KEYS or entry.get("schema") != ENTRY_SCHEMA:
            raise AssertionError(f"invalid {label} journal-monitor journal entry")
        if type(entry.get("sequence")) is not int or entry["sequence"] != index:
            raise AssertionError(f"non-monotonic {label} journal-monitor journal sequence")
        if index == 0:
            if entry.get("predecessor_entry_sha256") is not None:
                raise AssertionError(f"unexpected {label} journal-monitor journal genesis predecessor")
        elif entry.get("predecessor_entry_sha256") != sha256(canonical(entries[index - 1])):
            raise AssertionError(f"broken {label} journal-monitor journal hash chain")
...
def validate_checkpoint_envelope(
    ...
    if not isinstance(checkpoint, dict) or frozenset(checkpoint) != CHECKPOINT_KEYS or checkpoint.get("schema") != CHECKPOINT_SCHEMA:
        raise AssertionError(f"invalid {label} journal-monitor checkpoint fields")
    if checkpoint.get("algorithm") != ALGORITHM or checkpoint.get("production") is not False:
        raise AssertionError(f"invalid {label} journal-monitor checkpoint boundary")
    if type(checkpoint.get("threshold")) is not int or checkpoint["threshold"] != THRESHOLD:
        raise AssertionError(f"invalid {label} journal-monitor checkpoint threshold")
    statement = checkpoint.get("statement")
    if not isinstance(statement, dict) or frozenset(statement) != STATEMENT_KEYS:
        raise AssertionError(f"invalid {label} journal-monitor checkpoint statement fields")
    if statement.get("schema") != STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError(f"invalid {label} journal-monitor checkpoint statement boundary")
    monitors = checkpoint.get("monitors")
    if not isinstance(monitors, list) or not (THRESHOLD <= len(monitors) <= len(pins)):
        raise AssertionError(f"invalid {label} journal-monitor checkpoint signature count")
    if not all(
        isinstance(item, dict) and frozenset(item) == CHECKPOINT_MONITOR_KEYS
        for item in monitors
    ):
        raise AssertionError(f"invalid {label} journal-monitor checkpoint signature row")
    ids = [item["monitor_id"] for item in monitors]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError(f"{label} journal-monitor checkpoint ids must be unique and sorted")
    if any(monitor_id not in pins for monitor_id in ids):
        raise AssertionError(f"unknown {label} journal-monitor checkpoint signer")
...
def validate_checkpoint(
    ...
    if statement != expected_statement:
        raise AssertionError(f"{label} journal-monitor checkpoint statement mismatch")
...
def reject_observed_fork(
    ...
    left = validate_checkpoint_envelope(
        canonical_checkpoint, journal_raw, pins, "canonical final"
    )
    right = validate_checkpoint_envelope(
        observed_checkpoint, journal_raw, pins, "observed final"
    )
    same_parent = (
        left.get("monitor_set_sequence") == right.get("monitor_set_sequence")
        and left.get("previous_checkpoint_sha256")
        == right.get("previous_checkpoint_sha256")
    )
...
def verify(*args) -> None:
    if len(args) != 58:
        raise AssertionError("unexpected RUST-053 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 56:
        raise AssertionError("unexpected RUST-053 path count")

    base_paths = path_args[:52]
    prefix_journal_path, prefix_checkpoint_path, final_journal_path, final_checkpoint_path = path_args[52:56]

    rotation2_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_observer_checkpoint_raw, final_journal_observer_checkpoint = (
        floor_verify.load_canonical(
            base_paths[44], "final journal-observer checkpoint"
        )
    )
    target = monitor_verify.checkpoint_target(
        final_journal_observer_checkpoint_raw,
        final_journal_observer_checkpoint["statement"],
    )
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        base_paths[45], "old journal-monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        base_paths[46], "first journal-monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        base_paths[47], "first journal-monitor rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        base_paths[48], "first journal-monitor successor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        base_paths[49], "second journal-monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        base_paths[50], "second journal-monitor rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        base_paths[51], "final journal-monitor bundle"
    )
    ...
    prefix_journal_raw, prefix_journal = floor_verify.load_canonical(
        prefix_journal_path, "prefix journal-monitor journal"
    )
    prefix_checkpoint_raw, prefix_checkpoint = floor_verify.load_canonical(
        prefix_checkpoint_path, "prefix journal-monitor checkpoint"
    )
    final_journal_raw, final_journal = floor_verify.load_canonical(
        final_journal_path, "final journal-monitor journal"
    )
    _, final_checkpoint = floor_verify.load_canonical(
        final_checkpoint_path, "final journal-monitor checkpoint"
    )

    validate_journal(
        prefix_journal,
        expected_journal(entries[:2], expected_source_sha, target),
        "prefix",
    )
    ...
    validate_journal(
        final_journal,
        expected_journal(entries, expected_source_sha, target),
        "final",
    )
    ...
    print(
        "RUST-053 append-only journal-monitor rotation journal: GREEN "
        f"source={expected_source_sha} entries=3 "
        f"checkpoint={sha256(canonical(final_checkpoint))}"
    )


def main() -> None:
    if len(sys.argv) != 60 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_053_journal_monitor_rotation_journal_verify.py verify "
            "... PREFIX_JOURNAL PREFIX_CHECKPOINT FINAL_JOURNAL FINAL_CHECKPOINT "
            "SOURCE_SHA REQUIRED_FLOOR"
        )
