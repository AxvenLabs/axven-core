#!/usr/bin/env python3
"""RUST-126 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 259:
        raise SystemExit(
            "usage: rust_126_rust125_checkpoint_monitor_selftest.py "
    ...
    if len(base) != 255:
        raise AssertionError("unexpected RUST-126 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[252], "RUST-125 final monitor rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-126 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[254], "RUST-126 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-126 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-126 monitor subset count")
    print("[GREEN] RUST-126 monitor availability: 3/3 valid two-monitor subsets accepted")

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path("/tmp/axven-rust125-observed-fork-monitor-rotation-checkpoint.json"),
        "RUST-125 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-126 signed observed same-parent fork evidence validated")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        paths = list(base); paths[254] = noncanonical
        fail(
            "noncanonical-monitor-bundle",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[254] = base[253]
        fail(
            "rust125-checkpoint-replay",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[254] = fork_bundle_path
        fail(
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-126 fail-closed case count: {cases}")
    print("RUST-126 RUST-125 journal checkpoint monitor fail-closed contract: 27/27 expected cases passed")
