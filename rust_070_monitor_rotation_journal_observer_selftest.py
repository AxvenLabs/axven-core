#!/usr/bin/env python3
"""RUST-070 detached observer availability and fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 105:
        raise SystemExit(
            "usage: rust_070_monitor_rotation_journal_observer_selftest.py "
    ...
    *base_paths, bundle_path, fork_bundle_path = paths
    ...
    if len(base_paths) != 100:
        raise AssertionError("unexpected RUST-070 selftest base path count")
    ...
    _, bundle = floor_verify.load_canonical(
        bundle_path, "RUST-070 monitor-rotation-journal observer bundle"
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print(
            "[GREEN] RUST-070 monitor-rotation-journal observer availability: "
        ...
        fail("observed-cross-observer-RUST-069-monitor-rotation-journal-fork", lambda: run(fork_bundle_path)); cases += 1

    if cases != 25:
        raise AssertionError(f"unexpected RUST-070 selftest case count: {cases}")
    print(
        "RUST-070 monitor-rotation-journal observation fail-closed contract: "
