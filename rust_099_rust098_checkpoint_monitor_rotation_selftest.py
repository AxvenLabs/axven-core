#!/usr/bin/env python3
"""RUST-099 detached monitor-set rotation availability/fail-closed selftest."""
...
import rust_099_rust098_checkpoint_monitor_rotation_verify as rotation_verify
...
def main() -> None:
    if len(sys.argv) != 185:
        raise SystemExit(
            "usage: rust_099_rust098_checkpoint_monitor_rotation_selftest.py "
    ...
    if len(base) != 181:
        raise AssertionError("unexpected RUST-099 selftest base path count")
    ...
    rotation_raw, rotation = floor_verify.load_canonical(base[178], "RUST-099 monitor-set rotation")
    _, auth = floor_verify.load_canonical(base[179], "RUST-099 rotation authorization")
    _, successor = floor_verify.load_canonical(base[180], "RUST-099 successor monitor bundle")
    target = monitor_verify.checkpoint_target(checkpoint_raw, checkpoint["statement"])
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-099 selftest target source mismatch")
    ...
    for subset in itertools.combinations(auth["monitors"], rotation_verify.THRESHOLD):
        ...
        auth_ok += 1
    if auth_ok != 3:
        raise AssertionError("unexpected RUST-099 predecessor authorization subset count")
    print("[GREEN] RUST-099 predecessor authorization availability: 3/3 valid two-monitor subsets accepted")
    ...
    for subset in itertools.combinations(successor["reports"], rotation_verify.THRESHOLD):
        ...
        successor_ok += 1
    if successor_ok != 3:
        raise AssertionError("unexpected RUST-099 successor monitoring subset count")
    print("[GREEN] RUST-099 successor monitoring availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("rotation-sequence", lambda: run_with(178, write("a.json", value))); cases += 1
        value = copy.deepcopy(rotation); value["from_set_sha256"] = "0" * 64
        fail("rotation-predecessor-set", lambda: run_with(178, write("b.json", value))); cases += 1
        value = copy.deepcopy(rotation); value["to_set"]["threshold"] = 1
        fail("rotation-successor-set", lambda: run_with(178, write("c.json", value))); cases += 1
        value = copy.deepcopy(rotation); value["revoked_monitor_ids"] = []
        fail("rotation-revocation", lambda: run_with(178, write("d.json", value))); cases += 1
        value = copy.deepcopy(rotation); value["predecessor_monitor_bundle_sha256"] = "0" * 64
        fail("rotation-predecessor-bundle", lambda: run_with(178, write("e.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"rt{idx}.json": run_with(178, write(n, v)),
            )
            cases += 1

        value = copy.deepcopy(rotation); value["production"] = True
        fail("rotation-production", lambda: run_with(178, write("f.json", value))); cases += 1

        value = copy.deepcopy(auth); value["threshold"] = 1
        fail("auth-threshold", lambda: run_with(179, write("g.json", value))); cases += 1
        value = copy.deepcopy(auth); value["monitors"] = value["monitors"][:1]
        fail("auth-below-threshold", lambda: run_with(179, write("h.json", value))); cases += 1
        value = copy.deepcopy(auth); value["monitors"] = [value["monitors"][0], copy.deepcopy(value["monitors"][0])]
        fail("auth-duplicate", lambda: run_with(179, write("i.json", value))); cases += 1
        value = copy.deepcopy(auth); value["monitors"] = list(reversed(value["monitors"]))
        fail("auth-unsorted", lambda: run_with(179, write("j.json", value))); cases += 1
        value = copy.deepcopy(auth); value["payload_sha256"] = "0" * 64
        fail("auth-payload", lambda: run_with(179, write("k.json", value))); cases += 1
        ...
        fail("auth-signature", lambda: run_with(179, write("l.json", value))); cases += 1

        value = copy.deepcopy(successor); value["threshold"] = 1
        fail("successor-threshold", lambda: run_with(180, write("m.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"] = value["reports"][:1]
        fail("successor-below-threshold", lambda: run_with(180, write("n.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][1] = copy.deepcopy(value["reports"][0])
        fail("successor-duplicate", lambda: run_with(180, write("o.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"] = list(reversed(value["reports"]))
        fail("successor-unsorted", lambda: run_with(180, write("p.json", value))); cases += 1
        value = copy.deepcopy(successor); value["monitor_set_sequence"] = 0
        fail("successor-set-sequence", lambda: run_with(180, write("q.json", value))); cases += 1
        value = copy.deepcopy(successor); value["monitor_set_sha256"] = "0" * 64
        fail("successor-set-digest", lambda: run_with(180, write("r.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][0]["statement"]["monitor_id"] = rotation_verify.REVOKED_MONITOR_ID
        fail("revoked-monitor-resurrection", lambda: run_with(180, write("s.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][0]["statement"]["successor_monitor_set_sequence"] = 0
        fail("successor-statement-set-sequence", lambda: run_with(180, write("ss.json", value))); cases += 1
        value = copy.deepcopy(successor); value["reports"][0]["statement"]["successor_monitor_set_sha256"] = "0" * 64
        fail("successor-statement-set-digest", lambda: run_with(180, write("sd.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"st{idx}.json": run_with(180, write(n, v)),
            )
            cases += 1
        ...
        fail("successor-signature", lambda: run_with(180, write("t.json", value))); cases += 1
        value = copy.deepcopy(successor); value["production"] = True
        fail("successor-production", lambda: run_with(180, write("u.json", value))); cases += 1

        noncanonical = root / "noncanonical.json"
        noncanonical.write_text(json.dumps(successor, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-successor", lambda: run_with(180, noncanonical)); cases += 1
        fail("old-rust098-bundle-replay", lambda: run_with(180, base[177])); cases += 1
        fail("observed-valid-successor-same-parent-fork", lambda: run_with(180, fork_path)); cases += 1

    if cases != 50:
        raise AssertionError(f"unexpected RUST-099 selftest case count: {cases}")
    print("RUST-099 monitor-set rotation fail-closed contract: 50/50 expected cases passed")
