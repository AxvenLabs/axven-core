#!/usr/bin/env python3
"""RUST-128 static policy for TEST-ONLY second RUST-126 checkpoint monitor rotation."""
...
DOC = ROOT / "docs/history/rust/RUST_128.md"
VERIFY = ROOT / "rust_128_multistep_rust126_checkpoint_monitor_rotation_verify.py"
FIXTURE = ROOT / "rust_128_multistep_rust126_checkpoint_monitor_rotation_fixture.py"
SELFTEST = ROOT / "rust_128_multistep_rust126_checkpoint_monitor_rotation_selftest.py"
WORKFLOW = ROOT / ".github/workflows/native-rust128-multistep-checkpoint-monitor-rotation.yml"
BASE = ROOT / "rust_127_rust126_checkpoint_monitor_rotation_verify.py"
PREDECESSOR_WORKFLOW = ROOT / ".github/workflows/native-rust127-rust126-checkpoint-monitor-rotation.yml"
EXPECTED_RUST127_GIT_BLOB = "decc79603212233ec4261143a8b6a9b88ebed556"
EXPECTED_RUST127_WORKFLOW_GIT_BLOB = "548fed704d87405d416cc0bb534ebd22cbf2a0e1"
...
def main() -> None:
    ...
    assert blob(BASE.read_bytes()) == EXPECTED_RUST127_GIT_BLOB
    assert blob(PREDECESSOR_WORKFLOW.read_bytes()) == EXPECTED_RUST127_WORKFLOW_GIT_BLOB
    require(
        verify,
        ("import rust_127_rust126_checkpoint_monitor_rotation_verify as rotation1_verify",),
        "RUST-128 verifier composition",
    )
    checks += 1
    print("[GREEN] exact reviewed RUST-127 verifier and workflow are pinned")
    ...
    checks += 1
    print("[GREEN] detached RUST-128 verifier/selftest have no signing or network capability")
    ...
    require(
        ...
        (
            ...
            "final_monitor_set_sha256", "observed RUST-128 final same-parent checkpoint fork",
            "*monitor_verify.TARGET_KEYS",
            "base_paths[253]", "base_paths[255]", "base_paths[256]", "base_paths[257]",
            "path_args[258:261]",
        ),
        "RUST-128 verifier",
    )
    ...
    require(
        ...
        (
            ...
            "Ed25519PrivateKey", "RUST-128 TEST-only monitor public-key pin mismatch",
        ),
        "RUST-128 producer fixture",
    )
    require(
        selftest,
        (
    ...
    require(
        ...
        (
            ...
            'python-version: "3.13.15"', "chmod 0444", "axven-rust128",
            "env -i HOME=/tmp PATH=/usr/bin:/bin", "/usr/bin/python3 -S",
            '"rust_*.py"', '"RUST_*.md"', 'printf -v n3 "%03d" "$n"',
            'test ! -e "$c/rust_128_multistep_rust126_checkpoint_monitor_rotation_fixture.py"',
            'test "$(find "$c" -maxdepth 1 -type f | wc -l)" -eq 101',
            "expected 258 RUST-127 paths", "expected 261 RUST-128 paths",
            "axven-rust128-final-monitor-bundle.json",
        ),
        "RUST-128 workflow",
    )
    for forbidden in (
    ...
    require(
        ...
        (
            ...
            "fixed 261-path manifest", "101-file verifier-only detached consumer",
            "Production consensus remains Python-authoritative.",
        ),
        "RUST-128 documentation",
    )
    checks += 1
    print("[GREEN] documentation preserves TEST-only multi-step rotation boundary")

    assert checks == 6
    print("RUST-128 static policy: 6/6 checks passed")
