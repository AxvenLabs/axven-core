#!/usr/bin/env python3
"""RUST-066 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 94:
        raise SystemExit(
            "usage: rust_066_monitor_rotation_journal_observer_rotation_journal_monitor_selftest.py "
    ...
    if len(base) != 90:
        raise AssertionError("unexpected RUST-066 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[87], "final monitor-rotation-journal observer rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-066 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[89], "RUST-066 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-066 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-066 monitor subset count")
    print(
        "[GREEN] RUST-066 monitor availability: "
        "3/3 valid two-monitor subsets accepted"
    )

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path(
            "/tmp/axven-rust065-observed-fork-monitor-rotation-journal-observer-rotation-checkpoint.json"
        ),
        "RUST-065 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-066 signed observed same-parent fork evidence validated")
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-066 fail-closed case count: {cases}")
    print(
        "RUST-066 monitor-rotation-journal observer-rotation-journal monitor fail-closed contract: "
