#!/usr/bin/env python3
"""RUST-103 TEST-ONLY monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-103 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_103_rust102_checkpoint_monitor_rotation_fixture.py SOURCE_SHA")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust102-monitor-bundle.json",
        "RUST-103 predecessor monitor bundle",
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust102-observed-fork-monitor-bundle.json",
        "RUST-103 predecessor observed fork monitor bundle",
    )
    ...
    (OUT / "axven-rust103-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust103-monitor-set-rotation-auth.json").write_bytes(material_verify.canonical(auth))
    (OUT / "axven-rust103-successor-monitor-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust103-successor-fork-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-103 TEST-only monitor-set rotation fixture: GREEN")
