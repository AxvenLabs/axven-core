#!/usr/bin/env python3
"""RUST-093 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
OUT = Path("/tmp")
SEEDS = {
    monitor_verify.MONITOR_2_ID: "0b" * 32,
    monitor_verify.MONITOR_3_ID: "1b" * 32,
    rotation1_verify.M4_ID: "2b" * 32,
    rotation2_verify.M5_ID: "3b" * 32,
}
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-093 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(c not in "0123456789abcdef" for c in sys.argv[1]):
        raise SystemExit(
            "usage: rust_093_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_monitor_rotation_journal_monitor_rotation_journal_fixture.py SOURCE_SHA"
        )
    source_sha = sys.argv[1]

    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust089-final-monitor-rotation-checkpoint.json",
        "RUST-089 final monitor rotation checkpoint",
    )
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-093 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust090-monitor-bundle.json", "RUST-093 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust091-monitor-set-rotation.json", "RUST-093 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust091-monitor-set-rotation-auth.json", "RUST-093 first rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust091-successor-monitor-bundle.json", "RUST-093 first successor monitor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust092-second-monitor-set-rotation.json", "RUST-093 second monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust092-second-monitor-set-rotation-auth.json", "RUST-093 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust092-final-monitor-bundle.json", "RUST-093 final monitor bundle"
    )
    ...
    (OUT / "axven-rust093-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust093-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust093-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust093-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust093-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-093 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
