#!/usr/bin/env python3
"""RUST-055: TEST-ONLY journal-monitor-journal observer-set rotation verifier."""
...
OBSERVER_SET_SCHEMA = "axven-native-journal-monitor-journal-observer-set-v1"
ROTATION_SCHEMA = "axven-native-journal-monitor-journal-observer-set-rotation-v1"
ROTATION_AUTH_SCHEMA = "axven-native-journal-monitor-journal-observer-set-rotation-quorum-v1"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-journal-monitor-journal-observer-set-rotation.v1+json"
SUCCESSOR_BUNDLE_SCHEMA = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-bundle-v2"
SUCCESSOR_REPORT_SCHEMA = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-v2"
SUCCESSOR_STATEMENT_SCHEMA = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-statement-v2"
ROTATION_DOMAIN = b"AXVEN_NATIVE_JOURNAL_MONITOR_JOURNAL_OBSERVER_SET_ROTATION_V1\x00"
SUCCESSOR_DOMAIN = b"AXVEN_NATIVE_JOURNAL_MONITOR_ROTATION_JOURNAL_CHECKPOINT_OBSERVATION_V2\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
OLD_SET_SEQUENCE = 0
NEW_SET_SEQUENCE = 1
...
def validate_rotation(rotation: dict, old_bundle_raw: bytes, target: dict, source_sha: str) -> None:
    if (
        ...
        raise AssertionError("invalid journal-monitor-journal observer rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != NEW_SET_SEQUENCE:
        raise AssertionError("invalid journal-monitor-journal observer rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(old_observer_set())):
        raise AssertionError("journal-monitor-journal observer predecessor set mismatch")
    if rotation.get("to_set") != new_observer_set():
        raise AssertionError("unexpected successor journal-monitor-journal observer set")
    if rotation.get("revoked_observer_ids") != [REVOKED_OBSERVER_ID]:
        raise AssertionError("journal-monitor-journal observer revocation continuity mismatch")
    ...
    if rotation.get("production") is not False:
        raise AssertionError("production journal-monitor-journal observer rotation forbidden in RUST-055")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if (
        ...
        raise AssertionError("invalid journal-monitor-journal observer rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid journal-monitor-journal observer rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid journal-monitor-journal observer rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("journal-monitor-journal observer rotation authorization payload mismatch")
    observers = auth.get("observers")
    if not isinstance(observers, list) or not (THRESHOLD <= len(observers) <= len(OLD_PINNED_OBSERVERS)):
        raise AssertionError("invalid journal-monitor-journal observer rotation authorization size")
    if not all(isinstance(item, dict) and frozenset(item) == AUTH_OBSERVER_KEYS for item in observers):
        raise AssertionError("invalid journal-monitor-journal observer rotation authorization entry")
    ...
    if any(observer_id not in OLD_PINNED_OBSERVERS for observer_id in ids):
        raise AssertionError("unknown journal-monitor-journal observer rotation authorizer")
...
def validate_successor_report(report: dict, set_sha: str) -> dict:
    if not isinstance(report, dict) or frozenset(report) != SUCCESSOR_REPORT_KEYS:
        raise AssertionError("invalid successor journal-monitor-journal observer report fields")
    if report.get("schema") != SUCCESSOR_REPORT_SCHEMA or report.get("algorithm") != ALGORITHM:
        raise AssertionError("invalid successor journal-monitor-journal observer report envelope")
    statement = report.get("statement")
    if not isinstance(statement, dict) or frozenset(statement) != SUCCESSOR_STATEMENT_KEYS:
        raise AssertionError("invalid successor journal-monitor-journal observer statement fields")
    if statement.get("schema") != SUCCESSOR_STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError("invalid successor journal-monitor-journal observer statement boundary")
    observer_id = statement.get("observer_id")
    if not isinstance(observer_id, str) or observer_id not in NEW_PINNED_OBSERVERS:
        raise AssertionError("unknown successor journal-monitor-journal observer")
    if observer_id == REVOKED_OBSERVER_ID:
        raise AssertionError("revoked journal-monitor-journal observer resurrected")
    if (
        statement.get("observer_set_sequence") != NEW_SET_SEQUENCE
        or statement.get("observer_set_sha256") != set_sha
    ):
        raise AssertionError("successor journal-monitor-journal observer set epoch mismatch")
    material_verify.ed25519_verify(
...
def validate_successor_bundle(bundle: dict, target: dict) -> None:
    ...
    if (
        ...
        raise AssertionError("invalid successor journal-monitor-journal observation bundle fields")
    if (
        bundle.get("observer_set_sequence") != NEW_SET_SEQUENCE
        or bundle.get("observer_set_sha256") != set_sha
    ):
        raise AssertionError("successor journal-monitor-journal observation bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid successor journal-monitor-journal observation threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production successor journal-monitor-journal observation forbidden in RUST-055")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(NEW_PINNED_OBSERVERS)):
        raise AssertionError("invalid successor journal-monitor-journal observation report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("successor journal-monitor-journal observer ids must be unique and sorted")
    matching = 0
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("successor journal-monitor-journal observer source mismatch")
        same_epoch_parent = (
        ...
        if not exact:
            raise AssertionError(
                "successor journal-monitor-journal observer report does not match canonical checkpoint"
            )
        matching += 1
    if matching < THRESHOLD:
        raise AssertionError("successor journal-monitor-journal observer quorum below threshold")


def verify(*args) -> None:
    if len(args) != 62:
        raise AssertionError("unexpected RUST-055 verifier argument count")
    (
        *base_paths,
        old_bundle_path,
        rotation_path,
    ...
    if len(base_paths) != 56:
        raise AssertionError("unexpected RUST-055 base path count")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        old_bundle_path, "predecessor journal-monitor-journal observer bundle"
    )
    rotation_raw, rotation = floor_verify.load_canonical(
        rotation_path, "journal-monitor-journal observer rotation"
    )
    _, auth = floor_verify.load_canonical(
        auth_path, "journal-monitor-journal observer rotation authorization"
    )
    _, successor = floor_verify.load_canonical(
        successor_path, "successor journal-monitor-journal observer bundle"
    )
    ...
    print(
        "RUST-055 journal-monitor-journal observer set rotation: GREEN "
        f"source={expected_source_sha} revoked={REVOKED_OBSERVER_ID} successor={ids}"
    )


def main() -> None:
    if len(sys.argv) != 64 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_055_journal_monitor_journal_observer_set_rotation_verify.py verify "
