#!/usr/bin/env python3
"""SEC-102 rejects non-canonical fields and schemes inside P2P transaction inputs."""
...
def expect_rejected(name, session, message, expected_error):
    try:
        session.handle(message)
    except p2p.ProtocolError as exc:
        assert str(exc) == expected_error, exc
        print(f"[GREEN] {name}")
        return
...
def main():
    ...
    raw = canonical_block()
    raw["transactions"][0]["inputs"][0]["scheme"] = "future-scheme"
    expect_rejected(
