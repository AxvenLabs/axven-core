#!/usr/bin/env python3
"""RUST-115: TEST-ONLY monitor-set rotation for RUST-114 checkpoint monitors."""
...
MONITOR_SET_SCHEMA = "axven-native-rust115-monitor-set-v1"
ROTATION_SCHEMA = "axven-native-rust115-monitor-set-rotation-v1"
ROTATION_AUTH_SCHEMA = "axven-native-rust115-monitor-set-rotation-quorum-v1"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-rust115-monitor-set-rotation.v1+json"
SUCCESSOR_BUNDLE_SCHEMA = "axven-native-rust115-checkpoint-monitor-bundle-v2"
SUCCESSOR_REPORT_SCHEMA = "axven-native-rust115-checkpoint-monitor-report-v2"
SUCCESSOR_STATEMENT_SCHEMA = "axven-native-rust115-checkpoint-monitor-statement-v2"
...
M4_ID = "rust-115-test-only-monitor-rotation-journal-monitor-4-v1"
M4_PUBLIC_KEY = bytes.fromhex("91a28a0b74381593a4d9469579208926afc8ad82c8839b7644359b9eba9a4b3a")
...
def validate_rotation(rotation: dict, old_bundle_raw: bytes, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-115 monitor rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != NEW_SET_SEQUENCE:
        raise AssertionError("invalid RUST-115 monitor rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(old_monitor_set())):
        raise AssertionError("RUST-115 predecessor monitor set mismatch")
    if rotation.get("to_set") != new_monitor_set():
        raise AssertionError("unexpected RUST-115 successor monitor set")
    if rotation.get("revoked_monitor_ids") != [REVOKED_MONITOR_ID]:
        raise AssertionError("RUST-115 monitor revocation continuity mismatch")
    if rotation.get("predecessor_monitor_bundle_sha256") != sha256(old_bundle_raw):
        raise AssertionError("RUST-115 predecessor monitor bundle mismatch")
    for key in monitor_verify.TARGET_KEYS:
        if rotation.get(key) != target[key]:
            raise AssertionError(f"RUST-115 rotation target binding mismatch: {key}")
    if rotation.get("production") is not False:
        raise AssertionError("production monitor rotation forbidden in RUST-115")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-115 rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid RUST-115 rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-115 rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("RUST-115 rotation authorization payload mismatch")
    monitors = auth.get("monitors")
    if not isinstance(monitors, list) or not (THRESHOLD <= len(monitors) <= len(OLD_PINNED_MONITORS)):
        raise AssertionError("invalid RUST-115 rotation authorization size")
    if not all(isinstance(item, dict) and frozenset(item) == AUTH_MONITOR_KEYS for item in monitors):
        raise AssertionError("invalid RUST-115 rotation authorization entry")
    ids = [item["monitor_id"] for item in monitors]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-115 rotation authorizer ids must be unique and sorted")
    if any(monitor_id not in OLD_PINNED_MONITORS for monitor_id in ids):
        raise AssertionError("unknown RUST-115 rotation authorizer")
...
def validate_successor_bundle(bundle: dict, target: dict) -> None:
    ...
    if (
        ...
        raise AssertionError("invalid RUST-115 successor monitor bundle fields")
    if bundle.get("monitor_set_sequence") != NEW_SET_SEQUENCE or bundle.get("monitor_set_sha256") != set_sha:
        raise AssertionError("RUST-115 successor monitor bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-115 successor monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production successor monitoring forbidden in RUST-115")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(NEW_PINNED_MONITORS)):
        raise AssertionError("invalid RUST-115 successor monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-115 successor monitor ids must be unique and sorted")
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-115 successor monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed RUST-115 successor same-parent checkpoint fork")
        if not exact:
            raise AssertionError("RUST-115 successor monitor report does not match canonical checkpoint")


def verify(*args) -> None:
    if len(args) != 227:
        raise AssertionError("unexpected RUST-115 verifier argument count")
    *base_paths, old_bundle_path, rotation_path, auth_path, successor_path, expected_source_sha, required_floor_text = args
    if len(base_paths) != 221:
        raise AssertionError("unexpected RUST-115 base path count")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(old_bundle_path, "RUST-115 predecessor monitor bundle")
    rotation_raw, rotation = floor_verify.load_canonical(rotation_path, "RUST-115 monitor-set rotation")
    _, auth = floor_verify.load_canonical(auth_path, "RUST-115 monitor-set rotation authorization")
    _, successor = floor_verify.load_canonical(successor_path, "RUST-115 successor monitor bundle")
    ...
    print(
        "RUST-115 RUST-101 journal checkpoint monitor-set rotation: GREEN "
        f"source={expected_source_sha} revoked={REVOKED_MONITOR_ID} successor={ids}"
    )


def main() -> None:
    if len(sys.argv) != 229 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_115_rust102_checkpoint_monitor_rotation_verify.py verify "
