#!/usr/bin/env python3
"""RUST-086 detached observer availability and fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 149:
        raise SystemExit(
            "usage: rust_086_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_selftest.py "
    ...
    *base_paths, bundle_path, fork_bundle_path = paths
    ...
    _, bundle = floor_verify.load_canonical(
        bundle_path, "RUST-086 journal checkpoint observer bundle"
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print(
            "[GREEN] RUST-086 journal checkpoint observer availability: "
    ...
    if cases != 25:
        raise AssertionError(f"unexpected RUST-086 selftest case count: {cases}")
    print(
        "RUST-086 journal checkpoint observation fail-closed contract: "
