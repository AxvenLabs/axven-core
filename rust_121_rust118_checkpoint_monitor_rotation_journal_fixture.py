#!/usr/bin/env python3
"""RUST-121 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
OUT = Path("/tmp")
SEEDS = {
    monitor_verify.MONITOR_2_ID: "3d" * 32,
    monitor_verify.MONITOR_3_ID: "4d" * 32,
    rotation1_verify.M4_ID: "5d" * 32,
    rotation2_verify.M5_ID: "6d" * 32,
}
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-121 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_121_rust118_checkpoint_monitor_rotation_journal_fixture.py SOURCE_SHA")
    source_sha = sys.argv[1]

    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust117-final-monitor-rotation-checkpoint.json",
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-121 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust118-monitor-bundle.json", "RUST-121 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust119-monitor-set-rotation.json", "RUST-121 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust119-monitor-set-rotation-auth.json", "RUST-121 first rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust119-successor-monitor-bundle.json", "RUST-121 first successor monitor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust120-second-monitor-set-rotation.json", "RUST-121 second monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust120-second-monitor-set-rotation-auth.json", "RUST-121 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust120-final-monitor-bundle.json", "RUST-121 final monitor bundle"
    )
    ...
    (OUT / "axven-rust121-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust121-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust121-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust121-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust121-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-121 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
