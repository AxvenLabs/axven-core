# RUST-128 — TEST-ONLY second RUST-126 checkpoint monitor-set rotation

RUST-128 composes the exact reviewed RUST-127 first monitor-set rotation and performs the second TEST-only rotation for the RUST-126 checkpoint-monitor family.

## Contract

- Monitor set rotates from M2/M3/M4 to M3/M4/M5 with a 2-of-3 threshold.
- M1 remains revoked from RUST-127 and M2 is newly revoked; cumulative revocation continuity is explicit.
- The second rotation binds the exact RUST-127 rotation, authorization and successor-bundle SHA-256 values plus all 12 inherited RUST-125 checkpoint target fields.
...
- CI keeps generated evidence read-only with chmod 0444, uses a fixed 261-path manifest and an 101-file verifier-only detached consumer under `env -i` and `/usr/bin/python3 -S`.
