#!/usr/bin/env python3
"""RUST-085 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
OUT = Path("/tmp")
SEEDS = {
    monitor_verify.MONITOR_2_ID: "69" * 32,
    monitor_verify.MONITOR_3_ID: "79" * 32,
    rotation1_verify.M4_ID: "89" * 32,
    rotation2_verify.M5_ID: "99" * 32,
}
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-085 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit(
            "usage: rust_085_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_fixture.py SOURCE_SHA"
        )
    source_sha = sys.argv[1]

    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust081-final-observer-rotation-checkpoint.json",
        "RUST-081 final observer rotation checkpoint",
    )
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-085 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust082-monitor-bundle.json", "RUST-085 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust083-monitor-set-rotation.json", "RUST-085 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust083-monitor-set-rotation-auth.json", "RUST-085 first rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust083-successor-monitor-bundle.json", "RUST-085 first successor monitor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust084-second-monitor-set-rotation.json", "RUST-085 second monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust084-second-monitor-set-rotation-auth.json", "RUST-085 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust084-final-monitor-bundle.json", "RUST-085 final monitor bundle"
    )
    ...
    (OUT / "axven-rust085-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust085-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust085-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust085-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust085-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-085 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
