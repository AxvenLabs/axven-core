#!/usr/bin/env python3
"""RUST-081 detached availability and fail-closed observer rotation journal selftest."""
...
def main() -> None:
    if len(sys.argv) != 137:
        raise SystemExit(
            "usage: rust_081_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_selftest.py "
    ...
    if len(base) != 133:
        raise AssertionError("unexpected RUST-081 selftest base path count")
    ...
    target = observer_verify.canonical_target(
        observed_checkpoint_raw, observed_checkpoint, source_sha
    )
    old_bundle_raw, _ = floor_verify.load_canonical(base[122], "RUST-081 old observer bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base[123], "RUST-081 first rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base[124], "RUST-081 first auth")
    first_successor_raw, _ = floor_verify.load_canonical(base[125], "RUST-081 first successor")
    second_rotation_raw, _ = floor_verify.load_canonical(base[126], "RUST-081 second rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base[127], "RUST-081 second auth")
    final_bundle_raw, _ = floor_verify.load_canonical(base[128], "RUST-081 final bundle")
    ...
    for subset in itertools.combinations(prefix_checkpoint["observers"], journal_verify.THRESHOLD):
        ...
        prefix_ok += 1
    if prefix_ok != 3:
        raise AssertionError("unexpected RUST-081 prefix subset count")
    print("[GREEN] RUST-081 prefix checkpoint availability: 3/3 valid two-observer subsets accepted")
    ...
    for subset in itertools.combinations(final_checkpoint["observers"], journal_verify.THRESHOLD):
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-081 final subset count")
    print("[GREEN] RUST-081 final checkpoint availability: 3/3 valid two-observer subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("prefix-genesis-set-rewrite", lambda: run_with(129, write("a.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_sha256"] = "0" * 64
        fail("prefix-first-rotation-digest", lambda: run_with(129, write("b.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_auth_sha256"] = "0" * 64
        fail("prefix-first-auth-digest", lambda: run_with(129, write("c.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["observed_checkpoint_sha256"] = "0" * 64
        fail("prefix-observed-checkpoint", lambda: run_with(129, write("d.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["observed_checkpoint_statement_sha256"] = "0" * 64
        fail("prefix-observed-statement", lambda: run_with(129, write("e.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["observed_target_sha256"] = "0" * 64
        fail("prefix-observed-target", lambda: run_with(129, write("f.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["activation_source_commit"] = "0" * 40
        fail("prefix-source", lambda: run_with(129, write("g.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["production"] = True
        fail("prefix-production", lambda: run_with(129, write("h.json", value))); cases += 1

        value = copy.deepcopy(final_journal); value["entries"] = value["entries"][:2]
        fail("final-entry-truncation", lambda: run_with(131, write("i.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][0]["observation_bundle_sha256"] = "0" * 64
        fail("final-prefix-rewrite", lambda: run_with(131, write("j.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["sequence"] = 1
        fail("final-sequence-rollback", lambda: run_with(131, write("k.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["predecessor_entry_sha256"] = "0" * 64
        fail("final-predecessor-entry", lambda: run_with(131, write("l.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_sha256"] = "0" * 64
        fail("final-rotation-digest", lambda: run_with(131, write("m.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_auth_sha256"] = "0" * 64
        fail("final-rotation-auth-digest", lambda: run_with(131, write("n.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["observation_bundle_sha256"] = "0" * 64
        fail("final-observation-bundle", lambda: run_with(131, write("o.json", value))); cases += 1
        ...
        fail("final-revocation-omission", lambda: run_with(131, write("p.json", value))); cases += 1

        value = copy.deepcopy(prefix_checkpoint); value["threshold"] = 1
        fail("prefix-threshold", lambda: run_with(130, write("q.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["observers"] = value["observers"][:1]
        fail("prefix-below-threshold", lambda: run_with(130, write("r.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["observers"] = [value["observers"][0], copy.deepcopy(value["observers"][0])]
        fail("prefix-duplicate", lambda: run_with(130, write("s.json", value))); cases += 1
        ...
        fail("prefix-signature", lambda: run_with(130, write("t.json", value))); cases += 1

        value = copy.deepcopy(final_checkpoint); value["statement"]["previous_checkpoint_sha256"] = "0" * 64
        fail("final-previous-checkpoint", lambda: run_with(132, write("u.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["head_entry_sha256"] = "0" * 64
        fail("final-head-entry", lambda: run_with(132, write("v.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observed_checkpoint_sha256"] = "0" * 64
        fail("final-observed-checkpoint", lambda: run_with(132, write("w.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observed_checkpoint_statement_sha256"] = "0" * 64
        fail("final-observed-statement", lambda: run_with(132, write("x.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observed_target_sha256"] = "0" * 64
        fail("final-observed-target", lambda: run_with(132, write("y.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["activation_source_commit"] = "0" * 40
        fail("final-source", lambda: run_with(132, write("z.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["production"] = True
        fail("final-production", lambda: run_with(132, write("aa.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observer_set_sequence"] = 1
        fail("final-observer-set-sequence", lambda: run_with(132, write("ab.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observer_set_sha256"] = "0" * 64
        fail("final-observer-set-digest", lambda: run_with(132, write("ac.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["threshold"] = 1
        fail("final-threshold", lambda: run_with(132, write("ad.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["observers"] = value["observers"][:1]
        fail("final-below-threshold", lambda: run_with(132, write("ae.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint)
        sig = bytearray(material_verify.decode_signature(value["observers"][-1]["signature"])); sig[-1] ^= 1
        value["observers"][-1]["signature"] = base64.b64encode(bytes(sig)).decode("ascii")
        fail("final-signature", lambda: run_with(132, write("af.json", value))); cases += 1

        noncanonical_journal = root / "noncanonical-journal.json"
        noncanonical_journal.write_text(json.dumps(final_journal, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-journal", lambda: run_with(131, noncanonical_journal)); cases += 1
        ...
        fail("noncanonical-final-checkpoint", lambda: run_with(132, noncanonical_checkpoint)); cases += 1
    ...
    if cases != 35:
        raise AssertionError(f"unexpected RUST-081 selftest case count: {cases}")
    print("RUST-081 observer rotation journal fail-closed contract: 35/35 expected cases passed")
