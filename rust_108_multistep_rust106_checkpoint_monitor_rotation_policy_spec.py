#!/usr/bin/env python3
"""RUST-108 static policy for TEST-ONLY second RUST-106 checkpoint monitor rotation."""
...
DOC = ROOT / "docs/history/rust/RUST_108.md"
VERIFY = ROOT / "rust_108_multistep_rust106_checkpoint_monitor_rotation_verify.py"
FIXTURE = ROOT / "rust_108_multistep_rust106_checkpoint_monitor_rotation_fixture.py"
SELFTEST = ROOT / "rust_108_multistep_rust106_checkpoint_monitor_rotation_selftest.py"
WORKFLOW = ROOT / ".github/workflows/native-rust108-multistep-checkpoint-monitor-rotation.yml"
BASE = ROOT / "rust_107_rust106_checkpoint_monitor_rotation_verify.py"
PREDECESSOR_WORKFLOW = ROOT / ".github/workflows/native-rust107-rust106-checkpoint-monitor-rotation.yml"
EXPECTED_RUST107_GIT_BLOB = "574fd3ce8187b7eadfb7338813f47afe636c4d77"
EXPECTED_RUST107_WORKFLOW_GIT_BLOB = "df76aab8ce36c78e8ab671e32804671bef73faec"
...
def main() -> None:
    ...
    assert blob(BASE.read_bytes()) == EXPECTED_RUST107_GIT_BLOB
    assert blob(PREDECESSOR_WORKFLOW.read_bytes()) == EXPECTED_RUST107_WORKFLOW_GIT_BLOB
    require(
        verify,
        ("import rust_107_rust106_checkpoint_monitor_rotation_verify as rotation1_verify",),
        "RUST-108 verifier composition",
    )
    checks += 1
    print("[GREEN] exact reviewed RUST-107 verifier and workflow are pinned")
    ...
    checks += 1
    print("[GREEN] detached RUST-108 verifier/selftest have no signing or network capability")
    ...
    require(
        ...
        (
            ...
            "final_monitor_set_sha256", "observed RUST-108 final same-parent checkpoint fork",
            "*monitor_verify.TARGET_KEYS",
            "base_paths[198]", "base_paths[200]", "base_paths[201]", "base_paths[202]",
            "path_args[203:206]",
        ),
        "RUST-108 verifier",
    )
    ...
    require(
        ...
        (
            ...
            "Ed25519PrivateKey", "RUST-108 TEST-only monitor public-key pin mismatch",
        ),
        "RUST-108 producer fixture",
    )
    require(
        selftest,
        (
    ...
    require(
        ...
        (
            ...
            'python-version: "3.13.15"', "chmod 0444", "axven-rust108",
            "env -i HOME=/tmp PATH=/usr/bin:/bin", "/usr/bin/python3 -S",
            '"rust_*.py"', '"RUST_*.md"', 'printf -v n3 "%03d" "$n"',
            'test ! -e "$c/rust_108_multistep_rust106_checkpoint_monitor_rotation_fixture.py"',
            'test "$(find "$c" -maxdepth 1 -type f | wc -l)" -eq 81',
            "expected 203 RUST-107 paths", "expected 206 RUST-108 paths",
            "axven-rust108-final-monitor-bundle.json",
        ),
        "RUST-108 workflow",
    )
    for forbidden in (
    ...
    require(
        ...
        (
            ...
            "fixed 206-path manifest", "81-file verifier-only detached consumer",
            "Production consensus remains Python-authoritative.",
        ),
        "RUST-108 documentation",
    )
    checks += 1
    print("[GREEN] documentation preserves TEST-only multi-step rotation boundary")

    assert checks == 6
    print("RUST-108 static policy: 6/6 checks passed")
