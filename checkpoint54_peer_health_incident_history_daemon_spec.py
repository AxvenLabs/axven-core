...
root=tempfile.mkdtemp(prefix="axven_checkpoint54_daemon_")
...
try:
    ...
    ok(
        "new daemon peer exposes empty incident history",
        initial["health_incident_history"] == [],
    )

    offline=wait_health_state(a_rpc,"offline",deadline=8)

    ok(
        "active offline incident does not enter completed history",
        offline["health_incident_history"] == [],
    )
    ...
    ok(
        "active backoff incident keeps completed history empty",
        backoff["health_incident_history"] == [],
    )

    ok(
        "backoff preserves active incident start",
    ...
    history=recovered["health_incident_history"]

    ok(
        "recovery appends first completed incident",
        len(history) == 1,
    )
    ...
    ok(
        "completed history entry matches last incident",
        incident == recovered["last_health_incident"],
    )

    ok(
        "completed history records opening state",
        incident["from_state"] == "never_connected",
    )

    ok(
        "completed history records last unhealthy state",
        incident["last_unhealthy_state"] == "backoff",
    )

    ok(
        "completed history records recovery state",
        incident["recovered_to"] == "recovered",
    )

    ok(
        "completed history preserves incident start",
        incident["started_at"] == started_at,
    )

    ok(
        "completed history end matches recovery transition",
        incident["ended_at"]
        == recovered["last_health_transition_at"],
    )

    ok(
        "completed history counts unhealthy transitions",
        incident["unhealthy_transitions"] == 2,
    )

    ok(
        "completed incident history schema is stable",
        set(incident) == {
            "from_state",
    ...
    ok(
        "healthy sync does not duplicate completed incident",
        peer_state(a_rpc)["health_incident_history"] == stable_history,
    )

    ok(
        "transition history still records full lifecycle",
        [
            entry["to_state"]
...
print(
    f"Checkpoint 54 daemon health incident history: "
