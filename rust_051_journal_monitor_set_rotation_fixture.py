#!/usr/bin/env python3
"""RUST-051 TEST-ONLY journal-monitor rotation producer. Private seeds remain producer-side."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-051 TEST-only journal-monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(c not in "0123456789abcdef" for c in sys.argv[1]):
        raise SystemExit("usage: rust_051_journal_monitor_set_rotation_fixture.py SOURCE_SHA")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust050-monitor-bundle.json", "predecessor journal-monitor bundle"
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust050-observed-fork-monitor-bundle.json",
    ...
    fork_statement = next(
        ...
        if report["statement"]["journal_observer_checkpoint_sha256"]
        != target["journal_observer_checkpoint_sha256"]
    )
    fork_target = {key: copy.deepcopy(fork_statement[key]) for key in target}
    fork_report = successor_report(rotation_verify.JM4_ID, fork_target, set_sha)

    (OUT / "axven-rust051-journal-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust051-journal-monitor-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust051-successor-journal-monitor-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust051-successor-fork-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-051 TEST-only journal-monitor set rotation fixture: GREEN")
