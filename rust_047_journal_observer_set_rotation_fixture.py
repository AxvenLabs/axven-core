#!/usr/bin/env python3
"""RUST-047 TEST-ONLY journal-observer rotation producer. Private seeds remain producer-side."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-047 TEST-only journal-observer public-key pin mismatch")
    return private
...
def auth_rows(rotation_raw: bytes) -> list[dict]:
    ...
    for observer_id in sorted(rotation_verify.OLD_PINNED_OBSERVERS):
        private = private_for(observer_id, rotation_verify.OLD_PINNED_OBSERVERS)
        rows.append({
...
def successor_report(observer_id: str, target: dict, set_sha: str) -> dict:
    ...
    return {
        ...
        "statement": statement,
        "signature": base64.b64encode(private.sign(rotation_verify.successor_message(statement))).decode("ascii"),
    }
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(c not in "0123456789abcdef" for c in sys.argv[1]):
        raise SystemExit("usage: rust_047_journal_observer_set_rotation_fixture.py SOURCE_SHA")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust046-journal-observer-bundle.json", "predecessor journal-observer bundle"
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust046-observed-fork-bundle.json", "predecessor observed fork bundle"
    )
    ...
    rotation = {
        ...
        "from_set_sha256": rotation_verify.sha256(material_verify.canonical(rotation_verify.old_observer_set())),
    ...
    set_sha = rotation_verify.sha256(material_verify.canonical(rotation_verify.new_observer_set()))
    reports = [
    ...
    fork_report = successor_report(rotation_verify.J4_ID, fork_target, set_sha)

    (OUT / "axven-rust047-journal-observer-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust047-journal-observer-set-rotation-auth.json").write_bytes(material_verify.canonical(auth))
    (OUT / "axven-rust047-successor-journal-observer-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust047-successor-fork-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-047 TEST-only journal-observer set rotation fixture: GREEN")
