#!/usr/bin/env python3
"""RUST-066: TEST-ONLY monitor-rotation-journal observer-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-observer-set-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-observer-set-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-observer-rotation-journal-monitor-set-rotation-journal-observer-set-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_OBSERVER_ROTATION_JOURNAL_MONITOR_SET_ROTATION_JOURNAL_OBSERVER_SET_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

MONITOR_1_ID = "rust-066-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-1-v1"
MONITOR_2_ID = "rust-066-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-2-v1"
MONITOR_3_ID = "rust-066-test-only-monitor-rotation-journal-observer-rotation-journal-monitor-3-v1"
MONITOR_1_PUBLIC_KEY = bytes.fromhex("4745d19c66ec9179fa25097056409ef16967b6a1a74bfd8f7afd528f59630615")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("a11dbe28d6b2c2d80c4f6cb2b0bca67baa2f8c9ceb81c0c94ac47e3cf0697035")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("332ebe8d27cb7323b3a401c1c13b5dd64bccc0e10ecda1c2b5d11a03779a85e5")
PINNED_MONITORS = {
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-066 canonical checkpoint statement fields")
    return {
...
def validate_report_envelope(report: dict) -> dict:
    if not isinstance(report, dict) or frozenset(report) != REPORT_KEYS:
        raise AssertionError("invalid monitor-rotation-journal observer-rotation-journal monitor report fields")
    if report.get("schema") != REPORT_SCHEMA or report.get("algorithm") != ALGORITHM:
        raise AssertionError("invalid monitor-rotation-journal observer-rotation-journal monitor report envelope")
    statement = report.get("statement")
    if not isinstance(statement, dict) or frozenset(statement) != STATEMENT_KEYS:
        raise AssertionError("invalid monitor-rotation-journal observer-rotation-journal monitor statement fields")
    if statement.get("schema") != STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError("invalid monitor-rotation-journal observer-rotation-journal monitor statement boundary")
    monitor_id = statement.get("monitor_id")
    if not isinstance(monitor_id, str) or monitor_id not in PINNED_MONITORS:
        raise AssertionError("unknown monitor-rotation-journal observer-rotation-journal checkpoint monitor")
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid monitor-rotation-journal observer-rotation-journal monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid monitor-rotation-journal observer-rotation-journal monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError(
            "production monitor-rotation-journal observer-rotation-journal monitoring forbidden in RUST-066"
        )
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid monitor-rotation-journal observer-rotation-journal monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("monitor-rotation-journal observer-rotation-journal monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("monitor-rotation-journal observer-rotation-journal monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError(
                "observed monitor same-parent monitor-rotation-journal observer-rotation-journal checkpoint fork"
            )
        if not exact:
            raise AssertionError(
                "monitor report does not match canonical monitor-rotation-journal observer-rotation-journal checkpoint"
            )
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError(
            "observed monitor-rotation-journal observer-rotation-journal checkpoint is not a distinct same-parent fork"
        )
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-066 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-066 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 92:
        raise AssertionError("unexpected RUST-066 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 90:
        raise AssertionError("unexpected RUST-066 path count")
    base_paths = path_args[:89]
    monitor_bundle_path = path_args[89]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[87], "final monitor-rotation-journal observer rotation journal"
    )
    ...
    statement = journal_verify.validate_checkpoint_envelope(
        ...
        final_journal_raw,
        rotation2_verify.FINAL_PINNED_OBSERVERS,
        "RUST-066 canonical final",
    )
    target = checkpoint_target(final_checkpoint_raw, statement)
    if target["activation_source_commit"] != expected_source_sha:
        raise AssertionError("RUST-066 monitor-rotation-journal observer-rotation-journal checkpoint source mismatch")

    _, bundle = floor_verify.load_canonical(
        monitor_bundle_path, "monitor-rotation-journal observer-rotation-journal monitor bundle"
    )
    validate_bundle(bundle, target)
    ids = ",".join(report["statement"]["monitor_id"] for report in bundle["reports"])
    print(
        "RUST-066 monitor-rotation-journal observer-rotation-journal checkpoint monitoring: GREEN "
...
def main() -> None:
    if len(sys.argv) != 94 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_066_monitor_rotation_journal_observer_rotation_journal_monitor_verify.py verify "
