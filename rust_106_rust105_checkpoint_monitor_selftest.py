#!/usr/bin/env python3
"""RUST-106 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 204:
        raise SystemExit(
            "usage: rust_106_rust105_checkpoint_monitor_selftest.py "
    ...
    if len(base) != 200:
        raise AssertionError("unexpected RUST-106 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[197], "RUST-105 final monitor rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-106 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[199], "RUST-106 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-106 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-106 monitor subset count")
    print("[GREEN] RUST-106 monitor availability: 3/3 valid two-monitor subsets accepted")

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path("/tmp/axven-rust105-observed-fork-monitor-rotation-checkpoint.json"),
        "RUST-105 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-106 signed observed same-parent fork evidence validated")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        paths = list(base); paths[199] = noncanonical
        fail(
            "noncanonical-monitor-bundle",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[199] = base[198]
        fail(
            "rust105-checkpoint-replay",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[199] = fork_bundle_path
        fail(
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-106 fail-closed case count: {cases}")
    print("RUST-106 RUST-105 journal checkpoint monitor fail-closed contract: 27/27 expected cases passed")
