#!/usr/bin/env python3
"""RUST-072 TEST-ONLY second observer-set rotation producer."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-072 TEST-only observer public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_072_multistep_monitor_rotation_journal_observer_set_rotation_fixture.py SOURCE_SHA"
        )
    ...
    fork_target = {
        key: copy.deepcopy(fork_statement[key]) for key in observer_verify.TARGET_KEYS
    }
    fork_report = final_report(rotation2_verify.O5_ID, fork_target, set_sha)

    (OUT / "axven-rust072-second-observer-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust072-second-observer-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust072-final-observer-bundle.json").write_bytes(
        material_verify.canonical(final_bundle(reports, set_sha))
    )
    (OUT / "axven-rust072-final-fork-bundle.json").write_bytes(
        material_verify.canonical(
            final_bundle([reports[0], reports[1], fork_report], set_sha)
        )
    )
    print("RUST-072 TEST-only second observer-set rotation fixture: GREEN")
