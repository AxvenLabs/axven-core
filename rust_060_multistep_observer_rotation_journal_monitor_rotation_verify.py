#!/usr/bin/env python3
"""RUST-060: TEST-ONLY multi-step observer-rotation-journal monitor rotation verifier."""
...
ROTATION_AUTH_SCHEMA = "axven-native-journal-monitor-journal-observer-rotation-journal-monitor-set-rotation-quorum-v2"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-journal-monitor-journal-observer-rotation-journal-monitor-set-rotation.v2+json"
FINAL_BUNDLE_SCHEMA = "axven-native-journal-monitor-journal-observer-rotation-journal-checkpoint-monitor-bundle-v3"
FINAL_REPORT_SCHEMA = "axven-native-journal-monitor-journal-observer-rotation-journal-checkpoint-monitor-report-v3"
FINAL_STATEMENT_SCHEMA = "axven-native-journal-monitor-journal-observer-rotation-journal-checkpoint-monitor-statement-v3"
ROTATION_DOMAIN = b"AXVEN_NATIVE_OBSERVER_ROTATION_JOURNAL_MONITOR_SET_ROTATION_V2\x00"
FINAL_DOMAIN = b"AXVEN_NATIVE_OBSERVER_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V3\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
def validate_rotation(
    ...
    if not isinstance(rotation, dict) or frozenset(rotation) != ROTATION_KEYS or rotation.get("schema") != ROTATION_SCHEMA:
        raise AssertionError("invalid second observer-rotation-journal monitor rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != FINAL_SET_SEQUENCE:
        raise AssertionError("invalid second monitor rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(rotation1_verify.new_monitor_set())):
        raise AssertionError("second monitor rotation predecessor set mismatch")
    if rotation.get("to_set") != final_monitor_set():
        raise AssertionError("unexpected final observer-rotation-journal monitor set")
    if rotation.get("cumulative_revoked_monitor_ids") != CUMULATIVE_REVOKED_MONITOR_IDS:
        raise AssertionError("cumulative observer-rotation-journal monitor revocation mismatch")
    ...
    for key in (
        ...
        if rotation.get(key) != target[key]:
            raise AssertionError(f"second rotation inherited checkpoint binding mismatch: {key}")
    ...
    if rotation.get("production") is not False:
        raise AssertionError("production second monitor rotation forbidden in RUST-060")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("second monitor rotation ids must be unique and sorted")
...
def validate_final_report(report: dict, set_sha: str) -> dict:
    ...
    if not isinstance(monitor_id, str) or monitor_id not in FINAL_PINNED_MONITORS:
        raise AssertionError("unknown final observer-rotation-journal monitor")
    if monitor_id in CUMULATIVE_REVOKED_MONITOR_IDS:
        raise AssertionError("revoked monitor resurrected in final set")
...
def validate_final_bundle(bundle: dict, target: dict) -> None:
    ...
    if not isinstance(bundle, dict) or frozenset(bundle) != FINAL_BUNDLE_KEYS or bundle.get("schema") != FINAL_BUNDLE_SCHEMA:
        raise AssertionError("invalid final observer-rotation-journal monitor bundle fields")
    ...
    if bundle.get("production") is not False:
        raise AssertionError("production final monitoring forbidden in RUST-060")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("final monitor ids must be unique and sorted")
    for statement in statements:
        ...
        if not exact:
            raise AssertionError("final monitor report does not match canonical observer-rotation-journal checkpoint")


def verify(*args) -> None:
    if len(args) != 76:
        raise AssertionError("unexpected RUST-060 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 74:
        raise AssertionError("unexpected RUST-060 path count")

    base_paths = path_args[:67]
    old_bundle_path, first_rotation_path, first_auth_path, first_successor_path = path_args[67:71]
    second_rotation_path, second_auth_path, final_bundle_path = path_args[71:74]
    ...
    first_rotation_raw, _ = floor_verify.load_canonical(first_rotation_path, "first monitor rotation")
    first_auth_raw, _ = floor_verify.load_canonical(first_auth_path, "first monitor rotation authorization")
    first_successor_raw, _ = floor_verify.load_canonical(first_successor_path, "first successor monitor bundle")
    second_rotation_raw, second_rotation = floor_verify.load_canonical(second_rotation_path, "second monitor rotation")
    _, second_auth = floor_verify.load_canonical(second_auth_path, "second monitor rotation authorization")
    _, final_bundle = floor_verify.load_canonical(final_bundle_path, "final monitor bundle")
    ...
    print(
        "RUST-060 multi-step observer-rotation-journal monitor rotation: GREEN "
        f"source={expected_source_sha} sequence=2 revoked={','.join(CUMULATIVE_REVOKED_MONITOR_IDS)} final={ids}"
    )


def main() -> None:
    if len(sys.argv) != 78 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_060_multistep_observer_rotation_journal_monitor_rotation_verify.py verify "
