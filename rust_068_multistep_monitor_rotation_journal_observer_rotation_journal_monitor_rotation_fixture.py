#!/usr/bin/env python3
"""RUST-068 TEST-ONLY second monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-068 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit(
            "usage: rust_068_multistep_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_fixture.py SOURCE_SHA"
        )
    ...
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust067-monitor-set-rotation.json", "RUST-068 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust067-monitor-set-rotation-auth.json",
        "RUST-068 first monitor rotation authorization",
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust067-successor-monitor-bundle.json",
        "RUST-068 first successor monitor bundle",
    )
    _, predecessor_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust067-successor-fork-bundle.json",
        "RUST-068 predecessor fork monitor bundle",
    )
    ...
    (OUT / "axven-rust068-second-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust068-second-monitor-set-rotation-auth.json").write_bytes(
        material_verify.canonical(auth)
    )
    (OUT / "axven-rust068-final-monitor-bundle.json").write_bytes(
        material_verify.canonical(final_bundle(reports, set_sha))
    )
    (OUT / "axven-rust068-final-fork-bundle.json").write_bytes(
        material_verify.canonical(final_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-068 TEST-only second monitor-set rotation fixture: GREEN")
