#!/usr/bin/env python3
"""RUST-050 detached availability/fail-closed journal-monitor contract."""
...
def expect_reject(label: str, paths: list[Path], source: str, floor: str, mutate) -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        _, value = floor_verify.load_canonical(paths[-1], "journal-monitor bundle")
...
def main() -> None:
    if len(sys.argv) != 51:
        raise SystemExit(
            "usage: rust_050_journal_observer_journal_monitor_selftest.py "
            "... MONITOR_BUNDLE FORK_MONITOR_BUNDLE FORK_CHECKPOINT SOURCE_SHA REQUIRED_FLOOR"
        )
    source = sys.argv[-2]
    floor = sys.argv[-1]
    all_paths = [Path(value) for value in sys.argv[1:-2]]
    base = all_paths[:45]
    canonical_bundle_path = all_paths[45]
    fork_bundle_path = all_paths[46]
    fork_checkpoint_path = all_paths[47]
    verify_paths = base + [canonical_bundle_path]

    invoke(verify_paths, source, floor)
    _, canonical_bundle = floor_verify.load_canonical(canonical_bundle_path, "journal-monitor bundle")
    accepted = 0
    for rows in itertools.combinations(canonical_bundle["reports"], monitor_verify.THRESHOLD):
        with tempfile.TemporaryDirectory() as td:
            ...
            accepted += 1
    if accepted != 3:
        raise AssertionError("unexpected journal-monitor subset count")
    print("[GREEN] RUST-050 journal-monitor availability: 3/3 valid two-monitor subsets accepted")
    ...
    mutations = [
        ...
        ("checkpoint-digest", lambda b: b["reports"][0]["statement"].__setitem__("journal_observer_checkpoint_sha256", "0" * 64)),
        ...
        ("entry-count", lambda b: b["reports"][0]["statement"].__setitem__("entry_count", 2)),
        ...
        ("monitor-journal-checkpoint", lambda b: b["reports"][0]["statement"].__setitem__("monitor_journal_checkpoint_sha256", "0" * 64)),
        ("monitor-journal-statement", lambda b: b["reports"][0]["statement"].__setitem__("monitor_journal_checkpoint_statement_sha256", "0" * 64)),
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            print("[GREEN] mutation rejected: noncanonical-journal-monitor-bundle")
        else:
            raise AssertionError("noncanonical journal-monitor bundle accepted")

    final_journal_raw, _ = floor_verify.load_canonical(base[43], "final journal-observer journal")
    ...
    canonical_statement = monitor_verify.journal_verify.validate_checkpoint_envelope(
        final_checkpoint,
        final_journal_raw,
        monitor_verify.rotation2_verify.FINAL_PINNED_OBSERVERS,
        "canonical monitored",
    )
    canonical_target = monitor_verify.checkpoint_target(final_checkpoint_raw, canonical_statement)
    fork_checkpoint_raw, fork_checkpoint_value = floor_verify.load_canonical(
        fork_checkpoint_path, "observed fork journal-observer checkpoint"
    )
    _, fork_bundle_value = floor_verify.load_canonical(
        fork_bundle_path, "observed fork journal-monitor bundle"
    )
    monitor_verify.validate_observed_fork_evidence(
        fork_bundle_value,
        canonical_target,
        fork_checkpoint_raw,
        fork_checkpoint_value,
        final_journal_raw,
    )
    print("[GREEN] valid signed RUST-049 fork is bound by a signed RUST-050 journal-monitor report")
    try:
        invoke(base + [fork_bundle_path], source, floor)
    except AssertionError as exc:
        if "observed journal-monitor same-parent journal-observer-journal fork" not in str(exc):
            raise
        print("[GREEN] mutation rejected: observed-valid-same-parent-journal-monitor-split-view")
    else:
        raise AssertionError("valid signed journal-monitor split view unexpectedly accepted")

    print(
        f"RUST-050 journal-observer-journal monitor fail-closed contract: "
        f"{len(mutations) + 2}/{len(mutations) + 2} expected cases passed"
    )
