#!/usr/bin/env python3
"""RUST-074 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 116:
        raise SystemExit(
            "usage: rust_074_monitor_rotation_journal_observer_rotation_journal_monitor_selftest.py "
    ...
    if len(base) != 112:
        raise AssertionError("unexpected RUST-074 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[109], "RUST-073 final observer rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-074 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[111], "RUST-074 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-074 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-074 monitor subset count")
    print("[GREEN] RUST-074 monitor availability: 3/3 valid two-monitor subsets accepted")

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path("/tmp/axven-rust073-observed-fork-observer-rotation-checkpoint.json"),
        "RUST-073 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-074 signed observed same-parent fork evidence validated")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        target_fields = [
            ("monitor_rotation_journal_observer_rotation_journal_checkpoint_sha256", "0" * 64),
            ("monitor_rotation_journal_observer_rotation_journal_checkpoint_statement_sha256", "1" * 64),
            ...
            ("monitor_rotation_journal_checkpoint_sha256", "6" * 64),
            ("monitor_rotation_journal_checkpoint_statement_sha256", "7" * 64),
        ...
        paths = list(base); paths[111] = noncanonical
        fail(
            "noncanonical-monitor-bundle",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[111] = base[110]
        fail(
            "rust073-checkpoint-replay",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[111] = fork_bundle_path
        fail(
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-074 fail-closed case count: {cases}")
    print("RUST-074 observer-rotation-journal monitor fail-closed contract: 27/27 expected cases passed")
