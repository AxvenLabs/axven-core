#!/usr/bin/env python3
"""RUST-057 TEST-ONLY observer rotation journal/checkpoint producer."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-057 TEST-only observer journal public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit("usage: rust_057_observer_rotation_journal_fixture.py SOURCE_SHA")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust054-journal-monitor-journal-observer-bundle.json", "old observer bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust055-journal-monitor-journal-observer-set-rotation.json", "first observer rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust055-journal-monitor-journal-observer-set-rotation-auth.json", "first observer auth"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust055-successor-observer-bundle.json", "first observer successor"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust056-second-journal-monitor-journal-observer-set-rotation.json", "second observer rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust056-second-journal-monitor-journal-observer-set-rotation-auth.json", "second observer auth"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust056-final-observer-bundle.json", "final observer bundle"
    )
    ...
    (OUT / "axven-rust057-prefix-observer-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust057-prefix-observer-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust057-final-observer-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust057-final-observer-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust057-observed-fork-observer-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-057 TEST-only observer rotation journal/checkpoint fixture: GREEN")
