#!/usr/bin/env python3
"""RUST-114 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 226:
        raise SystemExit(
            "usage: rust_114_rust113_checkpoint_monitor_selftest.py "
    ...
    if len(base) != 222:
        raise AssertionError("unexpected RUST-114 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[219], "RUST-113 final monitor rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-114 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[221], "RUST-114 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-114 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-114 monitor subset count")
    print("[GREEN] RUST-114 monitor availability: 3/3 valid two-monitor subsets accepted")

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path("/tmp/axven-rust113-observed-fork-monitor-rotation-checkpoint.json"),
        "RUST-113 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-114 signed observed same-parent fork evidence validated")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        paths = list(base); paths[221] = noncanonical
        fail(
            "noncanonical-monitor-bundle",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[221] = base[220]
        fail(
            "rust113-checkpoint-replay",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[221] = fork_bundle_path
        fail(
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-114 fail-closed case count: {cases}")
    print("RUST-114 RUST-113 journal checkpoint monitor fail-closed contract: 27/27 expected cases passed")
