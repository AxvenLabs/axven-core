#!/usr/bin/env python3
"""RUST-056: TEST-ONLY multi-step journal-monitor-journal observer rotation verifier."""
...
ROTATION_AUTH_SCHEMA = "axven-native-journal-monitor-journal-observer-set-rotation-quorum-v2"
ROTATION_PAYLOAD_TYPE = "application/vnd.axven.native-journal-monitor-journal-observer-set-rotation.v2+json"
FINAL_BUNDLE_SCHEMA = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-bundle-v3"
FINAL_REPORT_SCHEMA = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-v3"
FINAL_STATEMENT_SCHEMA = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-statement-v3"
ROTATION_DOMAIN = b"AXVEN_NATIVE_JOURNAL_MONITOR_JOURNAL_OBSERVER_SET_ROTATION_V2\x00"
FINAL_DOMAIN = b"AXVEN_NATIVE_JOURNAL_MONITOR_ROTATION_JOURNAL_CHECKPOINT_OBSERVATION_V3\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
ROTATION_KEYS = frozenset({
    ...
    "predecessor_successor_bundle_sha256", "checkpoint_sha256",
...
FINAL_STATEMENT_KEYS = frozenset({
    ...
    "monitor_set_sha256", "entry_count", "journal_sha256", "head_entry_sha256",
    "previous_checkpoint_sha256", "journal_observer_checkpoint_sha256",
...
def validate_rotation(
    ...
    if (
        ...
        raise AssertionError("invalid second journal-monitor-journal observer rotation fields")
    if type(rotation.get("sequence")) is not int or rotation["sequence"] != FINAL_SET_SEQUENCE:
        raise AssertionError("invalid second journal-monitor-journal observer rotation sequence")
    if rotation.get("from_set_sha256") != sha256(canonical(rotation1_verify.new_observer_set())):
        raise AssertionError("second journal-monitor-journal observer predecessor set mismatch")
    if rotation.get("to_set") != final_observer_set():
        raise AssertionError("unexpected final journal-monitor-journal observer set")
    if rotation.get("cumulative_revoked_observer_ids") != CUMULATIVE_REVOKED_OBSERVER_IDS:
        raise AssertionError("cumulative journal-monitor-journal observer revocation mismatch")
    if rotation.get("predecessor_rotation_sha256") != sha256(first_rotation_raw):
        raise AssertionError("predecessor journal-monitor-journal observer rotation digest mismatch")
    ...
    if rotation.get("predecessor_successor_bundle_sha256") != sha256(first_successor_raw):
        raise AssertionError("predecessor journal-monitor-journal observer bundle digest mismatch")
    ...
    if rotation.get("activation_source_commit") != source_sha:
        raise AssertionError("second journal-monitor-journal observer rotation source mismatch")
    if rotation.get("production") is not False:
        raise AssertionError("production second journal-monitor-journal observer rotation forbidden in RUST-056")
...
def validate_rotation_auth(auth: dict, rotation_raw: bytes) -> None:
    if (
        ...
        raise AssertionError("invalid second journal-monitor-journal observer rotation authorization fields")
    if auth.get("algorithm") != ALGORITHM or auth.get("payload_type") != ROTATION_PAYLOAD_TYPE:
        raise AssertionError("invalid second journal-monitor-journal observer rotation authorization envelope")
    if type(auth.get("threshold")) is not int or auth["threshold"] != THRESHOLD:
        raise AssertionError("invalid second journal-monitor-journal observer rotation authorization threshold")
    if auth.get("payload_sha256") != sha256(rotation_raw) or auth.get("production") is not False:
        raise AssertionError("second journal-monitor-journal observer rotation authorization payload mismatch")
    ...
    if not isinstance(observers, list) or not (
        ...
        raise AssertionError("invalid second journal-monitor-journal observer rotation authorization size")
    if not all(isinstance(item, dict) and frozenset(item) == AUTH_OBSERVER_KEYS for item in observers):
        raise AssertionError("invalid second journal-monitor-journal observer rotation authorization entry")
    ...
    if any(observer_id not in PREDECESSOR_PINNED_OBSERVERS for observer_id in ids):
        raise AssertionError("unknown second journal-monitor-journal observer rotation authorizer")
...
def validate_final_report(report: dict, set_sha: str) -> dict:
    if not isinstance(report, dict) or frozenset(report) != FINAL_REPORT_KEYS:
        raise AssertionError("invalid final journal-monitor-journal observer report fields")
    if report.get("schema") != FINAL_REPORT_SCHEMA or report.get("algorithm") != ALGORITHM:
        raise AssertionError("invalid final journal-monitor-journal observer report envelope")
    statement = report.get("statement")
    if not isinstance(statement, dict) or frozenset(statement) != FINAL_STATEMENT_KEYS:
        raise AssertionError("invalid final journal-monitor-journal observer statement fields")
    if statement.get("schema") != FINAL_STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError("invalid final journal-monitor-journal observer statement boundary")
    observer_id = statement.get("observer_id")
    if not isinstance(observer_id, str) or observer_id not in FINAL_PINNED_OBSERVERS:
        raise AssertionError("unknown final journal-monitor-journal observer")
    if observer_id in CUMULATIVE_REVOKED_OBSERVER_IDS:
        raise AssertionError("revoked journal-monitor-journal observer resurrected in final set")
    if (
        statement.get("observer_set_sequence") != FINAL_SET_SEQUENCE
        or statement.get("observer_set_sha256") != set_sha
    ):
        raise AssertionError("final journal-monitor-journal observer set epoch mismatch")
    material_verify.ed25519_verify(
...
def validate_final_bundle(bundle: dict, target: dict) -> None:
    ...
    if (
        ...
        raise AssertionError("invalid final journal-monitor-journal observation bundle fields")
    if (
        bundle.get("observer_set_sequence") != FINAL_SET_SEQUENCE
        or bundle.get("observer_set_sha256") != set_sha
    ):
        raise AssertionError("final journal-monitor-journal observation bundle set mismatch")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid final journal-monitor-journal observation threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production final journal-monitor-journal observation forbidden in RUST-056")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (
        THRESHOLD <= len(reports) <= len(FINAL_PINNED_OBSERVERS)
    ):
        raise AssertionError("invalid final journal-monitor-journal observation report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("final journal-monitor-journal observer ids must be unique and sorted")
    matching = 0
    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("final journal-monitor-journal observer source mismatch")
        same_epoch_parent = (
        ...
        if not exact:
            raise AssertionError(
                "final journal-monitor-journal observer report does not match canonical checkpoint"
            )
        matching += 1
    if matching < THRESHOLD:
        raise AssertionError("final journal-monitor-journal observer quorum below threshold")


def verify(*args) -> None:
    if len(args) != 65:
        raise AssertionError("unexpected RUST-056 verifier argument count")
    (
        *base_paths,
        old_bundle_path,
        first_rotation_path,
    ...
    if len(base_paths) != 56:
        raise AssertionError("unexpected RUST-056 base path count")
    ...
    first_rotation_raw, _ = floor_verify.load_canonical(
        first_rotation_path, "first journal-monitor-journal observer rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        first_auth_path, "first journal-monitor-journal observer rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        first_successor_path, "first successor journal-monitor-journal observer bundle"
    )
    second_rotation_raw, second_rotation = floor_verify.load_canonical(
        second_rotation_path, "second journal-monitor-journal observer rotation"
    )
    _, second_auth = floor_verify.load_canonical(
        second_auth_path, "second journal-monitor-journal observer rotation authorization"
    )
    _, final_bundle = floor_verify.load_canonical(
        final_bundle_path, "final journal-monitor-journal observer bundle"
    )
    validate_rotation(
        second_rotation,
        target,
    ...
    print(
        "RUST-056 multi-step journal-monitor-journal observer rotation: GREEN "
...
def main() -> None:
    if len(sys.argv) != 67 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_056_multistep_journal_monitor_journal_observer_rotation_verify.py verify "
