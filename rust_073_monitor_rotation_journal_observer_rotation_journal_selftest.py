#!/usr/bin/env python3
"""RUST-073 detached availability and fail-closed observer rotation journal selftest."""
...
def main() -> None:
    if len(sys.argv) != 115:
        raise SystemExit(
            "usage: rust_073_monitor_rotation_journal_observer_rotation_journal_selftest.py "
    ...
    if len(base) != 111:
        raise AssertionError("unexpected RUST-073 selftest base path count")
    ...
    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        base[99], "RUST-069 final monitor rotation checkpoint"
    )
    target = observer_verify.canonical_target(
        monitored_checkpoint_raw, monitored_checkpoint, source_sha
    )
    old_bundle_raw, _ = floor_verify.load_canonical(base[100], "RUST-073 old observer bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base[101], "RUST-073 first rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base[102], "RUST-073 first auth")
    first_successor_raw, _ = floor_verify.load_canonical(base[103], "RUST-073 first successor")
    second_rotation_raw, _ = floor_verify.load_canonical(base[104], "RUST-073 second rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base[105], "RUST-073 second auth")
    final_bundle_raw, _ = floor_verify.load_canonical(base[106], "RUST-073 final bundle")
    ...
    for subset in itertools.combinations(prefix_checkpoint["observers"], journal_verify.THRESHOLD):
        ...
        prefix_ok += 1
    if prefix_ok != 3:
        raise AssertionError("unexpected RUST-073 prefix subset count")
    print("[GREEN] RUST-073 prefix checkpoint availability: 3/3 valid two-observer subsets accepted")
    ...
    for subset in itertools.combinations(final_checkpoint["observers"], journal_verify.THRESHOLD):
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-073 final subset count")
    print("[GREEN] RUST-073 final checkpoint availability: 3/3 valid two-observer subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("prefix-genesis-set-rewrite", lambda: run_with(107, write("a.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_sha256"] = "0" * 64
        fail("prefix-first-rotation-digest", lambda: run_with(107, write("b.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_auth_sha256"] = "0" * 64
        fail("prefix-first-auth-digest", lambda: run_with(107, write("c.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitor_rotation_journal_checkpoint_sha256"] = "0" * 64
        fail("prefix-monitored-checkpoint", lambda: run_with(107, write("d.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitor_rotation_journal_checkpoint_statement_sha256"] = "0" * 64
        fail("prefix-monitored-statement", lambda: run_with(107, write("e.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["observed_target_sha256"] = "0" * 64
        fail("prefix-observed-target", lambda: run_with(107, write("f.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["activation_source_commit"] = "0" * 40
        fail("prefix-source", lambda: run_with(107, write("g.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["production"] = True
        fail("prefix-production", lambda: run_with(107, write("h.json", value))); cases += 1

        value = copy.deepcopy(final_journal); value["entries"] = value["entries"][:2]
        fail("final-entry-truncation", lambda: run_with(109, write("i.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][0]["observation_bundle_sha256"] = "0" * 64
        fail("final-prefix-rewrite", lambda: run_with(109, write("j.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["sequence"] = 1
        fail("final-sequence-rollback", lambda: run_with(109, write("k.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["predecessor_entry_sha256"] = "0" * 64
        fail("final-predecessor-entry", lambda: run_with(109, write("l.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_sha256"] = "0" * 64
        fail("final-rotation-digest", lambda: run_with(109, write("m.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_auth_sha256"] = "0" * 64
        fail("final-rotation-auth-digest", lambda: run_with(109, write("n.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["observation_bundle_sha256"] = "0" * 64
        fail("final-observation-bundle", lambda: run_with(109, write("o.json", value))); cases += 1
        ...
        fail("final-revocation-omission", lambda: run_with(109, write("p.json", value))); cases += 1

        value = copy.deepcopy(prefix_checkpoint); value["threshold"] = 1
        fail("prefix-threshold", lambda: run_with(108, write("q.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["observers"] = value["observers"][:1]
        fail("prefix-below-threshold", lambda: run_with(108, write("r.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["observers"] = [value["observers"][0], copy.deepcopy(value["observers"][0])]
        fail("prefix-duplicate", lambda: run_with(108, write("s.json", value))); cases += 1
        ...
        fail("final-signature", lambda: run_with(110, write("af.json", value))); cases += 1

        noncanonical_journal = root / "noncanonical-journal.json"
        noncanonical_journal.write_text(json.dumps(final_journal, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-journal", lambda: run_with(109, noncanonical_journal)); cases += 1
        ...
        fail("noncanonical-final-checkpoint", lambda: run_with(110, noncanonical_checkpoint)); cases += 1
    ...
    if cases != 35:
        raise AssertionError(f"unexpected RUST-073 selftest case count: {cases}")
    print("RUST-073 observer rotation journal fail-closed contract: 35/35 expected cases passed")
