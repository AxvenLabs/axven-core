#!/usr/bin/env python3
"""RUST-046: TEST-ONLY monitor-rotation-journal checkpoint observation verifier."""
...
STATEMENT_KEYS = frozenset({
    ...
    "monitor_set_sequence", "monitor_set_sha256", "journal_sha256", "head_entry_sha256",
...
def canonical_target(checkpoint_raw: bytes, checkpoint: dict, source_sha: str) -> dict:
    ...
    if not isinstance(statement, dict):
        raise AssertionError("missing final monitor-journal checkpoint statement")
    if statement.get("activation_source_commit") != source_sha:
        raise AssertionError("final monitor-journal checkpoint source mismatch")
    return {
        ...
        "observer_checkpoint_sha256": statement["observer_checkpoint_sha256"],
...
def validate_bundle(bundle: dict, target: dict) -> None:
    ...
    for statement in statements:
        ...
        if not exact:
            raise AssertionError("journal observer report does not match canonical checkpoint")
        matching += 1
    if matching < THRESHOLD:
        raise AssertionError("journal-observer quorum below threshold")


def verify(*args) -> None:
    if len(args) != 37:
        raise AssertionError("unexpected RUST-046 verifier argument count")
    *base_paths, observation_bundle_path, expected_source_sha, required_floor_text = args
    if len(base_paths) != 34:
        raise AssertionError("unexpected RUST-046 base path count")
    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(base_paths[-2], "final monitor journal")
    ...
    journal_verify.validate_checkpoint_envelope(
        final_checkpoint, final_journal_raw, rotation2_verify.FINAL_PINNED_MONITORS, "RUST-046 canonical final"
    )
    _, bundle = floor_verify.load_canonical(observation_bundle_path, "journal observer bundle")
    ...
    print(
        "RUST-046 monitor-journal checkpoint observation: GREEN "
        f"source={expected_source_sha} threshold={THRESHOLD} observers={ids} checkpoint={target['checkpoint_sha256']}"
    )


def main() -> None:
    if len(sys.argv) != 39 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_046_monitor_journal_gossip_verify.py verify "
            "... FINAL_MON_JOURNAL FINAL_MON_CHECKPOINT OBSERVATION_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
