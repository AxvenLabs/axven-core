#!/usr/bin/env python3
"""RUST-073: TEST-ONLY append-only monitor-rotation-journal observer rotation journal verifier."""
...
JOURNAL_KEYS = frozenset({
    "schema", "activation_source_commit", "monitor_rotation_journal_checkpoint_sha256",
    "monitor_rotation_journal_checkpoint_statement_sha256", "observed_target_sha256",
    "entries", "production",
})
...
def target_digest(target: dict) -> str:
    if frozenset(target) != observer_verify.TARGET_KEYS:
        raise AssertionError("unexpected RUST-073 observed target fields")
...
def validate_journal(journal: dict, expected: dict, label: str) -> None:
    if (
        ...
        raise AssertionError(f"invalid {label} RUST-073 observer rotation journal fields")
    if journal.get("production") is not False:
        raise AssertionError(f"production {label} RUST-073 observer rotation journal forbidden")
    if journal != expected:
        raise AssertionError(f"{label} RUST-073 observer rotation journal continuity mismatch")
    entries = journal.get("entries")
    if not isinstance(entries, list) or not entries:
        raise AssertionError(f"empty {label} RUST-073 observer rotation journal")
    for index, entry in enumerate(entries):
        if (
            ...
            raise AssertionError(f"invalid {label} RUST-073 observer rotation journal entry")
        if type(entry.get("sequence")) is not int or entry["sequence"] != index:
            raise AssertionError(f"non-monotonic {label} RUST-073 journal sequence")
        if index == 0:
            if entry.get("predecessor_entry_sha256") is not None:
                raise AssertionError(f"unexpected {label} RUST-073 journal genesis predecessor")
        elif entry.get("predecessor_entry_sha256") != sha256(canonical(entries[index - 1])):
            raise AssertionError(f"broken {label} RUST-073 journal hash chain")
...
def validate_checkpoint_envelope(
    ...
    if (
        ...
        raise AssertionError(f"invalid {label} RUST-073 checkpoint fields")
    if checkpoint.get("algorithm") != ALGORITHM or checkpoint.get("production") is not False:
        raise AssertionError(f"invalid {label} RUST-073 checkpoint boundary")
    if type(checkpoint.get("threshold")) is not int or checkpoint["threshold"] != THRESHOLD:
        raise AssertionError(f"invalid {label} RUST-073 checkpoint threshold")
    statement = checkpoint.get("statement")
    if not isinstance(statement, dict) or frozenset(statement) != STATEMENT_KEYS:
        raise AssertionError(f"invalid {label} RUST-073 checkpoint statement fields")
    if statement.get("schema") != STATEMENT_SCHEMA or statement.get("production") is not False:
        raise AssertionError(f"invalid {label} RUST-073 checkpoint statement boundary")
    observers = checkpoint.get("observers")
    if not isinstance(observers, list) or not (THRESHOLD <= len(observers) <= len(pins)):
        raise AssertionError(f"invalid {label} RUST-073 checkpoint signature count")
    if not all(
        isinstance(item, dict) and frozenset(item) == CHECKPOINT_OBSERVER_KEYS
        for item in observers
    ):
        raise AssertionError(f"invalid {label} RUST-073 checkpoint signature row")
    ids = [item["observer_id"] for item in observers]
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError(f"{label} RUST-073 checkpoint observer ids must be unique and sorted")
    if any(observer_id not in pins for observer_id in ids):
        raise AssertionError(f"unknown {label} RUST-073 checkpoint signer")
...
def validate_checkpoint(
    ...
    if statement != expected_statement:
        raise AssertionError(f"{label} RUST-073 checkpoint statement mismatch")
...
def reject_observed_fork(
    ...
    if same_parent and canonical_checkpoint_raw != observed_checkpoint_raw:
        raise AssertionError("observed same-parent RUST-073 observer rotation journal checkpoint fork")


def verify(*args) -> None:
    if len(args) != 113:
        raise AssertionError("unexpected RUST-073 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 111:
        raise AssertionError("unexpected RUST-073 path count")

    base_paths = path_args[:107]
    prefix_journal_path, prefix_checkpoint_path, final_journal_path, final_checkpoint_path = path_args[107:111]
    ...
    monitored_checkpoint_raw, monitored_checkpoint = floor_verify.load_canonical(
        base_paths[99], "RUST-069 final monitor rotation checkpoint"
    )
    target = observer_verify.canonical_target(
        monitored_checkpoint_raw, monitored_checkpoint, expected_source_sha
    )

    old_bundle_raw, _ = floor_verify.load_canonical(base_paths[100], "RUST-073 old observer bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base_paths[101], "RUST-073 first observer rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base_paths[102], "RUST-073 first rotation authorization")
    first_successor_raw, _ = floor_verify.load_canonical(base_paths[103], "RUST-073 first successor observer bundle")
    second_rotation_raw, _ = floor_verify.load_canonical(base_paths[104], "RUST-073 second observer rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base_paths[105], "RUST-073 second rotation authorization")
    final_bundle_raw, _ = floor_verify.load_canonical(base_paths[106], "RUST-073 final observer bundle")
    ...
    prefix_journal_raw, prefix_journal = floor_verify.load_canonical(
        prefix_journal_path, "RUST-073 prefix observer rotation journal"
    )
    prefix_checkpoint_raw, prefix_checkpoint = floor_verify.load_canonical(
        prefix_checkpoint_path, "RUST-073 prefix observer rotation checkpoint"
    )
    final_journal_raw, final_journal = floor_verify.load_canonical(
        final_journal_path, "RUST-073 final observer rotation journal"
    )
    _, final_checkpoint = floor_verify.load_canonical(
        final_checkpoint_path, "RUST-073 final observer rotation checkpoint"
    )
    ...
    if final_journal["entries"][:2] != prefix_journal["entries"]:
        raise AssertionError("final RUST-073 observer rotation journal rewrites checkpointed prefix")
    ...
    print(
        "RUST-073 append-only observer rotation journal: GREEN "
        f"source={expected_source_sha} entries=3 checkpoint={sha256(canonical(final_checkpoint))}"
    )


def main() -> None:
    if len(sys.argv) != 115 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_073_monitor_rotation_journal_observer_rotation_journal_verify.py verify "
