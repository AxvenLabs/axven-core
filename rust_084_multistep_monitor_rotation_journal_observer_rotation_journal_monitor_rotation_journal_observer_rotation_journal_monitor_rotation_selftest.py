#!/usr/bin/env python3
"""RUST-084 detached second monitor rotation availability/fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 144:
        raise SystemExit(
            "usage: rust_084_multistep_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_selftest.py "
    ...
    if len(base) != 140:
        raise AssertionError("unexpected RUST-084 selftest base path count")
    ...
    for subset in itertools.combinations(second_auth["monitors"], rotation2_verify.THRESHOLD):
        ...
        auth_ok += 1
    if auth_ok != 3:
        raise AssertionError("unexpected RUST-084 authorization subset count")
    print("[GREEN] RUST-084 predecessor authorization availability: 3/3 valid two-monitor subsets accepted")
    ...
    for subset in itertools.combinations(final_bundle["reports"], rotation2_verify.THRESHOLD):
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-084 final monitoring subset count")
    print("[GREEN] RUST-084 final monitoring availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("rotation-sequence-rollback", lambda: run_with(137, write("a.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["from_set_sha256"] = "0" * 64
        fail("rotation-predecessor-set", lambda: run_with(137, write("b.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["to_set"]["threshold"] = 1
        fail("rotation-final-set", lambda: run_with(137, write("c.json", value))); cases += 1
        ...
        fail("rotation-cumulative-revocation", lambda: run_with(137, write("d.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_sha256"] = "0" * 64
        fail("rotation-predecessor-rotation", lambda: run_with(137, write("e.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_auth_sha256"] = "0" * 64
        fail("rotation-predecessor-auth", lambda: run_with(137, write("f.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_successor_bundle_sha256"] = "0" * 64
        fail("rotation-predecessor-successor", lambda: run_with(137, write("g.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"rt{idx}.json": run_with(137, write(n, v)),
            )
            cases += 1

        value = copy.deepcopy(second_rotation); value["production"] = True
        fail("rotation-production", lambda: run_with(137, write("h.json", value))); cases += 1

        value = copy.deepcopy(second_auth); value["threshold"] = 1
        fail("auth-threshold", lambda: run_with(138, write("i.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = value["monitors"][:1]
        fail("auth-below-threshold", lambda: run_with(138, write("j.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = [value["monitors"][0], copy.deepcopy(value["monitors"][0])]
        fail("auth-duplicate", lambda: run_with(138, write("k.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = list(reversed(value["monitors"]))
        fail("auth-unsorted", lambda: run_with(138, write("l.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["payload_sha256"] = "0" * 64
        fail("auth-payload", lambda: run_with(138, write("m.json", value))); cases += 1
        ...
        fail("auth-signature", lambda: run_with(138, write("n.json", value))); cases += 1

        value = copy.deepcopy(final_bundle); value["threshold"] = 1
        fail("final-threshold", lambda: run_with(139, write("o.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"] = value["reports"][:1]
        fail("final-below-threshold", lambda: run_with(139, write("p.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][1] = copy.deepcopy(value["reports"][0])
        fail("final-duplicate", lambda: run_with(139, write("q.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"] = list(reversed(value["reports"]))
        fail("final-unsorted", lambda: run_with(139, write("r.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["monitor_set_sequence"] = 1
        fail("final-set-sequence", lambda: run_with(139, write("s.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["monitor_set_sha256"] = "0" * 64
        fail("final-set-digest", lambda: run_with(139, write("t.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][0]["statement"]["monitor_id"] = rotation2_verify.CUMULATIVE_REVOKED_MONITOR_IDS[0]
        fail("revoked-m1-resurrection", lambda: run_with(139, write("u.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][0]["statement"]["monitor_id"] = rotation2_verify.CUMULATIVE_REVOKED_MONITOR_IDS[1]
        fail("revoked-m2-resurrection", lambda: run_with(139, write("v.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"ft{idx}.json": run_with(139, write(n, v)),
            )
            cases += 1
        ...
        fail("final-signature", lambda: run_with(139, write("w.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["production"] = True
        fail("final-production", lambda: run_with(139, write("x.json", value))); cases += 1

        noncanonical = root / "noncanonical.json"
        noncanonical.write_text(json.dumps(final_bundle, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-bundle", lambda: run_with(139, noncanonical)); cases += 1
        fail("first-successor-replay", lambda: run_with(139, base[136])); cases += 1
        fail("observed-valid-final-same-parent-fork", lambda: run_with(139, fork_path)); cases += 1

    if cases != 51:
        raise AssertionError(f"unexpected RUST-084 selftest case count: {cases}")
    print("RUST-084 multi-step monitor rotation fail-closed contract: 51/51 expected cases passed")
