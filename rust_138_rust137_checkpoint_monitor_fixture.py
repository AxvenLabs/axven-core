#!/usr/bin/env python3
"""RUST-138 TEST-ONLY RUST-137 checkpoint monitor producer."""
...
def private_for(monitor_id: str) -> Ed25519PrivateKey:
    ...
    if public != monitor_verify.PINNED_MONITORS[monitor_id]:
        raise AssertionError("RUST-138 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_138_rust137_checkpoint_monitor_fixture.py SOURCE_SHA")
    source_sha = sys.argv[1]

    final_journal_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust137-final-monitor-rotation-journal.json",
        "RUST-137 final monitor rotation journal",
    )
    ...
    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        OUT / "axven-rust137-observed-fork-monitor-rotation-checkpoint.json",
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-138 canonical target source mismatch")
    ...
    (OUT / "axven-rust138-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle(reports))
    )
    (OUT / "axven-rust138-observed-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle([reports[0], reports[1], fork_report]))
    )
    print("RUST-138 TEST-only RUST-137 journal checkpoint monitor fixture: GREEN")
