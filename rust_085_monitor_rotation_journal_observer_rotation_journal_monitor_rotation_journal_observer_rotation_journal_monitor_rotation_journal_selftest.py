#!/usr/bin/env python3
"""RUST-085 detached availability and fail-closed journal selftest."""
...
def main() -> None:
    if len(sys.argv) != 148:
        raise SystemExit(
            "usage: rust_085_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_selftest.py "
    ...
    if len(base) != 144:
        raise AssertionError("unexpected RUST-085 selftest base path count")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(base[133], "RUST-085 old monitor bundle")
    first_rotation_raw, _ = floor_verify.load_canonical(base[134], "RUST-085 first rotation")
    first_auth_raw, _ = floor_verify.load_canonical(base[135], "RUST-085 first auth")
    first_successor_raw, _ = floor_verify.load_canonical(base[136], "RUST-085 first successor")
    second_rotation_raw, _ = floor_verify.load_canonical(base[137], "RUST-085 second rotation")
    second_auth_raw, _ = floor_verify.load_canonical(base[138], "RUST-085 second auth")
    final_bundle_raw, _ = floor_verify.load_canonical(base[139], "RUST-085 final bundle")
    ...
    for subset in itertools.combinations(prefix_checkpoint["monitors"], journal_verify.THRESHOLD):
        ...
        prefix_ok += 1
    if prefix_ok != 3:
        raise AssertionError("unexpected RUST-085 prefix subset count")
    print("[GREEN] RUST-085 prefix checkpoint availability: 3/3 valid two-monitor subsets accepted")
    ...
    for subset in itertools.combinations(final_checkpoint["monitors"], journal_verify.THRESHOLD):
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-085 final subset count")
    print("[GREEN] RUST-085 final checkpoint availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("prefix-genesis-set-rewrite", lambda: run_with(140, write("a.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_sha256"] = "0" * 64
        fail("prefix-first-rotation-digest", lambda: run_with(140, write("b.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["entries"][1]["rotation_auth_sha256"] = "0" * 64
        fail("prefix-first-auth-digest", lambda: run_with(140, write("c.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitored_checkpoint_sha256"] = "0" * 64
        fail("prefix-monitored-checkpoint", lambda: run_with(140, write("d.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["monitored_checkpoint_statement_sha256"] = "0" * 64
        fail("prefix-monitored-statement", lambda: run_with(140, write("e.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["observed_target_sha256"] = "0" * 64
        fail("prefix-observed-target", lambda: run_with(140, write("f.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["activation_source_commit"] = "0" * 40
        fail("prefix-source", lambda: run_with(140, write("g.json", value))); cases += 1
        value = copy.deepcopy(prefix_journal); value["production"] = True
        fail("prefix-production", lambda: run_with(140, write("h.json", value))); cases += 1

        value = copy.deepcopy(final_journal); value["entries"] = value["entries"][:2]
        fail("final-entry-truncation", lambda: run_with(142, write("i.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][0]["monitor_bundle_sha256"] = "0" * 64
        fail("final-prefix-rewrite", lambda: run_with(142, write("j.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["sequence"] = 1
        fail("final-sequence-rollback", lambda: run_with(142, write("k.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["predecessor_entry_sha256"] = "0" * 64
        fail("final-predecessor-entry", lambda: run_with(142, write("l.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_sha256"] = "0" * 64
        fail("final-rotation-digest", lambda: run_with(142, write("m.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["rotation_auth_sha256"] = "0" * 64
        fail("final-rotation-auth-digest", lambda: run_with(142, write("n.json", value))); cases += 1
        value = copy.deepcopy(final_journal); value["entries"][2]["monitor_bundle_sha256"] = "0" * 64
        fail("final-monitor-bundle", lambda: run_with(142, write("o.json", value))); cases += 1
        ...
        fail("final-revocation-omission", lambda: run_with(142, write("p.json", value))); cases += 1

        value = copy.deepcopy(prefix_checkpoint); value["threshold"] = 1
        fail("prefix-threshold", lambda: run_with(141, write("q.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["monitors"] = value["monitors"][:1]
        fail("prefix-below-threshold", lambda: run_with(141, write("r.json", value))); cases += 1
        value = copy.deepcopy(prefix_checkpoint); value["monitors"] = [value["monitors"][0], copy.deepcopy(value["monitors"][0])]
        fail("prefix-duplicate", lambda: run_with(141, write("s.json", value))); cases += 1
        ...
        fail("prefix-signature", lambda: run_with(141, write("t.json", value))); cases += 1

        value = copy.deepcopy(final_checkpoint); value["statement"]["previous_checkpoint_sha256"] = "0" * 64
        fail("final-previous-checkpoint", lambda: run_with(143, write("u.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["head_entry_sha256"] = "0" * 64
        fail("final-head-entry", lambda: run_with(143, write("v.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitored_checkpoint_sha256"] = "0" * 64
        fail("final-monitored-checkpoint", lambda: run_with(143, write("w.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitored_checkpoint_statement_sha256"] = "0" * 64
        fail("final-monitored-statement", lambda: run_with(143, write("x.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["observed_target_sha256"] = "0" * 64
        fail("final-observed-target", lambda: run_with(143, write("y.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["activation_source_commit"] = "0" * 40
        fail("final-source", lambda: run_with(143, write("z.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["production"] = True
        fail("final-production", lambda: run_with(143, write("aa.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitor_set_sequence"] = 1
        fail("final-monitor-set-sequence", lambda: run_with(143, write("ab.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["statement"]["monitor_set_sha256"] = "0" * 64
        fail("final-monitor-set-digest", lambda: run_with(143, write("ac.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["threshold"] = 1
        fail("final-threshold", lambda: run_with(143, write("ad.json", value))); cases += 1
        value = copy.deepcopy(final_checkpoint); value["monitors"] = value["monitors"][:1]
        fail("final-below-threshold", lambda: run_with(143, write("ae.json", value))); cases += 1
        ...
        fail("final-signature", lambda: run_with(143, write("af.json", value))); cases += 1

        noncanonical_journal = root / "noncanonical-journal.json"
        noncanonical_journal.write_text(json.dumps(final_journal, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-journal", lambda: run_with(142, noncanonical_journal)); cases += 1
        ...
        fail("noncanonical-final-checkpoint", lambda: run_with(143, noncanonical_checkpoint)); cases += 1
    ...
    if cases != 35:
        raise AssertionError(f"unexpected RUST-085 selftest case count: {cases}")
    print("RUST-085 monitor rotation journal fail-closed contract: 35/35 expected cases passed")
