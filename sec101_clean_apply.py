...
for name in (
    "add_outbound_peer",
    "remove_outbound_peer",
    "peer_health_state",
    "peer_health_incident_history",
    "peer_health_history",
    "record_peer_health_transition",
    "outbound_peer_status",
    "peer_health_summary",
    "set_peer_retry_schedule",
    "clear_peer_retry_schedule",
    "peer_retry_delay",
):
    anchor = f"    def {name}("
    if anchor not in text:
        raise SystemExit(f"SEC-101 method anchor not found: {name}")
    text = text.replace(anchor, f"    @_peer_locked\n{anchor}", 1)
...
for temp in (Path("sec101_clean_apply.py"), Path(".github/workflows/sec101_clean_build.yml")):
    if temp.exists():
        temp.unlink()
