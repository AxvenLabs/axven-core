#!/usr/bin/env python3
"""RUST-066 TEST-ONLY monitor-rotation-journal observer-rotation-journal monitor producer."""
...
def private_for(monitor_id: str) -> Ed25519PrivateKey:
    ...
    if public != monitor_verify.PINNED_MONITORS[monitor_id]:
        raise AssertionError("RUST-066 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if (
        ...
        raise SystemExit(
            "usage: rust_066_monitor_rotation_journal_observer_rotation_journal_monitor_fixture.py SOURCE_SHA"
        )
    ...
    final_journal_raw, _ = floor_verify.load_canonical(
        ...
        "final monitor-rotation-journal observer rotation journal",
    )
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-066 canonical target source mismatch")
    ...
    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        ...
        "observed monitor-rotation-journal observer rotation checkpoint fork",
    )
    fork_statement = journal_verify.validate_checkpoint_envelope(
        fork_checkpoint,
        final_journal_raw,
        rotation2_verify.FINAL_PINNED_OBSERVERS,
        "RUST-066 observed fork producer",
    )
    ...
    (OUT / "axven-rust066-monitor-bundle.json").write_bytes(
        material_verify.canonical(bundle(canonical_reports))
    )
    (OUT / "axven-rust066-observed-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(
            bundle([canonical_reports[0], canonical_reports[1], fork_report])
        )
    )
    print(
        "RUST-066 TEST-only monitor-rotation-journal observer-rotation-journal monitor fixture: GREEN"
    )
