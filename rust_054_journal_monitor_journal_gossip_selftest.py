#!/usr/bin/env python3
"""RUST-054 detached availability and fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 61:
        raise SystemExit(
            "usage: rust_054_journal_monitor_journal_gossip_selftest.py "
            "... FINAL_JOURNAL_MONITOR_CHECKPOINT BUNDLE FORK_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
    ...
    *base_paths, bundle_path, fork_bundle_path = paths
    ...
    if len(base_paths) != 56:
        raise AssertionError("unexpected RUST-054 selftest base path count")
    ...
    _, bundle = floor_verify.load_canonical(
        bundle_path, "journal-monitor-journal observer bundle"
    )
    target = gossip_verify.canonical_target(
        final_checkpoint_raw, checkpoint, source_sha
    )
    cases = 0
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        def run(path: Path) -> None:
            gossip_verify.verify(
                *base_paths, path, source_sha, required_floor
            )

        for idx, pair in enumerate(((0, 1), (0, 2), (1, 2)), start=1):
            value = copy.deepcopy(bundle)
            value["reports"] = [
                copy.deepcopy(bundle["reports"][i]) for i in pair
            ]
            value["reports"].sort(
                key=lambda report: report["statement"]["observer_id"]
            )
            run(write(f"valid-{idx}.json", value))
        print(
            "[GREEN] RUST-054 journal-monitor-journal observer availability: "
        ...
        value = copy.deepcopy(bundle); value["schema"] = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-bundle-v0"; mutations.append(("bundle-schema", value))
        value = copy.deepcopy(bundle); value["reports"][0]["schema"] = "axven-native-journal-monitor-rotation-journal-checkpoint-observation-v0"; mutations.append(("report-schema", value))
        ...
        value = copy.deepcopy(bundle); value["reports"][0]["statement"]["journal_observer_checkpoint_sha256"] = "0" * 64; mutations.append(("journal-observer-checkpoint-digest", value))
        value = copy.deepcopy(bundle); value["reports"][0]["statement"]["monitor_journal_checkpoint_sha256"] = "0" * 64; mutations.append(("monitor-journal-checkpoint-digest", value))
        value = copy.deepcopy(bundle); value["reports"][0]["statement"]["monitor_journal_checkpoint_statement_sha256"] = "0" * 64; mutations.append(("monitor-journal-checkpoint-statement-digest", value))

        for idx, (label, value) in enumerate(mutations, start=1):
            fail(
                label,
                lambda value=value, idx=idx: run(
                    write(f"m-{idx}.json", value)
                ),
            )
            cases += 1
        ...
        fail(
            "observed-cross-observer-journal-monitor-rotation-journal-fork",
            lambda: run(fork_bundle_path),
        ); cases += 1

    if cases != 25:
        raise AssertionError(
            f"unexpected RUST-054 selftest case count: {cases}"
        )
    print(
        "RUST-054 journal-monitor-journal observation fail-closed contract: "
