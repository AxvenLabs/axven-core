#!/usr/bin/env python3
"""RUST-117 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
SEEDS = {
    ...
    rotation1_verify.M4_ID: "0d" * 32,
    rotation2_verify.M5_ID: "1d" * 32,
}
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-117 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_117_rust114_checkpoint_monitor_rotation_journal_fixture.py SOURCE_SHA")
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-117 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust114-monitor-bundle.json", "RUST-117 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust115-monitor-set-rotation.json", "RUST-117 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust115-monitor-set-rotation-auth.json", "RUST-117 first rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust115-successor-monitor-bundle.json", "RUST-117 first successor monitor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust116-second-monitor-set-rotation.json", "RUST-117 second monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust116-second-monitor-set-rotation-auth.json", "RUST-117 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust116-final-monitor-bundle.json", "RUST-117 final monitor bundle"
    )
    ...
    (OUT / "axven-rust117-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust117-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust117-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust117-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust117-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-117 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
