# RUST-116 — TEST-ONLY second RUST-114 checkpoint monitor-set rotation

RUST-116 composes the exact reviewed RUST-115 first monitor-set rotation and performs the second TEST-only rotation for the RUST-114 checkpoint-monitor family.

## Contract

- Monitor set rotates from M2/M3/M4 to M3/M4/M5 with a 2-of-3 threshold.
- M1 remains revoked from RUST-115 and M2 is newly revoked; cumulative revocation continuity is explicit.
- The second rotation binds the exact RUST-115 rotation, authorization and successor-bundle SHA-256 values plus all 12 inherited RUST-113 checkpoint target fields.
...
- CI keeps generated evidence read-only with chmod 0444, uses a fixed 228-path manifest and an 89-file verifier-only detached consumer under `env -i` and `/usr/bin/python3 -S`.
