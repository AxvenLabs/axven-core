#!/usr/bin/env python3
"""RUST-072 detached multi-step observer rotation availability/fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 111:
        raise SystemExit(
            "usage: rust_072_multistep_monitor_rotation_journal_observer_set_rotation_selftest.py "
    ...
    if len(base) != 107:
        raise AssertionError("unexpected RUST-072 selftest base path count")
    ...
    second_rotation_raw, second_rotation = floor_verify.load_canonical(
        base[104], "RUST-072 second observer rotation"
    )
    _, second_auth = floor_verify.load_canonical(
        base[105], "RUST-072 second rotation authorization"
    )
    _, final_bundle = floor_verify.load_canonical(
        base[106], "RUST-072 final observer bundle"
    )
    target = observer_verify.canonical_target(checkpoint_raw, checkpoint, source_sha)

    auth_ok = 0
    for subset in itertools.combinations(
        second_auth["observers"], rotation2_verify.THRESHOLD
    ):
        ...
        auth_ok += 1
    if auth_ok != 3:
        raise AssertionError("unexpected RUST-072 authorization subset count")
    print(
        "[GREEN] RUST-072 predecessor authorization availability: "
    ...
    for subset in itertools.combinations(
        ...
        final_ok += 1
    if final_ok != 3:
        raise AssertionError("unexpected RUST-072 final observation subset count")
    print(
        "[GREEN] RUST-072 final observation availability: "
        "3/3 valid two-observer subsets accepted"
    )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("rotation-sequence-rollback", lambda: run_with(104, write("a.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["from_set_sha256"] = "0" * 64
        fail("rotation-predecessor-set", lambda: run_with(104, write("b.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["to_set"]["threshold"] = 1
        fail("rotation-final-set", lambda: run_with(104, write("c.json", value))); cases += 1
        ...
        fail("rotation-cumulative-revocation", lambda: run_with(104, write("d.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_sha256"] = "0" * 64
        fail("rotation-predecessor-rotation", lambda: run_with(104, write("e.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_auth_sha256"] = "0" * 64
        fail("rotation-predecessor-auth", lambda: run_with(104, write("f.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_successor_bundle_sha256"] = "0" * 64
        fail("rotation-predecessor-successor", lambda: run_with(104, write("g.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(observer_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"rt{idx}.json": run_with(104, write(n, v)),
            )
            cases += 1

        value = copy.deepcopy(second_rotation); value["production"] = True
        fail("rotation-production", lambda: run_with(104, write("h.json", value))); cases += 1

        value = copy.deepcopy(second_auth); value["threshold"] = 1
        fail("auth-threshold", lambda: run_with(105, write("i.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["observers"] = value["observers"][:1]
        fail("auth-below-threshold", lambda: run_with(105, write("j.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["observers"] = [value["observers"][0], copy.deepcopy(value["observers"][0])]
        fail("auth-duplicate", lambda: run_with(105, write("k.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["observers"] = list(reversed(value["observers"]))
        fail("auth-unsorted", lambda: run_with(105, write("l.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["payload_sha256"] = "0" * 64
        fail("auth-payload", lambda: run_with(105, write("m.json", value))); cases += 1
        ...
        fail("auth-signature", lambda: run_with(105, write("n.json", value))); cases += 1

        value = copy.deepcopy(final_bundle); value["threshold"] = 1
        fail("final-threshold", lambda: run_with(106, write("o.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"] = value["reports"][:1]
        fail("final-below-threshold", lambda: run_with(106, write("p.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][1] = copy.deepcopy(value["reports"][0])
        fail("final-duplicate", lambda: run_with(106, write("q.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"] = list(reversed(value["reports"]))
        fail("final-unsorted", lambda: run_with(106, write("r.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["observer_set_sequence"] = 1
        fail("final-set-sequence", lambda: run_with(106, write("s.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["observer_set_sha256"] = "0" * 64
        fail("final-set-digest", lambda: run_with(106, write("t.json", value))); cases += 1
        ...
        fail("revoked-o1-resurrection", lambda: run_with(106, write("u.json", value))); cases += 1
        ...
        fail("revoked-o2-resurrection", lambda: run_with(106, write("v.json", value))); cases += 1
        ...
        for idx, field in enumerate(sorted(observer_verify.TARGET_KEYS)):
            ...
            fail(
                ...
                lambda v=value, n=f"ft{idx}.json": run_with(106, write(n, v)),
            )
            cases += 1
        ...
        fail("final-signature", lambda: run_with(106, write("w.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["production"] = True
        fail("final-production", lambda: run_with(106, write("x.json", value))); cases += 1

        noncanonical = root / "noncanonical.json"
        noncanonical.write_text(json.dumps(final_bundle, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-bundle", lambda: run_with(106, noncanonical)); cases += 1
        fail("first-successor-replay", lambda: run_with(106, base[103])); cases += 1
        fail("observed-valid-final-same-parent-fork", lambda: run_with(106, fork_path)); cases += 1

    if cases != 51:
        raise AssertionError(f"unexpected RUST-072 selftest case count: {cases}")
    print("RUST-072 multi-step observer rotation fail-closed contract: 51/51 expected cases passed")
