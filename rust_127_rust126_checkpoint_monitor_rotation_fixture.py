#!/usr/bin/env python3
"""RUST-127 TEST-ONLY monitor-set rotation producer."""
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-127 TEST-only monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_127_rust126_checkpoint_monitor_rotation_fixture.py SOURCE_SHA")
    ...
    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust126-monitor-bundle.json",
        "RUST-127 predecessor monitor bundle",
    )
    _, old_fork_bundle = floor_verify.load_canonical(
        OUT / "axven-rust126-observed-fork-monitor-bundle.json",
        "RUST-127 predecessor observed fork monitor bundle",
    )
    ...
    (OUT / "axven-rust127-monitor-set-rotation.json").write_bytes(rotation_raw)
    (OUT / "axven-rust127-monitor-set-rotation-auth.json").write_bytes(material_verify.canonical(auth))
    (OUT / "axven-rust127-successor-monitor-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle(reports, set_sha))
    )
    (OUT / "axven-rust127-successor-fork-bundle.json").write_bytes(
        material_verify.canonical(successor_bundle([reports[0], reports[1], fork_report], set_sha))
    )
    print("RUST-127 TEST-only monitor-set rotation fixture: GREEN")
