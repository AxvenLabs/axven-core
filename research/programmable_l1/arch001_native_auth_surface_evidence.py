#!/usr/bin/env python3
"""ARCH-001 research-only native-transfer authorization/state surface evidence.

Couples the existing eight-step native AXVEN transfer workload with deterministic
...
def witness_sizes() -> dict[str, int]:
    message = axven.sha256(b"arch-001-native-auth-surface").encode("ascii")
    ed = axven.Wallet()
...
def build_evidence() -> dict[str, object]:
    ...
    return {
        "schema": "axven-arch001-native-auth-surface-v1",
        "scope": "research-only; outside production consensus routing",
        "workload": "eight sequential Alice-to-Bob native AXVEN transfers of 1 each",
        "rows": rows,
...
def main() -> None:
    ...
    print("ARCH-001 native auth/state surface evidence: 5/5 GREEN")
