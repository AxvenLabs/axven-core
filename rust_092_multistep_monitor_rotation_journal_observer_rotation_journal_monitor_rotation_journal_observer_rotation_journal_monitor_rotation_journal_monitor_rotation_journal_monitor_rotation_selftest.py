#!/usr/bin/env python3
"""RUST-092 detached second monitor-set rotation availability/fail-closed selftest."""
...
def main() -> None:
    if len(sys.argv) != 166:
        raise SystemExit(
            "usage: rust_092_multistep_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_monitor_rotation_journal_monitor_rotation_selftest.py "
    ...
    if len(base) != 162:
        raise AssertionError("unexpected RUST-092 selftest base path count")

    rotation2_verify.verify(*base, source_sha, required_floor)
    checkpoint_raw, checkpoint = floor_verify.load_canonical(base[154], "RUST-089 final monitor rotation checkpoint")
    second_rotation_raw, second_rotation = floor_verify.load_canonical(base[159], "RUST-092 second monitor rotation")
    _, second_auth = floor_verify.load_canonical(base[160], "RUST-092 second rotation authorization")
    _, final_bundle = floor_verify.load_canonical(base[161], "RUST-092 final monitor bundle")
    target = monitor_verify.checkpoint_target(checkpoint_raw, checkpoint["statement"])
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-092 selftest target source mismatch")
    ...
    if auth_ok != 3:
        raise AssertionError("unexpected RUST-092 authorization subset count")
    print("[GREEN] RUST-092 predecessor authorization availability: 3/3 valid two-monitor subsets accepted")
    ...
    if final_ok != 3:
        raise AssertionError("unexpected RUST-092 final monitoring subset count")
    print("[GREEN] RUST-092 final monitoring availability: 3/3 valid two-monitor subsets accepted")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        fail("rotation-sequence-rollback", lambda: run_with(159, write("a.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["from_set_sha256"] = "0" * 64
        fail("rotation-predecessor-set", lambda: run_with(159, write("b.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["to_set"]["threshold"] = 1
        fail("rotation-final-set", lambda: run_with(159, write("c.json", value))); cases += 1
        ...
        fail("rotation-cumulative-revocation", lambda: run_with(159, write("d.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_sha256"] = "0" * 64
        fail("rotation-predecessor-rotation", lambda: run_with(159, write("e.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_rotation_auth_sha256"] = "0" * 64
        fail("rotation-predecessor-auth", lambda: run_with(159, write("f.json", value))); cases += 1
        value = copy.deepcopy(second_rotation); value["predecessor_successor_bundle_sha256"] = "0" * 64
        fail("rotation-predecessor-successor", lambda: run_with(159, write("g.json", value))); cases += 1

        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            value = copy.deepcopy(second_rotation); value[field] = replacement_for(field)
            fail(f"rotation-target-{field}", lambda v=value, n=f"rt{idx}.json": run_with(159, write(n, v))); cases += 1

        value = copy.deepcopy(second_rotation); value["production"] = True
        fail("rotation-production", lambda: run_with(159, write("h.json", value))); cases += 1

        value = copy.deepcopy(second_auth); value["threshold"] = 1
        fail("auth-threshold", lambda: run_with(160, write("i.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = value["monitors"][:1]
        fail("auth-below-threshold", lambda: run_with(160, write("j.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = [value["monitors"][0], copy.deepcopy(value["monitors"][0])]
        fail("auth-duplicate", lambda: run_with(160, write("k.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["monitors"] = list(reversed(value["monitors"]))
        fail("auth-unsorted", lambda: run_with(160, write("l.json", value))); cases += 1
        value = copy.deepcopy(second_auth); value["payload_sha256"] = "0" * 64
        fail("auth-payload", lambda: run_with(160, write("m.json", value))); cases += 1
        ...
        fail("auth-signature", lambda: run_with(160, write("n.json", value))); cases += 1

        value = copy.deepcopy(final_bundle); value["threshold"] = 1
        fail("final-threshold", lambda: run_with(161, write("o.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"] = value["reports"][:1]
        fail("final-below-threshold", lambda: run_with(161, write("p.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][1] = copy.deepcopy(value["reports"][0])
        fail("final-duplicate", lambda: run_with(161, write("q.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"] = list(reversed(value["reports"]))
        fail("final-unsorted", lambda: run_with(161, write("r.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["final_monitor_set_sequence"] = 1
        fail("final-set-sequence", lambda: run_with(161, write("s.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["final_monitor_set_sha256"] = "0" * 64
        fail("final-set-digest", lambda: run_with(161, write("t.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][0]["statement"]["final_monitor_set_sequence"] = 1
        fail("final-statement-set-sequence", lambda: run_with(161, write("u.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][0]["statement"]["final_monitor_set_sha256"] = "0" * 64
        fail("final-statement-set-digest", lambda: run_with(161, write("v.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][0]["statement"]["monitor_id"] = rotation2_verify.CUMULATIVE_REVOKED_MONITOR_IDS[0]
        fail("revoked-m1-resurrection", lambda: run_with(161, write("w.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["reports"][0]["statement"]["monitor_id"] = rotation2_verify.CUMULATIVE_REVOKED_MONITOR_IDS[1]
        fail("revoked-m2-resurrection", lambda: run_with(161, write("x.json", value))); cases += 1

        for idx, field in enumerate(sorted(monitor_verify.TARGET_KEYS)):
            value = copy.deepcopy(final_bundle); value["reports"][0]["statement"][field] = replacement_for(field)
            fail(f"final-target-{field}", lambda v=value, n=f"ft{idx}.json": run_with(161, write(n, v))); cases += 1
        ...
        fail("final-signature", lambda: run_with(161, write("y.json", value))); cases += 1
        value = copy.deepcopy(final_bundle); value["production"] = True
        fail("final-production", lambda: run_with(161, write("z.json", value))); cases += 1

        noncanonical = root / "noncanonical.json"
        noncanonical.write_text(json.dumps(final_bundle, indent=2) + "\n", encoding="utf-8")
        fail("noncanonical-final-bundle", lambda: run_with(161, noncanonical)); cases += 1
        fail("first-successor-replay", lambda: run_with(161, base[158])); cases += 1
        fail("observed-valid-final-same-parent-fork", lambda: run_with(161, fork_path)); cases += 1

    if cases != 53:
        raise AssertionError(f"unexpected RUST-092 selftest case count: {cases}")
    print("RUST-092 multi-step monitor rotation fail-closed contract: 53/53 expected cases passed")
