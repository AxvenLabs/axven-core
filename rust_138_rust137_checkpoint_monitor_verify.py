#!/usr/bin/env python3
"""RUST-138: TEST-ONLY RUST-137 monitor-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-rust138-monitor-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-rust138-monitor-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-rust138-monitor-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST138_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

MONITOR_1_ID = "rust-134-test-only-monitor-rotation-journal-monitor-1-v1"
MONITOR_2_ID = "rust-134-test-only-monitor-rotation-journal-monitor-2-v1"
MONITOR_3_ID = "rust-134-test-only-monitor-rotation-journal-monitor-3-v1"
MONITOR_1_PUBLIC_KEY = bytes.fromhex("953a13f9ce9ea62b68cf4676de7851d49ce1cb55642d4f425eb271d471e3b034")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("53840868cb293a6e0a636b1f2245befbe1988fb287bd6a9d84df5df4a519dd11")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("8d4abe074fef9229d3b441dfea4f98f805b1a2b3a06ae645810efece77fd5044")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust137_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust137_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-138 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-138 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-138 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-138 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-138 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-138 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-138 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-137 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-138 monitor report does not match canonical RUST-137 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-138 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-138 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-138 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 290:
        raise AssertionError("unexpected RUST-138 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 288:
        raise AssertionError("unexpected RUST-138 path count")
    base_paths = path_args[:287]
    monitor_bundle_path = path_args[287]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[285], "RUST-137 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 292 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_138_rust137_checkpoint_monitor_verify.py verify "
            "... RUST137_FINAL_CHECKPOINT MONITOR_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
