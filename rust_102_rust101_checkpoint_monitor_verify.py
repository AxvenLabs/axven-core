#!/usr/bin/env python3
"""RUST-102: TEST-ONLY RUST-101 monitor-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-rust102-monitor-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-rust102-monitor-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-rust102-monitor-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST102_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2

MONITOR_1_ID = "rust-102-test-only-monitor-rotation-journal-monitor-1-v1"
MONITOR_2_ID = "rust-102-test-only-monitor-rotation-journal-monitor-2-v1"
MONITOR_3_ID = "rust-102-test-only-monitor-rotation-journal-monitor-3-v1"
MONITOR_1_PUBLIC_KEY = bytes.fromhex("7866ae77a15ed3598ce780acaa13dbc3280f1716a520a9f3384aa4f701c99e6e")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("ee692b4356f36034599f6fcd56c438f501378d3b0559d9b2ca57f036b876f52e")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("0b513ad9b4924015ca0902ed079044d3ac5dbec2306f06948c10da8eb6e39f2d")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust101_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust101_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-102 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-102 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-102 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-102 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-102 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-102 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-102 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-101 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-102 monitor report does not match canonical RUST-101 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-102 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-102 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-102 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 191:
        raise AssertionError("unexpected RUST-102 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 189:
        raise AssertionError("unexpected RUST-102 path count")
    base_paths = path_args[:188]
    monitor_bundle_path = path_args[188]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[186], "RUST-101 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 193 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_102_rust101_checkpoint_monitor_verify.py verify "
            "... RUST101_FINAL_CHECKPOINT MONITOR_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
