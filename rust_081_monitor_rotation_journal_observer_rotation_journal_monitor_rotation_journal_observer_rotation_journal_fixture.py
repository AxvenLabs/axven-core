#!/usr/bin/env python3
"""RUST-081 TEST-ONLY RUST-077 checkpoint observer rotation journal producer."""
...
def private_for(observer_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[observer_id]:
        raise AssertionError("RUST-081 TEST-only observer public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_081_monitor_rotation_journal_observer_rotation_journal_monitor_rotation_journal_observer_rotation_journal_fixture.py SOURCE_SHA"
        )
    source_sha = sys.argv[1]

    checkpoint_raw, observed_checkpoint = floor_verify.load_canonical(
    ...
    target = observer_verify.canonical_target(
        checkpoint_raw, observed_checkpoint, source_sha
    )

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust078-monitor-rotation-journal-observer-bundle.json",
        "RUST-081 old observer bundle",
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust079-observer-set-rotation.json",
        "RUST-081 first observer rotation",
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust079-observer-set-rotation-auth.json",
        "RUST-081 first rotation authorization",
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust079-successor-observer-bundle.json",
        "RUST-081 first successor observer bundle",
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust080-second-observer-set-rotation.json",
        "RUST-081 second observer rotation",
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust080-second-observer-set-rotation-auth.json",
        "RUST-081 second rotation authorization",
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust080-final-observer-bundle.json",
        "RUST-081 final observer bundle",
    )
    ...
    (OUT / "axven-rust081-prefix-observer-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust081-prefix-observer-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust081-final-observer-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust081-final-observer-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust081-observed-fork-observer-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-081 TEST-only RUST-077 checkpoint observer rotation journal fixture: GREEN")
