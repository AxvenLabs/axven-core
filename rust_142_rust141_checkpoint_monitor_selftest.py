#!/usr/bin/env python3
"""RUST-142 detached availability and fail-closed monitor selftest."""
...
def main() -> None:
    if len(sys.argv) != 303:
        raise SystemExit(
            "usage: rust_142_rust141_checkpoint_monitor_selftest.py "
    ...
    if len(base) != 299:
        raise AssertionError("unexpected RUST-142 selftest base path count")

    monitor_verify.verify(*base, source_sha, required_floor)

    final_journal_raw, _ = floor_verify.load_canonical(
        base[296], "RUST-141 final monitor rotation journal"
    )
    ...
    final_statement = journal_verify.validate_checkpoint_envelope(
        ...
        "RUST-142 selftest canonical",
    )
    target = monitor_verify.checkpoint_target(final_checkpoint_raw, final_statement)
    _, bundle = floor_verify.load_canonical(base[298], "RUST-142 monitor bundle")
    _, fork_bundle = floor_verify.load_canonical(
        fork_bundle_path, "RUST-142 observed-fork monitor bundle"
    )
    ...
    for subset in itertools.combinations(bundle["reports"], monitor_verify.THRESHOLD):
        ...
        availability += 1
    if availability != 3:
        raise AssertionError("unexpected RUST-142 monitor subset count")
    print("[GREEN] RUST-142 monitor availability: 3/3 valid two-monitor subsets accepted")

    fork_checkpoint_raw, fork_checkpoint = floor_verify.load_canonical(
        Path("/tmp/axven-rust141-observed-fork-monitor-rotation-checkpoint.json"),
        "RUST-141 observed fork checkpoint",
    )
    ...
    print("[GREEN] RUST-142 signed observed same-parent fork evidence validated")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        paths = list(base); paths[298] = noncanonical
        fail(
            "noncanonical-monitor-bundle",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[298] = base[297]
        fail(
            "rust141-checkpoint-replay",
            lambda: monitor_verify.verify(*paths, source_sha, required_floor),
        ); cases += 1

        paths = list(base); paths[298] = fork_bundle_path
        fail(
    ...
    if cases != 27:
        raise AssertionError(f"unexpected RUST-142 fail-closed case count: {cases}")
    print("RUST-142 RUST-141 journal checkpoint monitor fail-closed contract: 27/27 expected cases passed")
