#!/usr/bin/env python3
"""RUST-054 TEST-ONLY journal-monitor-journal observer producer."""
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit(
            "usage: rust_054_journal_monitor_journal_gossip_fixture.py SOURCE_SHA"
        )
    source_sha = sys.argv[1]

    final_journal_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust053-final-journal-monitor-journal.json",
        "final journal-monitor journal",
    )
    ...
    fork_raw, fork_checkpoint = floor_verify.load_canonical(
        ...
        "observed fork journal-monitor checkpoint",
    )
    ...
    (OUT / "axven-rust054-journal-monitor-journal-observer-bundle.json").write_bytes(
        material_verify.canonical(bundle(reports))
    )
    (OUT / "axven-rust054-observed-fork-bundle.json").write_bytes(
        material_verify.canonical(bundle([reports[0], reports[1], fork_report]))
    )
    print("RUST-054 TEST-only journal-monitor-journal observation fixture: GREEN")
