#!/usr/bin/env python3
"""RUST-134: TEST-ONLY RUST-133 monitor-rotation-journal checkpoint monitor verifier."""
...
BUNDLE_SCHEMA = "axven-native-rust134-monitor-rotation-journal-checkpoint-monitor-bundle-v1"
REPORT_SCHEMA = "axven-native-rust134-monitor-rotation-journal-checkpoint-monitor-report-v1"
STATEMENT_SCHEMA = "axven-native-rust134-monitor-rotation-journal-checkpoint-monitor-statement-v1"
MONITOR_DOMAIN = b"AXVEN_NATIVE_RUST134_MONITOR_ROTATION_JOURNAL_CHECKPOINT_MONITOR_V1\x00"
ALGORITHM = "ed25519"
THRESHOLD = 2
...
MONITOR_1_PUBLIC_KEY = bytes.fromhex("014e4d922c596a380adfc9eed722e835822f1236761693e4babb61e1afbb04f1")
MONITOR_2_PUBLIC_KEY = bytes.fromhex("51bfae7fa92c515512cd3ad442760e41612e59dfec3913f670b721c04255b068")
MONITOR_3_PUBLIC_KEY = bytes.fromhex("ab3fa2fdab77f98706c76697b42bc99d7b5f967cadcd59c19633ea502fd1dcf3")
PINNED_MONITORS = {
...
CHECKPOINT_SHA_KEY = "rust133_monitor_rotation_journal_checkpoint_sha256"
CHECKPOINT_STATEMENT_SHA_KEY = "rust133_monitor_rotation_journal_checkpoint_statement_sha256"
...
def checkpoint_target(checkpoint_raw: bytes, statement: dict) -> dict:
    if not isinstance(statement, dict) or frozenset(statement) != journal_verify.STATEMENT_KEYS:
        raise AssertionError("invalid RUST-134 canonical checkpoint statement fields")
    return {
...
def validate_bundle(bundle: dict, target: dict) -> None:
    if (
        ...
        raise AssertionError("invalid RUST-134 monitor bundle fields")
    if type(bundle.get("threshold")) is not int or bundle["threshold"] != THRESHOLD:
        raise AssertionError("invalid RUST-134 monitor threshold")
    if bundle.get("production") is not False:
        raise AssertionError("production RUST-134 monitoring forbidden")
    reports = bundle.get("reports")
    if not isinstance(reports, list) or not (THRESHOLD <= len(reports) <= len(PINNED_MONITORS)):
        raise AssertionError("invalid RUST-134 monitor report count")
    ...
    if ids != sorted(ids) or len(ids) != len(set(ids)):
        raise AssertionError("RUST-134 monitor ids must be unique and sorted")

    for statement in statements:
        if statement["activation_source_commit"] != target["activation_source_commit"]:
            raise AssertionError("RUST-134 monitor source mismatch")
        same_parent = (
        ...
        if same_parent and not exact:
            raise AssertionError("observed monitor same-parent RUST-133 monitor rotation journal checkpoint fork")
        if not exact:
            raise AssertionError("RUST-134 monitor report does not match canonical RUST-133 checkpoint")
...
def validate_observed_fork_evidence(
    ...
    if not same_parent or observed_target == canonical_target:
        raise AssertionError("RUST-134 observed checkpoint is not a distinct same-parent fork")
    reports = bundle.get("reports") if isinstance(bundle, dict) else None
    if not isinstance(reports, list):
        raise AssertionError("invalid RUST-134 observed-fork monitor bundle")
    matched = 0
    for report in reports:
        ...
        if statement_matches_target(statement, observed_target):
            matched += 1
    if matched < 1:
        raise AssertionError("no signed monitor report binds RUST-134 observed valid fork")
    return observed_target


def verify(*args) -> None:
    if len(args) != 279:
        raise AssertionError("unexpected RUST-134 verifier argument count")
    *path_args, expected_source_sha, required_floor_text = args
    if len(path_args) != 277:
        raise AssertionError("unexpected RUST-134 path count")
    base_paths = path_args[:276]
    monitor_bundle_path = path_args[276]

    journal_verify.verify(*base_paths, expected_source_sha, required_floor_text)

    final_journal_raw, _ = floor_verify.load_canonical(
        base_paths[274], "RUST-133 final monitor rotation journal"
    )
...
def main() -> None:
    if len(sys.argv) != 281 or sys.argv[1] != "verify":
        raise SystemExit(
            "usage: rust_134_rust133_checkpoint_monitor_verify.py verify "
            "... RUST133_FINAL_CHECKPOINT MONITOR_BUNDLE SOURCE_SHA REQUIRED_FLOOR"
        )
