#!/usr/bin/env python3
"""RUST-109 TEST-ONLY monitor rotation journal/checkpoint producer."""
...
OUT = Path("/tmp")
SEEDS = {
    monitor_verify.MONITOR_2_ID: "4c" * 32,
    monitor_verify.MONITOR_3_ID: "5c" * 32,
    rotation1_verify.M4_ID: "6c" * 32,
    rotation2_verify.M5_ID: "7c" * 32,
}
...
def private_for(monitor_id: str, pins: dict[str, bytes]) -> Ed25519PrivateKey:
    ...
    if public != pins[monitor_id]:
        raise AssertionError("RUST-109 TEST-only journal monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(
        ...
        raise SystemExit("usage: rust_109_rust106_checkpoint_monitor_rotation_journal_fixture.py SOURCE_SHA")
    ...
    if target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-109 target source mismatch")

    old_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust106-monitor-bundle.json", "RUST-109 old monitor bundle"
    )
    first_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust107-monitor-set-rotation.json", "RUST-109 first monitor rotation"
    )
    first_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust107-monitor-set-rotation-auth.json", "RUST-109 first rotation authorization"
    )
    first_successor_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust107-successor-monitor-bundle.json", "RUST-109 first successor monitor bundle"
    )
    second_rotation_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust108-second-monitor-set-rotation.json", "RUST-109 second monitor rotation"
    )
    second_auth_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust108-second-monitor-set-rotation-auth.json", "RUST-109 second rotation authorization"
    )
    final_bundle_raw, _ = floor_verify.load_canonical(
        OUT / "axven-rust108-final-monitor-bundle.json", "RUST-109 final monitor bundle"
    )
    ...
    (OUT / "axven-rust109-prefix-monitor-rotation-journal.json").write_bytes(prefix_journal_raw)
    (OUT / "axven-rust109-prefix-monitor-rotation-checkpoint.json").write_bytes(prefix_checkpoint_raw)
    (OUT / "axven-rust109-final-monitor-rotation-journal.json").write_bytes(final_journal_raw)
    (OUT / "axven-rust109-final-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(final_checkpoint)
    )
    (OUT / "axven-rust109-observed-fork-monitor-rotation-checkpoint.json").write_bytes(
        material_verify.canonical(fork_checkpoint)
    )
    print("RUST-109 TEST-only monitor rotation journal/checkpoint fixture: GREEN")
