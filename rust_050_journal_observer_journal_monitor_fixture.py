#!/usr/bin/env python3
"""RUST-050 TEST-only journal-monitor producer. Private monitor seeds stay producer-side."""
...
def private_for(monitor_id: str) -> Ed25519PrivateKey:
    ...
    if public != monitor_verify.PINNED_MONITORS[monitor_id]:
        raise AssertionError("RUST-050 TEST-only journal-monitor public-key pin mismatch")
    return private
...
def main() -> None:
    if len(sys.argv) != 2 or len(sys.argv[1]) != 40 or any(c not in "0123456789abcdef" for c in sys.argv[1]):
        raise SystemExit("usage: rust_050_journal_observer_journal_monitor_fixture.py SOURCE_SHA")
    source_sha = sys.argv[1]

    final_raw, final_checkpoint = floor_verify.load_canonical(
    ...
    if canonical_target["activation_source_commit"] != source_sha or fork_target["activation_source_commit"] != source_sha:
        raise AssertionError("RUST-050 checkpoint source mismatch")

    canonical_reports = [
        report(monitor_id, canonical_target) for monitor_id in sorted(monitor_verify.PINNED_MONITORS)
    ]
    ...
    (OUT / "axven-rust050-monitor-bundle.json").write_bytes(material_verify.canonical(canonical_bundle))
    (OUT / "axven-rust050-observed-fork-monitor-bundle.json").write_bytes(
        material_verify.canonical(split_bundle)
    )
    print("RUST-050 TEST-only journal-observer-journal monitor fixture: GREEN")
